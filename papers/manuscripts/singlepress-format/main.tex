...
%% Target: Genome Biology — Software article
...
\author{
  Zach DeBruine\textsuperscript{1,2,*}
  \\[6pt]
  \textsuperscript{1}Department of Information Sciences and Technology, Grand Valley State University, Allendale, MI
  \\[2pt]
...
Across 3{,}253 scRNA-seq datasets spanning nine species and nine protocols, \texttt{.1pz} files are 2--4$\times$ smaller than H5AD and 2.5$\times$ smaller than BPCells.
...
An ablation study across seven datasets (4.6M--113M nonzeros) decomposes the speed advantage into three independent factors.
...
The comparison is apples-to-apples: both read the same matrix, report median wall-clock time over 5 trials with warmup, and measure from function call to usable sparse matrix---but H5AD's use of gzip inside HDF5 precludes multi-threaded decompression even in principle, whereas the \texttt{.1pz} chunk architecture was designed for it.
...
To quantify how file-format I/O overhead limits GPU utilization as a function of model complexity, we swept the hidden dimension of a single-layer autoencoder ($G \to h \to G$, ReLU, Adam) from $h = 32$ to $h = 4{,}096$, corresponding to 8~GFLOP--1~TFLOP per training step ($12 B n h$ FLOPs, batch size $B = 1{,}024$, $n = 20{,}000$ genes).
Three datasets spanning 10K--150K cells were benchmarked on an NVIDIA H100~NVL GPU using the native PyTorch \texttt{OnePZDataset} dataloader with four prefetch workers; H5AD files were read in backed mode with equivalent batch logic.
...
\subsection{Partial reads and USA-resolved storage}

Modern quantification pipelines~\cite{he2022alevin} produce USA-resolved matrices with three count layers (spliced, unspliced, ambiguous) stored as consecutive row blocks.
...
Each batch is assembled as a sparse CSR tensor via zero-copy conversion from the pre-loaded scipy matrix.
The \texttt{collate\_fn} handles sparse-to-sparse batching within the DataLoader's worker processes.
...
All benchmarks were run on a single CPU node (AMD EPYC, 128~GB RAM) with files stored on a shared parallel filesystem.
Timing measurements report the median of 5 replicates with a warm-up run discarded.
...
We swept the hidden dimension of a single-layer autoencoder ($G \to h \to G$, ReLU, Adam) from $h = 32$ to $h = 4{,}096$, corresponding to 8~GFLOP--1~TFLOP per training step ($12Bnh$ FLOPs, batch size $B = 1{,}024$, $n = 20{,}000$ genes).
...
LZ4 provides 3.34$\times$ at 4{,}692~MB/s---a viable fast-read tier that trades 22\% compression for 1.9$\times$ faster decompression.
...
\subsection*{S4. The production codec: bit-planes + bitmap + zstd-1}
...
$B' \gets \text{zstd-1-decompress}(F)$\;
...
The combined pipeline achieves a median 4.89$\times$ compression at 4{,}823~MB/s decompression---simultaneously +4.4\% better ratio and +68\% faster than the previous production codec (byte-split + zstd-1 without pre-filters; Table~\ref{tab:champion}).
...
\begin{table}[h]
\centering
\caption{Production codec (bit-planes + bitmap + zstd-1) vs.\ baselines across four representative datasets spanning the density spectrum.}
\label{tab:champion}
\footnotesize
\begin{tabular}{l r r r r r}
\toprule
...
The LZ4 tier achieves 13{,}000--16{,}000~MB/s decompression (2.7$\times$ faster than zstd) at the cost of 10\% lower ratio, making it suitable for read-hot workloads where decompression throughput is the bottleneck.
LZ4 is also 2.3$\times$ faster on the \emph{write} path (4{,}400 vs.\ 1{,}900~MB/s at level~1), which may be relevant for pipelines where compress speed matters.
However, the 10\% ratio penalty accumulates at scale: for a 100~TB corpus, LZ4 requires $\sim$10~TB more storage than zstd.
...
The penalty is entirely on the write path: zstd-16 compresses at only 45~MB/s, which is 42$\times$ slower than zstd-1.
For write-once/read-many workloads (the typical .1pz use case), this tradeoff is favorable.

\begin{table}[h]
\centering
\caption{Speed tiers for the bit-planes + bitmap pre-filter with different entropy backends (four holdout datasets, 40 chunks, 5 decompress / 3 compress trials per chunk). All tiers share the same pre-filter; only the compressor changes.}
...
bp + bitmap + lz4hc-9      & 4.91$\times$ & 13{,}100 & 77 \\
bp + bitmap + lz4-1        & 4.68$\times$ & 13{,}300 & 4{,}400 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table*}[h]
\centering
\caption{Side information experiments: median compression ratio across four representative datasets. Champion = bit-planes + bitmap + zstd-1 (no side information). Every side-information variant matches or underperforms the champion. For the three sparse datasets (GSE151346/277034/241998), all dictionary and prefix variants achieve $\pm$0.00\% difference from champion.}
\label{tab:side_info}
\footnotesize
...
\midrule
\multicolumn{4}{l}{\textit{Production: bit-planes + bitmap + zstd-1}} \\
...
\paragraph{LZ4 as a fast tier.}
LZ4 provides a qualitatively different speed/ratio tradeoff through the champion pipeline.
On the four holdout datasets (Table~\ref{tab:lz4_detail}), LZ4-1 achieves 2.7$\times$ faster decompression and 2.3$\times$ faster compression than zstd-1, at the cost of 10\% lower ratio.
The decompression advantage scales with sparsity: dense data sees 3.5$\times$ faster reads (4{,}294 vs.\ 1{,}233~MB/s), while very sparse data sees 2.7$\times$ faster reads (15{,}997 vs.\ 5{,}963~MB/s).
LZ4-HC level~9 recovers most of the ratio gap (4.91$\times$ vs.\ 5.20$\times$), maintaining 13{,}100~MB/s decompression, but compresses at only 77~MB/s---slower than zstd-16.

\begin{table}[h]
\centering
\caption{LZ4 vs.\ zstd per-dataset comparison through the champion pre-filter pipeline (bit-planes + bitmap). All speeds are median of 5 decompress / 3 compress trials.}
\label{tab:lz4_detail}
\footnotesize
\begin{tabular}{llrrrr}
\toprule
 & & \multicolumn{2}{c}{Ratio} & \multicolumn{2}{c}{Decomp.\ (MB/s)} \\
\cmidrule(lr){3-4} \cmidrule(lr){5-6}
Dataset & Density & lz4-1 & zstd-1 & lz4-1 & zstd-1 \\
\midrule
GSE135310 & Dense    & 1.53$\times$ & 1.69$\times$ & 4{,}294  & 1{,}233 \\
GSE151346 & Medium   & 4.59$\times$ & 5.08$\times$ & 13{,}100 & 4{,}700 \\
GSE277034 & Sparse   & 4.77$\times$ & 5.33$\times$ & 13{,}590 & 4{,}927 \\
GSE241998 & V.~sparse & 5.82$\times$ & 6.49$\times$ & 15{,}997 & 5{,}963 \\
\bottomrule
\end{tabular}
\end{table}
...
LZ4-1 provides a viable fast-read tier at 2.7$\times$ faster decompression with 10\% lower ratio.
...
Entropy backend & zstd level~16 & zstd 1--22, LZ4, LZ4-HC 1--12, brotli 0--11, LZMA 1/3/6 & z16 Pareto-dominant on pre-filtered output: +1.2\% ratio, +1.4\% decompress vs.\ z1 (Table~\ref{tab:extended_summary}) \\[3pt]
Fast-read tier & LZ4-1 (optional) & LZ4-HC 1/4/9/12 & lz4-1: 2.7$\times$ faster decompress, $-$10\% ratio; lz4hc-9: similar speed, $-$6\% ratio (Tables~\ref{tab:lz4_detail},~\ref{tab:mode_pareto}) \\[3pt]
...
\subsection*{S12. Compression mode system}

The production codec pipeline (bit-plane decomposition $\rightarrow$ bitmap $\rightarrow$ entropy backend) achieves near-theoretical compression ratios (Section~S5), but practical workloads vary in their ratio--speed requirements.
Archival pipelines prioritize file size while interactive analysis needs fast decompression.
We therefore expose three \emph{compression modes} that select the entropy backend and level, while the pre-filter chain remains fixed.

\paragraph{Pareto frontier.}
We benchmarked 14 backend configurations---zstd levels \{1,3,6,9,12,16,17,19,22\}, LZ4-1, and LZ4-HC levels \{1,4,9,12\}---across four holdout datasets spanning density from 0.007\% to 2.4\% (10 chunks per dataset, 3 compress trials, 5 decompress trials).
Three tiers emerge on the Pareto frontier of compression ratio versus decompression throughput (Table~\ref{tab:mode_pareto}).

\begin{table}[h]
\centering
\caption{Compression mode Pareto tiers, averaged across four datasets. Compress and decompress throughput in MB/s (single-threaded, pre-filtered data). Ratio is raw/compressed bytes after the full codec pipeline.}
\label{tab:mode_pareto}
\footnotesize
\begin{tabular}{llrrr}
\toprule
Mode & Backend & Ratio & Compress & Decompress \\
 & & ($\times$) & (MB/s) & (MB/s) \\
\midrule
\texttt{fast}    & LZ4-1    & 1.25 & 1\,097 & 3\,814 \\
\texttt{default} & zstd-3   & 1.38 &   287  & 1\,216 \\
\texttt{small}   & zstd-16  & 1.41 &    12  & 1\,212 \\
\bottomrule
\end{tabular}
\end{table}

The \texttt{fast} mode sacrifices 10\% compression ratio for a $3.1\times$ decompression speedup, suitable for repeatedly queried datasets or latency-sensitive applications.
The \texttt{small} mode gains 2.2\% ratio over \texttt{default} at the cost of $\sim$24$\times$ slower compression; decompression speed is identical to \texttt{default}.
Since the pre-filter pipeline contributes the majority of compression gain (Section~S4), all three modes share the same structural transforms and differ only in the final entropy coding pass.

\paragraph{Format integration.}
The compression mode is stored as a single byte (\texttt{codec\_id}) in the 16-byte reserved region of the \texttt{.1pz} header (byte offset~80): 0~= zstd (default, backward-compatible), 1~= LZ4, 2~= LZ4-HC.
Existing files with zeroed reserved bytes decode correctly as zstd.
On read, the codec is auto-detected from the header; no user intervention is required.
Only the data chunk payloads use the selected backend; metadata sections (column permutation, pointer arrays, column sums, obs/var/uns) always use zstd for their small overhead.

\paragraph{Python API.}
The \texttt{write\_1pz()} function accepts a \texttt{mode} parameter:

\begin{verbatim}
sp.write_1pz("out.1pz", matrix, mode="fast")
sp.write_1pz("out.1pz", matrix, mode="default")
sp.write_1pz("out.1pz", matrix, mode="small")
\end{verbatim}

Each mode sets both the backend and compression level to the optimal Pareto configuration.
Custom zstd levels remain accessible via the \texttt{level} parameter when \texttt{mode="default"}.

#!/usr/bin/env Rscript
## SinglePress manuscript figures — v4r2 (revised panel layout)
##
## Design:
...
##   - 10pt base text
##
## Fig 1: "File format comparison" (3.5 × 7.8, single-col 3×2 = 6 panels)
## Fig 2: "Performance"            (7.2 × 5.2, 3×2 = 5 panels + spacer)
## Fig 3: "I/O throughput"         (3.5 × 4.0, 2×2: writes top, reads bottom)
...
survey <- read.csv("all_datasets_survey.csv", stringsAsFactors = FALSE) %>%
...
io <- read.csv("io_benchmarks.csv", stringsAsFactors = FALSE) %>% filter(nnz > 1e6)
r_bench <- read.csv("r_format_benchmarks.csv", stringsAsFactors = FALSE)
bpcells <- read.csv("bpcells_compression_bench.csv", stringsAsFactors = FALSE)
read_tp <- read.csv("read_throughput.csv", stringsAsFactors = FALSE) %>%
  mutate(sp = species_recode(species)) %>% filter(nnz > 1e6)
write_bench <- read.csv("write_benchmarks.csv", stringsAsFactors = FALSE)
r_write_bench <- tryCatch(
  read.csv("r_write_benchmarks.csv", stringsAsFactors = FALSE),
  error = function(e) NULL
)
vdist <- read.csv("value_distributions.csv", stringsAsFactors = FALSE)
zinb <- read.csv("zinb_data.csv", stringsAsFactors = FALSE) %>%
  mutate(sp = species_recode(species))
ops <- read.csv("operations_benchmark.csv", stringsAsFactors = FALSE)

# Threading: prefer v2 (100 reps) if available
threading <- tryCatch(
  read.csv("threading_benchmark_v2.csv", stringsAsFactors = FALSE),
  error = function(e) read.csv("threading_benchmarks.csv", stringsAsFactors = FALSE)
)

# GPU: FLOP sweep benchmark (H100 NVL, 3 datasets × 8 hidden dims × 2 formats)
gpu_flops <- read.csv("gpu_flop_sweep.csv", stringsAsFactors = FALSE)

cat(sprintf("Loaded: survey=%d (main=%d), io=%d, threading=%d, gpu_flops=%d, ops=%d\n",
    nrow(survey), nrow(survey_main), nrow(io), nrow(threading), nrow(gpu_flops), nrow(ops)))
...
p1b <- ggplot(fmt_tidy %>% filter(!is.na(read_mbps), read_mbps > 0),
  ...
  scale_fill_manual(values = format_pal, guide = "none") +
  scale_color_manual(values = format_pal, guide = "none") +
  labs(x = NULL, y = "Decode (GB/s)", title = "Decode throughput") +
  theme_sp() +
  theme(axis.text.x = element_text(angle = 30, hjust = 1, size = 8))
...
# --- (schematic) File layout diagram for panel (a) ---
p1_schematic <- ggplot() +
  annotate("rect", xmin = 0, xmax = 1, ymin = 8.6, ymax = 9.2,
           fill = "#b2dfdb", color = "grey60", linewidth = 0.25) +
  annotate("text", x = 0.5, y = 8.9, label = "Header (96 B)", size = 2.5) +
  annotate("rect", xmin = 0, xmax = 1, ymin = 7.7, ymax = 8.3,
           fill = "#f5f5f5", color = "grey60", linewidth = 0.25) +
  annotate("text", x = 0.5, y = 8.0, label = "Row permutation", size = 2.5) +
  annotate("rect", xmin = 0, xmax = 1, ymin = 6.8, ymax = 7.4,
           fill = "#f5f5f5", color = "grey60", linewidth = 0.25) +
  annotate("text", x = 0.5, y = 7.1, label = "Column pointers", size = 2.5) +
  annotate("rect", xmin = 0, xmax = 1, ymin = 5.9, ymax = 6.5,
           fill = "#e0e0e0", color = "grey60", linewidth = 0.25) +
  annotate("text", x = 0.5, y = 6.2, label = "Chunk index", size = 2.5) +
  annotate("rect", xmin = 0, xmax = 1, ymin = 4.3, ymax = 5.6,
           fill = "#80cbc4", color = "grey60", linewidth = 0.25) +
  annotate("text", x = 0.5, y = 5.15, label = "Chunks 0 \u2026 N\u22121", size = 2.5) +
  annotate("text", x = 0.5, y = 4.7, label = "(VOCSC + zstd, CRC32)",
           size = 2.0, color = "grey40") +
  annotate("rect", xmin = 0, xmax = 1, ymin = 3.4, ymax = 4.0,
           fill = "#b2dfdb", color = "grey60", linewidth = 0.25) +
  annotate("text", x = 0.5, y = 3.7, label = "Column sums", size = 2.5) +
  annotate("rect", xmin = 0, xmax = 1, ymin = 2.5, ymax = 3.1,
           fill = "#f5f5f5", color = "grey60", linewidth = 0.25) +
  annotate("text", x = 0.5, y = 2.8, label = "Metadata (TLV)", size = 2.5) +
  annotate("rect", xmin = 0, xmax = 1, ymin = 1.6, ymax = 2.2,
           fill = "#fafafa", color = "grey60", linewidth = 0.25, linetype = "dashed") +
  annotate("text", x = 0.5, y = 1.9, label = "Transpose (opt.)",
           size = 2.5, color = "grey50") +
  annotate("rect", xmin = 0, xmax = 1, ymin = 0.7, ymax = 1.3,
           fill = "#b2dfdb", color = "grey60", linewidth = 0.25) +
  annotate("text", x = 0.5, y = 1.0, label = "Footer (16 B)", size = 2.5) +
  annotate("text", x = 1.05, y = 8.9, label = "magic, v3, dims",
           size = 2.0, hjust = 0, color = "grey50") +
  annotate("text", x = 1.05, y = 6.2, label = "u32 \u00d7 N[chunks]",
           size = 2.0, hjust = 0, color = "grey50") +
  annotate("text", x = 1.05, y = 3.7, label = "u64 \u00d7 n",
           size = 2.0, hjust = 0, color = "grey50") +
  annotate("text", x = 1.05, y = 2.8, label = "names, obs, var",
           size = 2.0, hjust = 0, color = "grey50") +
  annotate("text", x = 1.05, y = 1.0, label = "CRC32 + magic",
           size = 2.0, hjust = 0, color = "grey50") +
  coord_cartesian(xlim = c(-0.02, 1.55), ylim = c(0.5, 9.5)) +
  theme_void() +
  labs(title = ".1pz file layout") +
  theme(plot.title = element_text(face = "bold", size = 8.5, hjust = 0),
        plot.margin = margin(2, 4, 2, 2))

# Equal axes for .1pz vs H5AD size panel
max_mb <- max(c(file_sizes$pz_mb, file_sizes$h5ad_mb), na.rm = TRUE) * 1.05
p1c_eq <- p1c + coord_fixed(xlim = c(0, max_mb), ylim = c(0, max_mb)) +
  theme(aspect.ratio = NULL)

# --- Assemble Figure 1 (single-column 3\u00d72) ---
# (a) schematic   (b) value dist.
# (c) comp. ratio (d) decode throughput
# (e) .1pz vs H5AD (equal axes) (f) protocol
fig1 <- (p1_schematic | p1f) / (p1a | p1b) / (p1c_eq | p1e) +
  plot_annotation(tag_levels = "a") &
  theme(plot.tag = element_text(face = "bold", size = 10))

ggsave("fig1_compression.pdf", fig1, width = 3.5, height = 6.5, device = cairo_pdf)
cat("Wrote fig1_compression.pdf (single-column 3\u00d72)\n")

# Species panel (d) moves to supplemental
ggsave("figS2_species.pdf", p1d, width = 3.5, height = 3.0, device = cairo_pdf)
cat("Wrote figS2_species.pdf (species to supplemental)\n")
...
p2a <- ggplot(read_tp, aes(x = nnz / 1e6, y = read_mbps / 1000)) +
  ...
  labs(x = "Nonzeros (millions)", y = "Decode (GB/s)",
       title = "Decode throughput vs. size") +
  theme_sp() +
  theme(panel.grid.major.x = element_line(color = "grey92", linewidth = 0.25))
...
# --- (e) GPU utilization vs. model FLOPs/step ---
gf <- gpu_flops %>%
  mutate(fmt = ifelse(format == "1pz", ".1pz", "H5AD"),
         fmt = factor(fmt, levels = c(".1pz", "H5AD")),
         flops_G = flops_per_step / 1e9,
         dset = factor(label, levels = c("small", "medium", "large")))

# Fit hyperbolic model: GPU_util = 100 * F / (F + K)
fit_1pz <- nls(gpu_util_pct ~ 100 * flops_G / (flops_G + K),
               data = filter(gf, fmt == ".1pz"),
               start = list(K = 500))
fit_h5ad <- nls(gpu_util_pct ~ 100 * flops_G / (flops_G + K),
                data = filter(gf, fmt == "H5AD"),
                start = list(K = 2000))
K_1pz  <- coef(fit_1pz)[["K"]]
K_h5ad <- coef(fit_h5ad)[["K"]]
cat(sprintf("Fitted K (GFLOP): .1pz=%.0f, H5AD=%.0f (ratio=%.1f×)\n",
            K_1pz, K_h5ad, K_h5ad / K_1pz))

# Prediction curves (extend to 3 TFLOP to show extrapolated crossover)
pred_x <- data.frame(flops_G = 10^seq(log10(5), log10(3000), length.out = 200))
pred_curves <- bind_rows(
  pred_x %>% mutate(gpu_util = predict(fit_1pz, .), fmt = ".1pz"),
  pred_x %>% mutate(gpu_util = predict(fit_h5ad, .), fmt = "H5AD")
) %>% mutate(fmt = factor(fmt, levels = c(".1pz", "H5AD")))

p2e <- ggplot(gf, aes(x = flops_G, y = gpu_util_pct, color = fmt)) +
  geom_hline(yintercept = 50, linetype = "dashed", color = "grey65", linewidth = 0.25) +
  geom_line(data = pred_curves, aes(y = gpu_util), linewidth = 0.5, alpha = 0.6) +
  geom_point(aes(shape = dset), size = 1.2, alpha = 0.8) +
  scale_color_manual(values = c(".1pz" = PAL_1PZ, "H5AD" = PAL_H5AD), name = NULL) +
  scale_shape_manual(values = c("small" = 16, "medium" = 17, "large" = 15),
                     labels = c("10K cells", "50K cells", "150K cells"),
                     name = NULL) +
  scale_x_log10(breaks = c(10, 100, 1000),
                labels = c("10", "100", "1000")) +
  scale_y_continuous(limits = c(0, 60), breaks = seq(0, 60, 10),
                     labels = label_number(suffix = "%")) +
  annotate("text", x = 1200, y = 55, label = "compute-bound",
           color = "grey50", size = 2, fontface = "italic") +
  annotate("text", x = 12, y = 2, label = "I/O-bound",
           color = "grey50", size = 2, fontface = "italic") +
  labs(x = "GFLOP / step", y = "GPU utilization",
       title = "GPU compute fraction") +
  theme_sp() +
  theme(legend.position = c(0.30, 0.82),
        legend.key.size = unit(0.15, "cm"),
        legend.text = element_text(size = 6),
        panel.grid.major.x = element_line(color = "grey92", linewidth = 0.25))

# --- (f) Data-loading wait time per batch ---
gf_prep <- gf %>%
  mutate(prep_ms = (prep_s / n_steps) * 1000)

p2f <- ggplot(gf_prep, aes(x = flops_G, y = prep_ms, color = fmt)) +
  geom_point(aes(shape = dset), size = 1.2, alpha = 0.8) +
  geom_smooth(aes(group = fmt), method = "lm", formula = y ~ 1,
              se = TRUE, linewidth = 0.4, alpha = 0.10) +
  scale_color_manual(values = c(".1pz" = PAL_1PZ, "H5AD" = PAL_H5AD), name = NULL) +
  scale_shape_manual(values = c("small" = 16, "medium" = 17, "large" = 15),
  ...
  labs(x = "GFLOP / step", y = "Prep time / batch",
       title = "Data-loading overhead") +
  theme_sp() +
...
# --- Assemble Figure 2 (drop panel f: constant loading is obvious) ---
...
#
# Row 1: (a) Python write  (b) R write
# Row 2: (c) Python read   (d) R read
# ═══════════════════════════════════════════════════════════════════
...
p3a <- ggplot(py_write_tidy, aes(x = nnz / 1e6, y = write_mbps, color = format)) +
  ...
  scale_color_manual(values = c(".1pz" = PAL_1PZ, "H5AD" = PAL_H5AD), name = NULL) +
  scale_x_log10(labels = label_number(), breaks = c(0.1, 1, 10, 100)) +
  scale_y_log10() +
  ...
  labs(x = "Nonzeros (millions)", y = "Write (MB/s)", title = "Python write") +
  theme_sp() +
...
p3b <- ggplot(py_read_tidy, aes(x = nnz / 1e6, y = read_mbps, color = format)) +
  ...
  geom_smooth(method = "loess", se = FALSE, linewidth = 0.5) +
  scale_color_manual(values = c(".1pz" = PAL_1PZ, "H5AD" = PAL_H5AD), name = NULL) +
  scale_x_log10(labels = label_number(), breaks = c(1, 10, 100)) +
  scale_y_log10() +
  annotate("text", x = 2, y = max(py_read_tidy$read_mbps, na.rm = TRUE) * 0.5,
           label = sprintf("%.1f\u00d7", py_read_speedup),
           color = PAL_1PZ, size = 2.8, fontface = "bold", hjust = 0) +
  labs(x = "Nonzeros (millions)", y = "Decode (MB/s)", title = "Python read") +
  theme_sp() +
  theme(legend.position = c(0.70, 0.20),
        legend.key.size = unit(0.15, "cm"),
        panel.grid.major.x = element_line(color = "grey92", linewidth = 0.25))
...
if (!is.null(r_write_bench) && nrow(r_write_bench) > 0) {
  ...
  p3c <- ggplot(r_write_tidy, aes(x = nnz / 1e6, y = write_mbps, color = format)) +
    ...
    annotate("text", x = 0.5, y = max(r_write_tidy$write_mbps) * 0.5,
             ...
             color = PAL_1PZ, size = 2.8, fontface = "bold", hjust = 0) +
    labs(x = "Nonzeros (millions)", y = "Write (MB/s)", title = "R write") +
    theme_sp() +
    theme(legend.position = c(0.70, 0.20),
          legend.key.size = unit(0.15, "cm"),
          panel.grid.major.x = element_line(color = "grey92", linewidth = 0.25))
...
# --- R read data (use native R read_1pz if available, else Python as proxy) ---
has_r_pz_read <- "pz_read_mbps" %in% names(r_bench)
r_read_tidy <- bind_rows(
  if (has_r_pz_read) {
    r_bench %>% filter(!is.na(pz_read_mbps), pz_read_mbps > 0) %>%
      transmute(nnz, format = ".1pz", read_mbps = pz_read_mbps)
  } else {
    io %>% filter(!is.na(read_1pz_mbps), read_1pz_mbps > 0,
...
r_read_paired <- if (has_r_pz_read) {
  r_bench %>% filter(!is.na(pz_read_mbps), !is.na(rds_read_mbps),
                     pz_read_mbps > 0, rds_read_mbps > 0)
} else {
  r_bench %>%
...
r_read_speedup <- if (nrow(r_read_paired) > 0) {
  if (has_r_pz_read) median(r_read_paired$pz_read_mbps / r_read_paired$rds_read_mbps)
  else median(r_read_paired$read_1pz_mbps / r_read_paired$rds_read_mbps)
} else NA
...
p3d <- ggplot(r_read_tidy, aes(x = nnz / 1e6, y = read_mbps, color = format)) +
  ...
  labs(x = "Nonzeros (millions)", y = "Read (MB/s)", title = "R read") +
  theme_sp() +
  theme(legend.position = c(0.70, 0.20),
        legend.key.size = unit(0.15, "cm"),
        panel.grid.major.x = element_line(color = "grey92", linewidth = 0.25))

# --- Assemble Figure 3 (writes top, reads bottom) ---

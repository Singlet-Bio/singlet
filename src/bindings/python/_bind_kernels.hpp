...
struct PyNormalizeResult {
    singlet::gpu::core::DeviceMemory<float>   size_factors;
    singlet::gpu::core::DeviceMemory<uint8_t> qc_mask;
...
// ---------------------------------------------------------------------------
struct PyHvgResult {
    singlet::gpu::core::DeviceMemory<int>   indices;
    singlet::gpu::core::DeviceMemory<float> scores;
    singlet::gpu::core::DeviceMemory<float> mean;
    singlet::gpu::core::DeviceMemory<float> var;
    cudaStream_t                           stream = nullptr;
};
...
struct PySvdResult {
    ...
    singlet::gpu::core::DeviceMemory<float> d_U;    // rows × k  (col-major)
    singlet::gpu::core::DeviceMemory<float> d_d;    // k
    singlet::gpu::core::DeviceMemory<float> d_V;    // cols × k  (col-major)
    int k_selected = 0;
    int rows = 0;
    int cols = 0;
    cudaStream_t stream = nullptr;
...
// ---------------------------------------------------------------------------
struct PyNmfResult {
    singlet::gpu::core::DeviceMemory<float> d_W;   // rows × k  (col-major)
    singlet::gpu::core::DeviceMemory<float> d_d;   // k
    singlet::gpu::core::DeviceMemory<float> d_H;   // k × cols  (row-major in factornet)
    int k_used = 0;
    int iterations = 0;
...
// ---------------------------------------------------------------------------
static singlet::gpu::io::PzDeviceMatrix make_pz_view(PyDeviceCsc& py_csc)
{
...
// ---------------------------------------------------------------------------
static singlet::gpu::core::DeviceMemory<float>
upload_eigen_matrix(const float* host_ptr, std::size_t count)
{
    singlet::gpu::core::DeviceMemory<float> d(count);
    ...
    bool  approximate_median)
{
    ...
    singlet::gpu::preprocess::LogNormConfig cfg;
    cfg.method           = singlet::gpu::preprocess::LogNormMethod::TotalCount;
    ...
    singlet::gpu::preprocess::LogNormResult r =
        singlet::gpu::preprocess::log_normalize(
...
static void py_log1p(py::object mat_obj, float base)
{
    ...
    (void)singlet::gpu::preprocess::log_normalize(
        *py_csc.csc, cfg, py_csc.producer_stream);
    ...
    float       pearson_theta)
{
    ...
    singlet::gpu::preprocess::HvgConfig cfg;
    ...
    if (flavor_str == "seurat_v3" || flavor_str == "seurat") {
        cfg.flavor = singlet::gpu::preprocess::HvgFlavor::SeuratV3;
    } else if (flavor_str == "pearson_residuals" || flavor_str == "pearson") {
        cfg.flavor = singlet::gpu::preprocess::HvgFlavor::PearsonResiduals;
    } else {
    ...
    singlet::gpu::preprocess::HvgResult r =
        singlet::gpu::preprocess::select_hvg(
            *py_csc.csc, cfg, py_csc.producer_stream);
...
static singlet::gpu::reduce::svd::SvdConfig build_svd_config(
    int      k,
    bool     center,
    bool     scale,
    float    tol,
    ...
    uint64_t seed)
{
    singlet::gpu::reduce::svd::SvdConfig cfg;
    cfg.k_max    = k;
    cfg.center   = center;
    cfg.scale    = scale;
    cfg.tol      = tol;
...
static PySvdResult upload_svd_result(
    const singlet::gpu::reduce::svd::SvdResult& r,
...
{
    auto& py_csc = mat_obj.cast<PyDeviceCsc&>();
    singlet::gpu::io::PzDeviceMatrix pz = make_pz_view(py_csc);
    auto cfg = build_svd_config(k, center, scale, tol, max_iter, seed);
    auto r = singlet::gpu::reduce::svd::auto_select(pz, k, cfg);
    ...
    bool center, bool scale, float tol, int max_iter, uint64_t seed)
{
    ...
    singlet::gpu::io::PzDeviceMatrix pz = make_pz_view(py_csc);
    auto cfg = build_svd_config(k, center, scale, tol, max_iter, seed);
    auto r = singlet::gpu::reduce::svd::randomized(pz, cfg);
    return upload_svd_result(r, pz.mat.rows, pz.mat.cols, pz.producer_stream);
}
    ...
    py::kwargs kwargs)
{
    ...
    auto r = singlet::gpu::reduce::svd::deflation(pz, cfg);
    return upload_svd_result(r, pz.mat.rows, pz.mat.cols, pz.producer_stream);
}
...
static singlet::gpu::reduce::nmf::NmfConfig build_nmf_config(
    ...
    int max_iter, float tol, uint64_t seed)
{
    ...
    if (loss_str == "MSE" || loss_str == "mse") {
        cfg.loss.type = singlet::gpu::reduce::nmf::LossType::MSE;
    } else if (loss_str == "KL" || loss_str == "kl") {
        cfg.loss.type = singlet::gpu::reduce::nmf::LossType::KL;
    } else if (loss_str == "MAE" || loss_str == "mae") {
        cfg.loss.type = singlet::gpu::reduce::nmf::LossType::MAE;
    } else {
        throw py::value_error(
...
static PyNmfResult upload_nmf_result(
    const singlet::gpu::reduce::nmf::NmfResult& r,
    ...
    int max_iter, float tol, uint64_t seed)
{
    ...
    auto r = singlet::gpu::reduce::nmf::fit(pz, cfg);
...
static PyNmfResult py_nmf_chunked(
    ...
    int max_iter, float tol, uint64_t seed)
{
    auto& loader = loader_obj.cast<singlet::gpu::io::PzDataLoader&>();
    auto cfg = build_nmf_config(rank, loss, solver_mode, init_mode, max_iter, tol, seed);
    auto r = singlet::gpu::reduce::nmf::chunked_fit(loader, cfg);
    ...
    py::list inputs_list, int rank, std::string topology)
{
    ...
    std::vector<singlet::gpu::io::PzDeviceMatrix> pz_views;
    ...
    namespace G = singlet::gpu::reduce::nmf::graph;
    using SpMat = G::SpMat;
...
convert_pipeline_result(singlet::gpu::streaming::PipelineResult&& r)
{
...
{
    singlet::gpu::streaming::PipelineConfig cfg;
    ...
    auto r = singlet::gpu::streaming::run_pipeline(cfg, /*stream=*/nullptr);
...
static singlet::gpu::core::DeviceDense
build_dense_view(py::object arr, int& rows_out, int& cols_out)
{
    ...
    return singlet::gpu::core::DeviceDense::wrap(ptr, rows_out, cols_out);
}

// convert_knn_result — move C++ KnnResult → PyKnnResult.
static std::shared_ptr<PyKnnResult>
convert_knn_result(singlet::gpu::graph::KnnResult&& r, cudaStream_t stream)
{
    ...
    int         hnsw_ef)
{
    ...
    singlet::gpu::core::DeviceDense emb = build_dense_view(embedding_obj, n, d);
    ...
    if (backend_str == "auto" || backend_str == "Auto")
        ...
        cfg.backend = singlet::gpu::graph::KnnBackend::Cagra;
    else
    ...
    if (metric_str == "L2" || metric_str == "l2")
        ...
        cfg.metric = singlet::gpu::graph::DistanceMetric::Inner;
    else
        throw py::value_error("knn_graph: unknown metric '" + metric_str
                              + "'. Use 'L2', 'Cosine', or 'Inner'.");

    cudaStream_t stream = singlet::gpu::core::default_context().stream();
    auto r = singlet::gpu::graph::compute_knn(emb, cfg, stream);
...
static singlet::gpu::graph::KnnResult
borrow_knn_result(PyKnnResult& py_knn)
{
    ...
    singlet::gpu::graph::KnnResult r;
    r.n            = py_knn.n;
    r.k            = py_knn.k;
    r.backend_used = singlet::gpu::graph::KnnBackend::Auto;

    const std::size_t n1 = static_cast<std::size_t>(py_knn.n + 1);
    const std::size_t nk = static_cast<std::size_t>(py_knn.n) * py_knn.k;

    r.row_offsets = singlet::gpu::core::DeviceMemory<int>::wrap(
        py_knn.row_offsets.data(), n1);
    r.neighbors   = singlet::gpu::core::DeviceMemory<int>::wrap(
        py_knn.neighbors.data(), nk);
    r.distances   = singlet::gpu::core::DeviceMemory<float>::wrap(
    ...
    float       gaussian_sigma)
{
    ...
    if (weight_function_str == "Connectivity" || weight_function_str == "connectivity")
        ...
        cfg.weight_function = singlet::gpu::graph::LeidenWeight::Inverse;
    else if (weight_function_str == "Gaussian" || weight_function_str == "gaussian")
        cfg.weight_function = singlet::gpu::graph::LeidenWeight::Gaussian;
    else
    ...
    auto r = singlet::gpu::graph::leiden(knn, cfg, stream);
    ...
    float       gaussian_sigma)
{
    ...
    cudaStream_t stream = py_knn.stream;
    auto results = singlet::gpu::graph::leiden_multi(knn, resolutions, cfg, stream);
    ...
    int         negative_sample_rate)
{
    ...
    singlet::gpu::embed::UmapConfig cfg;
    ...
    if (init_str == "Random" || init_str == "random")
        cfg.init = singlet::gpu::embed::UmapInit::Random;
    else if (init_str == "Spectral" || init_str == "spectral")
        cfg.init = singlet::gpu::embed::UmapInit::Spectral;
    else
        throw py::value_error("umap_embed: unknown init '" + init_str
                              + "'. Use 'Random' or 'Spectral'.");

    cudaStream_t stream = py_knn.stream;
    auto r = singlet::gpu::embed::umap(knn, cfg, stream);
...
static singlet::gpu::core::DeviceMemory<int>
borrow_int_array(py::object arr, const char* name)
{
    ...
    return singlet::gpu::core::DeviceMemory<int>::wrap(ptr, n);
}

// convert_cluster_markers — move C++ ClusterMarkers → PyClusterMarkers.
static std::shared_ptr<PyClusterMarkers>
convert_cluster_markers(singlet::gpu::de::ClusterMarkers&& cm, cudaStream_t stream)
{
    ...
    bool       deterministic)
{
    ...
    auto r = singlet::gpu::de::wilcoxon_de(*py_csc.csc, labels, n_clusters, cfg, stream);
    ...
    bool       deterministic)
{
    ...
    auto r = singlet::gpu::de::ttest_de(*py_csc.csc, labels, n_clusters, cfg, stream);
...
static singlet::gpu::anno::GeneSetDB
    ...
    int         min_n_genes_per_set)
{
    ...
    singlet::gpu::anno::MarkerScoreConfig cfg;
    cfg.min_n_genes_per_set = min_n_genes_per_set;

    if (method_str == "Mlm" || method_str == "mlm")
        cfg.method = singlet::gpu::anno::MarkerMethod::Mlm;
    else if (method_str == "Ulm" || method_str == "ulm")
        cfg.method = singlet::gpu::anno::MarkerMethod::Ulm;
    else if (method_str == "Wsum" || method_str == "wsum")
        cfg.method = singlet::gpu::anno::MarkerMethod::Wsum;
    else if (method_str == "UCell" || method_str == "ucell")
        cfg.method = singlet::gpu::anno::MarkerMethod::UCell;
    else
        throw py::value_error("marker_score: unknown method '" + method_str
                              + "'. Use 'Mlm', 'Ulm', 'Wsum', or 'UCell'.");

    auto r = singlet::gpu::anno::marker_score(*py_csc.csc, db, cfg, stream);
...
py_load_celltypist_model(const std::string& npz_path)
{
    ...
    auto m_cpp = singlet::gpu::anno::load_celltypist_model(npz_path);
    ...
    py::object model_obj)
{
    ...
    singlet::gpu::anno::CelltypistModel m_borrow;
    m_borrow.n_classes = model.n_classes;
    m_borrow.n_pcs     = model.n_pcs;
    m_borrow.class_names = model.class_names;
    m_borrow.weights    = singlet::gpu::core::DeviceMemory<float>::wrap(
        model.weights.data(), model.weights.size());
    m_borrow.intercepts = singlet::gpu::core::DeviceMemory<float>::wrap(
        model.intercepts.data(), model.intercepts.size());

    cudaStream_t stream = model.stream;
    auto r = singlet::gpu::anno::project_to_reference(emb, m_borrow, stream);
...
borrow_float_array(py::object arr, const char* name)
{
    ...
    return singlet::gpu::core::DeviceMemory<float>::wrap(ptr, n);
}
    ...
    uint64_t   seed)
{
    ...
    auto r = singlet::gpu::gsea::fgsea(stats, db, cfg, /*stream=*/nullptr);
    ...
    uint64_t   seed)
{
    ...
    auto r = singlet::gpu::gsea::aucell(*py_csc.csc, db, cfg, stream);
    ...
    uint64_t   seed)
{
    ...
    singlet::gpu::integrate::HarmonyConfig cfg;
    ...
    cudaStream_t stream = singlet::gpu::core::default_context().stream();
    auto r = singlet::gpu::integrate::harmony(emb, batch_labels, n_batches, cfg, stream);
    ...
    int        approx_threshold)
{
    ...
    singlet::gpu::integrate::BbknnConfig cfg;
    cfg.k_within         = k_within;
    cfg.approx_threshold = approx_threshold;

    cudaStream_t stream = singlet::gpu::core::default_context().stream();
    auto r = singlet::gpu::integrate::bbknn(emb, batch_labels, n_batches, cfg, stream);
    return convert_knn_result(std::move(r), stream);
}
    ...
    bool       compute_velocity)
{
    ...
    singlet::gpu::preprocess::VelocityPrepConfig cfg;
    ...
    const singlet::gpu::graph::KnnResult* knn_ptr = nullptr;
    std::shared_ptr<singlet::gpu::graph::KnnResult> knn_owned;
    if (!knn_for_smoothing.is_none()) {
        ...
        knn_owned = std::make_shared<singlet::gpu::graph::KnnResult>(
            borrow_knn_result(py_knn));
        knn_ptr = knn_owned.get();
    }

    auto r = singlet::gpu::preprocess::velocity_prep(
    ...
    uint64_t   seed)
{
    ...
    auto r = singlet::gpu::anno::call_clones(*alt.csc, *depth.csc, cfg, stream);

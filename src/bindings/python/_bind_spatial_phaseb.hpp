...
//   singlet::gpu::spatial::flash_deconv / stagate
//   singlet::gpu::embed::cell2fate_fit
...
struct PyFlashDeconvResult {
    singlet::gpu::core::DeviceMemory<float> abundance;    // n_spots × n_types
    singlet::gpu::core::DeviceMemory<float> uncertainty;  // n_spots × n_types
...
struct PyStagateResult {
    singlet::gpu::core::DeviceMemory<float> embedding;       // n_spots × d_embed
    singlet::gpu::core::DeviceMemory<int>   spatial_domain;  // n_spots (Leiden labels)
...
struct PyCell2FateResult {
    singlet::gpu::core::DeviceMemory<float> alpha;            // [n_genes]
    singlet::gpu::core::DeviceMemory<float> beta;             // [n_genes]
    singlet::gpu::core::DeviceMemory<float> gamma;            // [n_genes]
    singlet::gpu::core::DeviceMemory<float> alpha_var;        // [n_genes]
    singlet::gpu::core::DeviceMemory<float> beta_var;         // [n_genes]
    singlet::gpu::core::DeviceMemory<float> gamma_var;        // [n_genes]
    singlet::gpu::core::DeviceMemory<float> module_loadings;  // [K × n_genes]
    singlet::gpu::core::DeviceMemory<float> cell_activities;  // [n_cells × K]
    singlet::gpu::core::DeviceMemory<float> velocity;         // [n_cells × n_genes] or empty
    singlet::gpu::core::DeviceMemory<int>   module_topgenes;  // [K × top_n]
    ...
    uint64_t           seed)
{
    ...
    const singlet::gpu::core::DeviceCSC& sc = *spatial_counts.csc;
    const singlet::gpu::core::DeviceCSC& rc = *reference_counts.csc;
    ...
    singlet::gpu::core::DeviceMemory<int> d_ref_types(n_ref_cells);
    cudaMemcpy(d_ref_types.get(), rct_data_ptr,
               n_ref_cells * sizeof(int), cudaMemcpyDeviceToDevice);

    // Optional spatial_coords.
    singlet::gpu::core::DeviceMemory<float> d_coords_buf;
    const singlet::gpu::core::DeviceMemory<float>* coords_ptr = nullptr;
    ...
    auto r = singlet::gpu::spatial::flash_deconv(sc, rc, d_ref_types, n_types, coords_ptr, cfg, s);
    ...
    uint64_t seed)
{
    ...
    size_t n_coord_elems = py::cast<size_t>(ci_shape[0]) *
                           (ci_shape.size() > 1 ? py::cast<size_t>(ci_shape[1]) : 1u);
    singlet::gpu::core::DeviceMemory<float> d_coords(n_coord_elems);
    cudaMemcpy(d_coords.get(), ci_ptr,
               n_coord_elems * sizeof(float), cudaMemcpyDeviceToDevice);

    singlet::gpu::spatial::StagateConfig cfg;
    ...
    auto r = singlet::gpu::spatial::stagate(mat, d_coords, cfg, s);
...
{
    cudaStream_t s = resolve_stream(stream);
    const singlet::gpu::core::DeviceCSC& sp = *spliced.csc;
    const singlet::gpu::core::DeviceCSC& un = *unspliced.csc;

    singlet::gpu::embed::Cell2FateConfig cfg;
    ...
    auto r = singlet::gpu::embed::cell2fate_fit(sp, un, nullptr, cfg, s);

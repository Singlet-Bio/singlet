...
//
// Performance benchmark: singlet::gpu::graph::leiden vs cugraph Leiden.
//
...
singlet::gpu::bench::RefResult run_leiden_reference(
...
// namespace — the public API exposes singlet::gpu::graph::leiden() (taking
...
// Disabled for compile-gate-phase-2 until the API is resolved.
    ...
    for (int i = 0; i < kWarmup; ++i) {
        auto res = singlet::gpu::graph::leiden_single_resolution(knn_result, leiden_cfg, stream);
        cudaStreamSynchronize(stream);
        (void)res;
    }

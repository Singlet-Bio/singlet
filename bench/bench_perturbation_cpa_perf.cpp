...
//
// Performance benchmark: singlet::gpu::perturbation::train_perturb_graph (CPA)
...
singlet::gpu::bench::RefResult run_cpa_reference(
...
int main()
{
    ...
    singlet::gpu::core::GPUContext ctx(stream);

    // ── Warmup ─────────────────────────────────────────────────────────────────
    std::printf("[cpa_bench] Warming up (%d iters × %d epochs)...\n", kWarmup, kNEpochs);
    for (int i = 0; i < kWarmup; ++i) {
        auto res = singlet::gpu::perturbation::train_perturb_graph(

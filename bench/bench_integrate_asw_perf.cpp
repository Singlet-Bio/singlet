...
// singlet/gpu/bench/bench_integrate_asw_perf.cpp
//
// CYCLE-170 Phase E — integrate/asw standalone kernel benchmark.
//
// Benchmarks integrate::asw() (from include/singlet/gpu/integrate/asw.h)
// against a manual numpy CPU baseline at two medium scales:
//
//   Scale "10k":  n_cells=10000, n_pcs=50, k=10, n_clusters=4
//   Scale "30k":  n_cells=30000, n_pcs=50, k=15, n_clusters=4
//
// GPU kernel: one block per cell; shared-memory float smem_sum[n_labels] and
//   int smem_cnt[n_labels]; thread 0 sweeps kNN row accumulating distances by
//   label; computes a (intra), b (min inter-cluster mean), silhouette = (b-a)/max(a,b).
...
//   ASW timing — 2 warmup + 5 timed via cudaEvent (bench::BenchTimer).
//   Memory       — bench::PeakMemTracker (cudaMemGetInfo delta).
//   Cluster labels — round-robin: label[c] = c % n_clusters (device-side).
//
// Output (stdout): CSV header + rows:
//   scale,n_cells,n_pcs,k,n_clusters,wall_ms,mem_mb
//
...
// ---------------------------------------------------------------------------
// make_round_robin_labels — device int[n] with label[c] = c % n_clusters.
// Filled on the CPU and uploaded once (labels are constant across all ASW
// bench iterations — only the ASW kernel execution is timed).
// ---------------------------------------------------------------------------
static core::DeviceMemory<int> make_round_robin_labels(int n, int n_clusters,
                                                        cudaStream_t stream)
{
    std::vector<int> h_label(n);
    for (int c = 0; c < n; ++c) h_label[c] = c % n_clusters;
    ...
    double&                        out_mem_mb)
{
    ...
    for (int iter = 0; iter < WARMUP + TIMED; ++iter) {
        ...
        {
            integrate::AswConfig cfg{};
            auto result = integrate::asw(knn, d_label.get(), n_clusters,
                                         cfg, stream);
            (void)result;
        }
...
int main() {
    if (!gpu_available()) {
        std::puts("NO GPU — skipping Cycle 170 integrate/asw bench.");
        return 0;
    }
    ...
    // CSV header.
    std::printf("scale,n_cells,n_pcs,k,n_clusters,wall_ms,mem_mb\n");
    std::fflush(stdout);
    ...
    for (const auto& s : scales) {
        ...
        // Build kNN graph (untimed warmup — not part of ASW timing).
        ...
        // Build round-robin cluster labels (untimed).
        core::DeviceMemory<int> d_label =
            make_round_robin_labels(s.n_cells, N_CLUSTERS, stream);

        std::printf("[bench] Timing %s asw() (n_clusters=%d k=%d)...\n",
                    s.name, N_CLUSTERS, s.k);
        std::fflush(stdout);

        double wall_ms = 0.0, mem_mb = 0.0;
        bench_asw(knn, d_label, N_CLUSTERS, stream, wall_ms, mem_mb);

        // CSV row.
        std::printf("%s,%d,%d,%d,%d,%.3f,%.1f\n",
                    s.name, s.n_cells, s.n_pcs, s.k, N_CLUSTERS,
                    wall_ms, mem_mb);
        std::fflush(stdout);
    ...
    std::printf("[bench] Cycle 170 integrate/asw perf bench complete.\n");
    return 0;
}

...
//
// Performance benchmark: singlet::gpu::graph::compute_knn vs cuML NearestNeighbors k=15.
//
        ...
        int n, int d, uint64_t seed, cudaStream_t stream)
{
    ...
    singlet::gpu::core::DeviceDense result(n, d);
...
singlet::gpu::bench::RefResult run_knn_reference(
...
int main()
{
    ...
    singlet::gpu::core::DeviceMemory<float> dev_emb(static_cast<size_t>(emb_n) * emb_d);
    cudaMemcpyAsync(dev_emb.get(), emb_host.data(),
                    emb_host.size() * sizeof(float),
                    cudaMemcpyHostToDevice, stream);
    cudaStreamSynchronize(stream);
    singlet::gpu::core::DeviceDense embedding(emb_n, emb_d);
    ...
    for (int i = 0; i < kWarmup; ++i) {
        auto res = singlet::gpu::graph::compute_knn(embedding, cfg, stream);
        cudaStreamSynchronize(stream);
        (void)res;
    }

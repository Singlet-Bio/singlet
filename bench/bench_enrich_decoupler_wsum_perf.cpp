...
// singlet/gpu/bench/bench_enrich_decoupler_wsum_perf.cpp
//
// CYCLE-163 Phase E — enrich/decoupler_wsum standalone kernel benchmark.
//
// Benchmarks decoupler_wsum::wsum() AND decoupler_wsum::wmean() (both from
// include/singlet/gpu/enrich/decoupler_wsum.h) against a manual scipy/numpy
// CPU baseline at two medium scales:
//
//   Scale "10k":  n_cells=10000, n_genes=5000, density=5%, n_pathways=50
//   Scale "30k":  n_cells=30000, n_genes=5000, density=5%, n_pathways=50
//
// GPU kernel: cuSPARSE SpMM (X^T · W) + per-pathway normalisation kernel.
// Reference: Badia-i-Mompel et al. (2022) decoupleR, Bioinformatics Advances.
// Per §J.6 audit: NOT at risk of dense-n×n bug (sparse@dense, O(nnz × p)).
//
...
//   scale,n_cells,n_genes,density,n_pathways,method,wall_ms,mem_mb
//
...
// ---------------------------------------------------------------------------
// bench_method — 2 warmup + 5 timed iterations of one method (wsum or wmean).
// Returns median wall_ms and peak mem_mb.
// ---------------------------------------------------------------------------
enum class Method { WSUM, WMEAN };

static void bench_method(
    ...
    double&                   out_mem_mb)
{
    ...
    for (int iter = 0; iter < WARMUP + TIMED; ++iter) {
        ...
        if (method == Method::WSUM) {
            enrich::WsumConfig cfg{};
            auto result = enrich::wsum(pzmat, d_W, n_pathways, cfg, stream);
            (void)result;
        } else {
            enrich::WmeanConfig cfg{};
            auto result = enrich::wmean(pzmat, d_W, n_pathways, cfg, stream);
            (void)result;
        }
...
int main() {
    if (!gpu_available()) {
        std::puts("NO GPU — skipping Cycle 163 decoupler_wsum bench.");
        return 0;
    }
    ...
    // CSV header.
    std::printf("scale,n_cells,n_genes,density,n_pathways,method,wall_ms,mem_mb\n");
    std::fflush(stdout);
    ...
    // Methods.
    struct MethodSpec { const char* name; Method method; };
    ...
    for (const auto& s : scales) {
        ...
        for (const auto& m : methods) {
            std::printf("[bench] Timing %s %s method=%s...\n",
                        s.name, m.name, m.name);
            std::fflush(stdout);

            double wall_ms = 0.0, mem_mb = 0.0;
            bench_method(pzmat, d_W_buf.get(), n_pathways, m.method,
                         stream, wall_ms, mem_mb);

            // CSV row.
            std::printf("%s,%d,%d,%.2f,%d,%s,%.3f,%.1f\n",
                        s.name, s.n_cells, s.n_genes, s.density, n_pathways,
                        m.name, wall_ms, mem_mb);
            std::fflush(stdout);

            // Registry row.
            bench::BenchRow row;
            row.date          = bench::today_iso();
            row.feature       = "enrich/decoupler_wsum";
            row.scale         = std::string(s.name) + "_" + m.name;
    ...
    std::printf("[bench] Cycle 163 decoupler_wsum perf bench complete.\n");
    return 0;
}

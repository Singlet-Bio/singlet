...
//
// Performance benchmark: singlet::gpu::spatial::flash_deconv vs cell2location Python.
//
...
singlet::gpu::core::DeviceMemory<float> make_reference_B(
        int n_types, int n_genes, uint64_t seed, cudaStream_t stream)
{
    ...
    singlet::gpu::core::DeviceMemory<float> dev(static_cast<size_t>(n_types) * n_genes);
...
singlet::gpu::bench::RefResult run_flash_deconv_reference(
        const singlet::gpu::core::DeviceCSC&         mat,
        const singlet::gpu::core::DeviceMemory<float>& d_B,
...
int main()
{
    ...
    for (int i = 0; i < kWarmup; ++i) {
        auto res = singlet::gpu::spatial::flash_deconv(mat, d_B, n_types, cfg, stream);
        cudaStreamSynchronize(stream);
        (void)res;
    }

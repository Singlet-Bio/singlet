...
//
// Performance benchmark: singlet::gpu::spatial::run_stagate vs STAGATE Python.
//
...
singlet::gpu::core::DeviceDense make_synthetic_coords(
        int n_spots, uint64_t seed, cudaStream_t stream)
{
    ...
    singlet::gpu::core::DeviceMemory<float> dev(static_cast<size_t>(n_spots) * 2);
    cudaMemcpyAsync(dev.get(), host.data(), host.size() * sizeof(float),
                    cudaMemcpyHostToDevice, stream);
    cudaStreamSynchronize(stream);
    return singlet::gpu::core::DeviceDense(n_spots, 2, std::move(dev));
    (void)seed;
}
...
singlet::gpu::bench::RefResult run_stagate_reference(
        const singlet::gpu::core::DeviceCSC&   mat,
        const singlet::gpu::core::DeviceDense& coords,
...
int main()
{
    ...
    singlet::gpu::core::DeviceDense coords;
    ...
    for (int i = 0; i < kWarmup; ++i) {
        auto res = singlet::gpu::spatial::run_stagate(mat, coords, cfg, stream);
        cudaStreamSynchronize(stream);
        (void)res;
    }

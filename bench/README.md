...
int main(int argc, char** argv) {
    auto cfg = singlet::gpu::bench::parse_args(argc, argv);

    // 1. Skip if no GPU
    if (!singlet::gpu::bench::gpu_available()) {
        std::puts("NO GPU — skipping " BENCH_FEATURE_NAME);
        return 0;
    }

    // 2. Load canonical sample for this scale (or generate tiny synthetic)
    auto mat = singlet::gpu::bench::load_sample(cfg);  // returns DeviceCSC or nullptr
    if (!mat) {
    ...
    double sota_wall_s = singlet::gpu::bench::run_sota_ref(
    ...
    for (int i = 0; i < 3; ++i) {
        singlet::gpu::{module}::{the_kernel}(*mat /*, ...params... */);
    }

    // 5. Timed iterations (5 real iters via cuEvents)
    singlet::gpu::bench::BenchTimer timer;
    for (int i = 0; i < 5; ++i) {
        timer.start();
        singlet::gpu::{module}::{the_kernel}(*mat /*, ...params... */);
        timer.stop();
    ...
    row.mem_mb_peak  = singlet::gpu::bench::peak_mem_delta_mb(mat->bytes_device());
    ...
    singlet::gpu::bench::log_row(row, cfg.registry_path);

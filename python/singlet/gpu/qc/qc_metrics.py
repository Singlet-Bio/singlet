...
def _run_qc_on_device(
    ...
    qc_result = _core.calculate_qc_metrics(device_csc, d_is_mt, d_is_ribo, stream=stream)
...
def calculate_qc_metrics(
    ...
    *,
    ...
    qc_result = _core.calculate_qc_metrics(
        device_csc, d_is_mt, d_is_ribo, stream=stream, deterministic=deterministic
    )

    _qc_result_to_obs_var(working, qc_result, mt_key, ribo_key, is_mt_np, is_ribo_np)
...
def filter_cells(
    ...
    *,
    ...
    if min_genes is not None:
        keep &= n_genes_h >= int(min_genes)
    if max_genes is not None:
        keep &= n_genes_h <= int(max_genes)
    if min_counts is not None:
        keep &= n_umis_h >= float(min_counts)
    if max_counts is not None:
        keep &= n_umis_h <= float(max_counts)
    if max_pct_mt is not None:
        keep &= mt_h <= float(max_pct_mt)
    ...
    class _CaiView:
        def __init__(self, x):
            self.__cuda_array_interface__ = x
    ...
    _qc_result_to_obs_var(filtered, qc_result, mt_key, ribo_key, is_mt_np, is_ribo_np)
    # Restrict QC obs to kept cells.
    for col in [
        "total_counts",
        "n_genes_by_counts",
        f"pct_counts_{mt_key}",
        f"pct_counts_{ribo_key}",
    ]:
...
def filter_genes(
    ...
    *,
    ...
    if min_cells is not None:
        keep &= gene_ncells >= int(min_cells)
    if min_counts is not None:
    ...
    filtered_csc = _core.filter_genes(device_csc, qc_result, min_cells=threshold, stream=stream)
    ...
    _qc_result_to_obs_var(filtered, qc_result, mt_key, ribo_key, is_mt_np, is_ribo_np)
    for col in ["mean_counts", "var_counts", "n_cells_by_counts", f"is_{mt_key}", f"is_{ribo_key}"]:

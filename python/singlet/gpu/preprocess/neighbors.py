...
"""
singlet.gpu.preprocess.neighbors — GPU-native kNN graph construction.
...
code-reader; this function can be substituted without changing call sites:
    ...
    sgpp.neighbors(adata, n_neighbors=15)   # instead of sc.pp.neighbors(adata)
...
def _get_rep(
    ...
    warnings.warn(
        ...
        "Falling back to adata.X.  Consider running singlet.gpu.preprocess.pca() first.",
        UserWarning,
        stacklevel=3,
    )
    x = adata.X
...
def neighbors(
    ...
    **Result locations** (match scanpy exactly):
    ...
    Default kNN (15 neighbours, using X_pca)::
        ...
        import singlet.gpu.preprocess as sgpp
        sgpp.neighbors(adata)

    Custom representation and distance::

        sgpp.neighbors(adata, use_rep='X_nmf', n_neighbors=20, metric='cosine')
    """
    if n_neighbors < 2:
    ...
    if not hasattr(_core, "knn_graph"):
        raise AttributeError(
            ...
            "singlet.gpu.preprocess.neighbors() is callable."
        )

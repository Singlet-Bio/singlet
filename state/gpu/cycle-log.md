...
- Lessons: do not read `/CLAUDE.md` or `singlet/.github/agents/singlet.agent.md`.
...
- Correctness: pending compile + diff vs `singlet.io.read_pz` Python reference
- Dominates on: TBD
- Commit: (uncommitted)
- **Dispatched in parallel**:
  1. `gpu-kernel-dev` (Sonnet) — wrote `core/types.h` (35), `core/handles.h` (59), `core/memory.h` (138), `io/pz_device_loader.h` (971). Total 1203 LOC. Loader is 2.4× over the 400-LOC budget because `PzChunkIterator` is non-trivial and the agent flagged "no stubs allowed". Reasonable. Mirrors `pz_writer.h` byte-level layout independently of `pz_reader.h` to avoid pulling singlet's `pz` namespace into GPU-facing headers.
...
- **Dispatched in parallel**:
  ...
  2. `analysis-validator` (Sonnet, 172s) — wrote `tests/streaming_pipeline_correctness.cpp` (772 LOC, 11 test cases). Added 3 edge cases beyond the design doc: single chunk fits entire input, zero inputs throws, PCA threshold boundary. Uses `make_tiny_pz` helper that calls `singlet::gpu::io::write_pz` — that helper does NOT exist yet, so a follow-up is needed at first GPU dispatch (either implement `write_pz` as a small helper or rewrite the helper to call singlet Python).
...
- **🟡 New follow-up CYCLE-7-FOLLOWUP-WRITE-PZ**: validator's `make_tiny_pz` helper references `singlet::gpu::io::write_pz` which doesn't exist. Either: (a) implement a small `write_pz` helper in `io/pz_device_loader.h` (mirrors `pz_writer.h` byte-level layout), or (b) rewrite the helper to shell out to a Python `singlet.io.write_pz` call. Defer to first GPU dispatch.
...
- Feature: #13 `preprocess/velocity_prep.h` — exploits singlet's exon+intron .1pz outputs directly
...
- **Dispatched in parallel**:
  ...
  2. `analysis-validator` (Sonnet, 256s) — wrote `tests/preprocess_velocity_prep_correctness.cpp` (934 LOC) + `tests/refs/velocity_prep_scvelo_reference.py` (242 LOC). 6 test cases. **Both `exon_counts.1pz` AND `intron_counts.1pz` confirmed present on GSM4037629** — singlet's "unique to us" output is real on disk.
...
| # | Feature | LOC | Type |
|---|---|---|---|
| 0 | `io/pz_device_loader.h` | 971 | wrapper-of-wrapper (singlet .1pz → factornet device CSC) |
...
| 13 | `preprocess/velocity_prep.h` | 915 | **unique to us — exploits singlet exon+intron .1pz** |
...
### "Unique to us" features exploiting singlet-only outputs (3)
- Cycle 13: velocity_prep (exon + intron .1pz from singlet directly)
- Cycle 16: mt_lineage (mt_heteroplasmy.1pz from singlet directly)
- Cycle 17: donor_pseudobulk (donor_assignments.tsv from singlet directly)
...
- **Dispatched in parallel**:
  ...
  3. `lit-scout` (Haiku, 54s) — cycle 38 prep: **NEBULA-GPU single-cell eQTL mapping** as 13th first-GPU candidate. First cycle to exploit singlet's snp_ad.1pz + snp_dp.1pz for eQTL inference (not just donor demux). Design `state/designs/38-nebula-eqtl.md` pre-written same turn.
...
- **🎉 THIRTEENTH "first GPU implementation"**: 12 prior + **NEBULA-eQTL**. FIRST cycle to read `snp_ad.1pz` + `snp_dp.1pz` artifacts — singlet pipeline's genuinely unique outputs now have a downstream analysis tool. No other library can even read these files.
...
- **🎉 FOURTEENTH "first GPU implementation"**: 13 prior + **OmniDoublet multimodal**. FIRST cycle to exploit `adt.1pz` — singlet's CITE-seq ADT output now has a first-class GPU downstream tool.
...
- **Unique-artifact milestone**: Across cycles 38-39, singlet-gpu now reads ALL three of singlet's uniquely-emitted artifacts (snp_ad.1pz, snp_dp.1pz, adt.1pz) as first-class inputs to GPU kernels. No other single-cell library can even parse these files.
...
- **🎉 SEVENTEENTH "first GPU implementation"**: 16 prior + **Monopogen somatic SNV calling**. singlet-gpu now has THREE distinct downstream tools consuming singlet's unique snp_ad/snp_dp outputs: NEBULA (eQTL mapping), DAESC (ASE), Monopogen (variant discovery). No other library reads these files at all; singlet-gpu is the sole platform for downstream genetic analysis on singlet pipeline output.
...
`factornet io/spz_loader` uses a `.spz` (streampress v2) format. singlet produces `.1pz` only. No `.1pz → .spz` conversion utility exists. This baseline cannot be run without a dedicated encoder. Documented in benchmark-registry.md with a `skipped` row. Promotion proceeds on anndata-gpu + scanpy (≥2 baselines per criterion 3).
...
3. **Rule 30 novel pursuit (closed-form size factors from singlet artifacts)** is a headline unique-to-us contribution that extends the "singlet exploit" pattern started in Cycle 56. Two adjacent cycles shipping unique-to-us features is strong signal.
...
- **Lessons**:
  1. singlet outputs are named `exon_counts.1pz` (not `counts.1pz`). Fix all bench scripts. Add a preflight check: `ls $PZ_SMALL || exit 1`.
...
- C++ direct usage via the umbrella header — proven by cycle 92/100 verify (`cpp_minimal` compiles cleanly, `singlet::gpu::*` aliases all resolve).
...
- **Streaming refactor (gpu-kernel-dev Sonnet, background, ~6 min)**:
  - `include/singlet-gpu/io/chunk.h` (NEW, 48 LOC): native `singlet::gpu::io::Chunk` struct (host-side CSC slab) replacing `factornet::io::Chunk<float>`.
  ...
  - `streaming/streamed_pipeline.h` (557 → 555 LOC): 12+ `factornet::gpu::DeviceMemory` → `singlet::gpu::core::DeviceMemory`, 4× `factornet::io::Chunk<float>` → `singlet::gpu::io::Chunk`.
...
- **Files**:
  - `include/singlet-gpu/preprocess/magic.h` (NEW, 475 LOC): three kernels (`csc_transpose_to_dense_kernel`, `row_sum_graph_kernel`, `normalize_weights_kernel`) + ping-pong cuSPARSE SpMM iteration. API: `magic_impute(X, graph, cfg) → MagicResult` where graph is `singlet::gpu::graph::SnnResult`.
...
## Cycle 154 (2026-04-30) — pareto-frontier.md row backfill (21 entries)
- **Feature**: state/pareto-frontier.md — 21 features promoted to frontier from CYCLE-118 onward had no rows; only docs/api pages were written (across CYCLE-150/151/152). This cycle adds the rows so future Phase E cycles can fill numbers into existing schemas rather than restructure each time.
- **Outcome**: 1 Haiku gpu-doc-scribe dispatch. 21 entries added (~24.7 KB), one per feature with cycle reference, citation, and 3-row scale table (small/100k/1M). Schema matches existing entries (qc/metrics is the model). pareto-frontier.md grew from ~9 KB to ~33 KB.
- **Entries added**: preprocess/pearson_residuals (CYCLE-118 — pulled real bench numbers 0.269ms / 8MB / 12,609× vs scanpy from docs/api page), preprocess/magic (CYCLE-124), preprocess/model_gene_var (CYCLE-127), enrich/score_genes (CYCLE-129), enrich/decoupler_{wsum,ulm,ora,mlm,viper} (CYCLE-128/130/132/136/137), integrate/combat (CYCLE-131), integrate/lisi (CYCLE-133), integrate/asw (CYCLE-139), integrate/kbet (CYCLE-140), qc/empty_drops (CYCLE-134), qc/soupx (CYCLE-141), anno/celltypist (CYCLE-135), anno/symphony (CYCLE-138), embed/dpt (CYCLE-142), embed/dendrogram (CYCLE-146), graph/kmeans (CYCLE-149), embed/diffmap (CYCLE-150).
- **Notes**:
  1. **Phase E status field on each entry** — tracks which entries are pending bench cycles (currently 20 of 21; only pearson_residuals has actual SOTA-comparison numbers).
  2. **Total file structure** is now 1 schema header + 11 original + 21 new = 33 frontier-section entries.
- **Lessons**:
  1. **Pareto-frontier rows lag promotion by default** — the orchestrator's per-cycle workflow appends to cycle-log.md but doesn't always touch pareto-frontier.md. Going forward, every cycle that promotes a feature to frontier should add the row immediately (Phase F), not wait for a backfill cycle.
  2. **Small Haiku-only cycles have a place after a high-risk FAIL.** Following CYCLE-153 iter-1 FAIL, a cleanup-style cycle restored signal vs noise and produced concrete artifact value (~25 KB of structured docs) without exposing more risk.
- **Next cycle**: CYCLE-155 candidate options — (a) **CYCLE-122 enrichment zero-output diagnosis** (1 shared root-cause hypothesis: uninitialized scratch buffer), (b) **Phase E benchmark backfill** (start filling the new pareto-frontier.md rows — pearson_residuals is a clean first target since it already has small-scale numbers), (c) **fresh port** (palantir or GRN), or (d) **CYCLE-148.1 Scrublet deeper diag** (with required Phase B research first — could be split across 2 iterations). Default: Phase E for pearson_residuals at 100k/1M scales (low risk, builds on the row added this cycle, fills concrete benchmark data).

## Cycle 155 (2026-04-30) — Phase G publish flow (frontier_sync.py)
- **Feature**: state/pareto-frontier.md → Supabase `gpu_frontier` table → singlet.bio/benchmarks. CYCLE-154 added 21 new pareto-frontier sections but did NOT run the publish flow that Phase G requires for every cycle that updates the frontier. This cycle closes that gap.
- **Outcome**: `scripts/frontier_sync.py` ran clean. Sourced `~/.config/singlet/supabase.env` via `scripts/load_secrets.sh`, ran the sync, 24 entries pushed to `gpu_frontier` (full refresh). The script's TBD filter (`scripts/frontier_sync.py:101-106`) correctly skipped the 20 placeholder rows from the CYCLE-154 backfill — only entries with actual SOTA-comparison numbers reach the public website. The 24-row count matches the previous baseline because CYCLE-154's pearson_residuals row collapses under the existing `hvg` feature_id (the script dedups by feature, see `scripts/frontier_sync.py:210` comment).
- **Phase G compliance**: now restored. CYCLE-150 / 151 / 152 / 154 had all bypassed Phase G (skipped `frontier_sync.py`); this cycle catches up.
- **Lessons**:
  1. **Phase G is automatic but easy to skip in cycle protocol.** Should bake into orchestrator's per-cycle checklist: "if pareto-frontier.md touched in this cycle, source secrets + run frontier_sync.py before close." Filed as a process improvement note in style-rules followups.
...
## Cycle 160 (2026-04-30) — Style-rules §J.6 + audit at-risk kernels (PASS — preventive cycle)
- **Feature**: state/style-rules.md §J.6 + state/pareto-frontier.md dpt warning. Captures the lesson from CYCLE-159 NEGATIVE result and proactively audits the rest of the codebase for the same dense-n×n + Ssyevd pattern.
- **Outcome**: PASS. Pure-Opus cycle (no worker dispatch). Three artifacts:
  1. **§J.6 added to style-rules**: rule that any kernel with O(n²) memory / O(n³) compute (dense n×n adjacency, full Sgemm/Ssyevd over a graph) requires a ≥10k-cell scale-smoke test before claiming `frontier`. Small-n correctness is necessary but not sufficient.
  2. **Audit results**: grepped all `include/singlet-gpu/{embed,preprocess,integrate}/*.h` for dense-n×n / Ssyevd patterns. Found **`embed/dpt.h`** uses the SAME pattern at `dpt.h:151/179/215/270/302/453` (dense W + symmetric normalization + cusolverDnSsyevd). Highly likely to exhibit identical scaling failure.
  3. **At-risk flag in pareto-frontier.md**: dpt entry now has ⚠️ "AT-RISK FOR SAME SCALING GAP AS DIFFMAP" header, expected SCALING FAILURE noted in 100k/1M rows, recommended action: bench dpt early to confirm/reject. Pre-emptively listing dpt as a CYCLE-159.1-equivalent rewrite candidate (same sparse-eigensolver fix applies).
- **Other kernels audited (NOT at risk)**:
  - `embed/dendrogram.h`: cusolverDnSsyevd only on k×k cluster matrix (k = n_clusters, ≤100). Bound by clusters, not cells. OK.
  - `integrate/{combat, lisi, asw, kbet}`, `qc/{empty_drops, soupx}`, `anno/{celltypist, symphony}`, `enrich/decoupler_*`: all O(n_cells) or O(n_cells × small) by design (per-cell histogram, kNN-aware metrics, sparse SpMM). OK.
- **Lessons**:
  1. **Pattern audits are cheap and high-leverage.** A 5-minute grep across `include/` exposed the second instance of a known scaling bug pattern. Worth doing periodically (every ~10 cycles?) once we have well-characterized failure modes documented in §J.
  2. **Negative bench results compound.** CYCLE-159's NEGATIVE result on diffmap directly enabled this CYCLE-160 audit and §J.6 — without it, we would have promoted dpt at the same scale-blind level and discovered the issue only at user runtime. Fixing one negative finding earns a 10× return through pattern-matching to other kernels.
  3. **§J is becoming a living rule set.** Started with §J.1-§J.5 (4 rules from CYCLE-153/154/155); now §J.6 from CYCLE-159; expect more to land as cycles surface new procedural lessons. The pattern of `cycle-N close → distill into §J → audit existing code` is healthy.
...
- **Sonnet wrote**:
  ...
  - `python/src/_singlet_gpu_core.cpp` (UPDATED) — `#include "_bind_score_genes.hpp"` + `singlet::gpu::python::bind_score_genes(m)`. **Ungated** (no `SINGLET_GPU_BUILD_DEFERRED`) — score_genes.h has no factornet dependency.

...
- Reuses singlet's `snp_ad/adt/fragments` outputs.
...
- Consumes `spatial_coords.parquet` from singlet's Visium output.
...
- Requires singlet to add `mC_counts.1pz` and `ATAC_counts.1pz` as new modalities — this is a CROSS-AGENT request to the singlet-dev orchestrator. File a `CROSS-AGENT-METHYLATION-OUTPUT` task in `dag.md` when ready.
- Cannot be implemented until the singlet side ships.
...
- 3 "unique to us" features exploiting singlet-only outputs

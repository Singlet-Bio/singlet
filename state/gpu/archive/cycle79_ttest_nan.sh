#!/bin/bash
#SBATCH --job-name=sg_cycle79_ttest_nan
#SBATCH --partition=gpu
#SBATCH --time=00:45:00
#SBATCH --cpus-per-task=8
#SBATCH --mem=48G
#SBATCH --gres=gpu:1
#SBATCH --output=/mnt/home/debruinz/Singlet-AI/singlet-gpu/state/cycle79_ttest_nan_%j.log
...
BUILD_DIR=/mnt/home/debruinz/Singlet-AI/singlet-gpu/build_cycle79_verify
SRC_DIR=/mnt/home/debruinz/Singlet-AI/singlet-gpu

echo "=== Cycle 79: t-test NaN sanitizer (finitize_or_zero on t/p/lfc writes) ==="
...
# ── T-test TinyPlanted (primary target) ────────────────────────────────────────
echo ""
echo "--- ctest: Ttest.*TinyPlanted ---"
for REGEX in "Ttest.*TinyPlanted" "TTest.*TinyPlanted"; do
...
# ── T-test RealData (secondary target) ─────────────────────────────────────────
echo ""
...
grep -E "Jaccard|Spearman|rho=|PASSED|FAILED|intersection|cluster [0-9]|log2_fc|not finite" \
    /tmp/ttest_tiny_${SLURM_JOB_ID}.txt | head -60 || true

echo ""
echo "=== KEY METRICS: T-test RealData ==="
grep -E "Jaccard|Spearman|rho=|PASSED|FAILED|log2_fc|not finite|cluster [0-9]" \
    /tmp/ttest_real_${SLURM_JOB_ID}.txt | head -40 || true

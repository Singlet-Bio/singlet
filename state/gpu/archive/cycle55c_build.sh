...
#SBATCH --output=/mnt/home/debruinz/Singlet-AI/singlet-gpu/state/cycle55c_build_%j.log
...
BUILD_DIR=/dev/shm/singletgpu_build55c_${SLURM_JOB_ID}
mkdir -p ${BUILD_DIR}
...
# Clear cached refs from prior runs
rm -f /tmp/singlet_gpu_hvg_refs_tmp/hvg_ref_tiny.npz
rm -f /tmp/singlet_gpu_hvg_refs_tmp/hvg_ref_10k.npz

echo "=== HVG TESTS ONLY (tests 8-11) ==="
cd ${BUILD_DIR}
ctest --output-on-failure -R 'HvgTest' -V --timeout 600 2>&1
HVG_EXIT=$?
echo "HVG tests exit: ${HVG_EXIT}"

echo "=== FULL CTEST (all tests) ==="
ctest --output-on-failure -j4 --timeout 300 2>&1 | tee /tmp/ctest_full_55c_${SLURM_JOB_ID}.log
CTEST_EXIT=$?

echo "=== CTEST SUMMARY ==="
grep -E "tests passed|tests failed|FAILED:" /tmp/ctest_full_55c_${SLURM_JOB_ID}.log | tail -10

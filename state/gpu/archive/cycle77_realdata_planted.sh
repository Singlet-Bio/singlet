#!/bin/bash
#SBATCH --job-name=sg_cycle77_realdata_planted
#SBATCH --partition=gpu
#SBATCH --time=01:00:00
#SBATCH --cpus-per-task=8
#SBATCH --mem=48G
#SBATCH --gres=gpu:1
#SBATCH --output=/mnt/home/debruinz/Singlet-AI/singlet-gpu/state/cycle77_realdata_planted_%j.log
...
BUILD_DIR=/mnt/home/debruinz/Singlet-AI/singlet-gpu/build_cycle77_verify
SRC_DIR=/mnt/home/debruinz/Singlet-AI/singlet-gpu

echo "=== Cycle 77 RealDataPlanted retest ==="
...
cmake --build "$BUILD_DIR" --target de_wilcoxon_correctness -j8 2>&1 | tail -40
BUILD_EXIT=$?
...
grep -E "Jaccard|Spearman|LFC|PvalRank|registry|RealDataPlanted|PASSED|FAILED" \
    /tmp/realplanted_${SLURM_JOB_ID}.txt | head -60 || true

echo ""
echo "=== KEY METRICS FROM TinyPlanted ==="
grep -E "Jaccard|Spearman|rho|PASSED|FAILED|PASS|FAIL" \
    /tmp/tinyplanted_${SLURM_JOB_ID}.txt | head -30 || true

echo ""
echo "=== TEST83 VERDICT ==="
grep -E "PASSED|FAILED|PASS|FAIL|NaN|CUDA error" \
    /tmp/test83_${SLURM_JOB_ID}.txt | head -10 || true

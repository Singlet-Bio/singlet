#!/bin/bash
#SBATCH --job-name=sg55c_diag
#SBATCH --partition=gpu
#SBATCH --gres=gpu:tesla_v100s:1
#SBATCH --cpus-per-task=16
#SBATCH --mem=64G
#SBATCH --time=01:30:00
#SBATCH --output=/mnt/home/debruinz/Singlet-AI/singlet-gpu/state/cycle55c_diag_%j.log
...
BUILD_DIR=/dev/shm/singletgpu_diag55c_${SLURM_JOB_ID}
mkdir -p ${BUILD_DIR}
...
echo "=== DIAGNOSTIC: SeuratV3 10k test only ==="
cd ${BUILD_DIR}
ctest --output-on-failure -R 'HvgTest.Gsm4037629_SeuratV3' -V --timeout 600 2>&1
HVG_EXIT=$?
echo "SeuratV3 diag test exit: ${HVG_EXIT}"

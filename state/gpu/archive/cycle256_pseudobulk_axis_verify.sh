...
# CYCLE-256: verify pseudobulk.py axis-swap fix (CYCLE-211 root cause).
#
# CYCLE-255 Sonnet diagnosis: _core.donor_pseudobulk_de expects (genes ×
# cells) CSC; the wrapper was passing AnnData's (cells × genes) shape so
# the kernel read csc_indptr OOB at element n_genes. Fix in
# python/singlet_gpu/de/pseudobulk.py: transpose mat before tocsc().
#
# Verify: rerun all test_de_pseudobulk.py. Expected:
#   - 3/4 PASS preserved (CYCLE-210)
#   - test_pseudobulk_de_min_cells_filter goes from xfail/CUDA-crash to PASS
#     OR a different, post-axis-fix failure mode that we then iterate on.
#   - File-level remains PASS.

#SBATCH --job-name=sg_c256_pseudobulk_verify
...
#SBATCH --output=/mnt/home/debruinz/Singlet-AI/singlet-gpu/state/cycle256_pseudobulk_verify_%j.log
...
echo "=== CYCLE-256: pseudobulk axis-swap verify ==="
...
echo ""
echo "--- pytest test_de_pseudobulk.py (full file) ---"
python -m pytest test_de_pseudobulk.py --runxfail -v --tb=short 2>&1 | tail -80

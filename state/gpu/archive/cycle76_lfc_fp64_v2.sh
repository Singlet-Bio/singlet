#!/bin/bash
#SBATCH --job-name=sg_cycle76_lfc_fp64_v2
#SBATCH --partition=gpu
#SBATCH --time=00:45:00
#SBATCH --cpus-per-task=8
#SBATCH --mem=48G
#SBATCH --gres=gpu:1
#SBATCH --output=/mnt/home/debruinz/Singlet-AI/singlet-gpu/state/cycle76_lfc_fp64_v2_%j.log
...
BUILD_DIR=/mnt/home/debruinz/Singlet-AI/singlet-gpu/build_cycle76_v2
SRC_DIR=/mnt/home/debruinz/Singlet-AI/singlet-gpu

echo "=== Cycle 76 v2: fp64 LFC + gene-aligned Spearman test ==="
...
ctest --test-dir "$BUILD_DIR" -R "Wilcoxon.*TinyPlanted" --output-on-failure -V 2>&1 | tee /tmp/tinyplanted_v2_$SLURM_JOB_ID.txt
TINY_EXIT=$?
echo "TINY_EXIT=$TINY_EXIT"

echo "--- ctest: Test83_RealDataSized_PostNormalize_NoCrash (no-regression) ---"
ctest --test-dir "$BUILD_DIR" -R "Test83_RealDataSized_PostNormalize_NoCrash" --output-on-failure -V 2>&1 | tee /tmp/test83_v2_$SLURM_JOB_ID.txt
TEST83_EXIT=$?
echo "TEST83_EXIT=$TEST83_EXIT"

echo ""
echo "=== KEY METRIC LINES FROM TINYPLANTED ==="
grep -E "Jaccard|Spearman|rho|LFC|intersection|PASSED|FAILED|PASS|FAIL" /tmp/tinyplanted_v2_$SLURM_JOB_ID.txt | head -60 || true
echo ""
echo "=== TEST83 VERDICT ==="
grep -E "PASSED|FAILED|PASS|FAIL|NaN|CUDA error" /tmp/test83_v2_$SLURM_JOB_ID.txt | head -10 || true

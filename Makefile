...
.PHONY: test test-cpp test-python lint format typecheck coverage build pipeline clean check help
...
test-python: ## Run 599 Python tests
...
check: lint typecheck test ## Run lint + typecheck + all tests

...
- **Python tests**: 1002 total (up from 205 at cycle 184, 98% core coverage)
- **Public API**: 80 functions exported
- **Preprocessing API**: `describe()`, `filter_cells()`, `filter_genes()`, `normalize()`, `highly_variable_genes()`, `highly_variable_genes_seurat_v3()`, `scale()`, `subsample()`, `concatenate()`, `calculate_qc_metrics()`, `regress_out()` — standard scRNA-seq workflow without scanpy
- **Analysis pipeline**: `pca()`, `neighbors()`, `leiden()`, `louvain()`, `umap()`, `tsne()`, `diffmap()`, `dpt()`, `harmony()`, `combat()`, `paga()`, `ingest()` — full dimensionality reduction + clustering + batch correction + trajectory + label transfer without scanpy
...
- **Visualization**: `plot_umap()`, `plot_violin()`, `plot_dotplot()`, `plot_scatter()`, `plot_heatmap()`, `plot_stacked_violin()`, `plot_paga()`, `rank_genes_groups_dotplot()` — publication-ready matplotlib plots

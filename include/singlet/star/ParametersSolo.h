...
public:
    ...
    // Hash table for fast barcode lookup (built from cbWL after loading)
    vector<int32> cbHashTable;  // open-addressing hash table: value = index into cbWL, -1 = empty
    uint32 cbHashMask;          // table size - 1 (power of 2)
    void buildCBhashTable();    // build hash table from cbWL
    int64 hashLookupCB(uint64 cb) const; // lookup barcode in hash table, returns index or -1

...
//   chromvar_cpp — wraps singlet::gpu::atac::chromvar
//
...
// ---------------------------------------------------------------------------
inline singlet::gpu::core::DeviceCSC
upload_host_csr_as_device_csc(const singlet::gpu::grn::HostCSR& csr, cudaStream_t stream)
{
    ...
    d.col_ptr     = singlet::gpu::core::DeviceMemory<int32_t>(indptr_sz);
    d.row_indices = singlet::gpu::core::DeviceMemory<int32_t>(nnz_sz);
    d.values      = singlet::gpu::core::DeviceMemory<float>  (nnz_sz);
        ...
        std::string           resource)
{
    ...
    // ── Upload matrices ───────────────────────────────────────────────────────
    singlet::gpu::core::DeviceCSC d_access;
    ...
    singlet::gpu::core::DeviceCSC d_motif;
    ...
    singlet::gpu::core::DeviceMemory<float> d_gc  (static_cast<std::size_t>(n_peaks));
    singlet::gpu::core::DeviceMemory<float> d_mean_acc(static_cast<std::size_t>(n_peaks));
    ...
    // ── Run kernel ─────────────────────────────────────────────────────────────
    singlet::gpu::atac::ChromVarResult res;
    try {
        res = singlet::gpu::atac::chromvar(d_access, d_motif, d_gc, d_mean_acc, cfg, stream);
    } catch (const std::exception& e) {

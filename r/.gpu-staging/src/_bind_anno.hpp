...
//   marker_score_cpp        — wraps singlet::gpu::anno::marker_score
//   celltypist_project_cpp  — wraps singlet::gpu::anno::project_to_reference
//   load_celltypist_model_cpp — wraps singlet::gpu::anno::load_celltypist_model
...
// ---------------------------------------------------------------------------
inline singlet::gpu::anno::GeneSetDB
...
struct CelltypistModelHandle {
    singlet::gpu::anno::CelltypistModel model;
    explicit CelltypistModelHandle(singlet::gpu::anno::CelltypistModel m)
        : model(std::move(m)) {}
};
...
inline SEXP load_celltypist_model_cpp(std::string npz_path)
{
    singlet::gpu::anno::CelltypistModel model;
    try {
        model = singlet::gpu::anno::load_celltypist_model(npz_path);
        ...
        std::string        method)
{
    ...
    if      (method == "mlm")   cfg.method = singlet::gpu::anno::MarkerMethod::Mlm;
    else if (method == "ulm")   cfg.method = singlet::gpu::anno::MarkerMethod::Ulm;
    else if (method == "wsum")  cfg.method = singlet::gpu::anno::MarkerMethod::Wsum;
    else if (method == "ucell") cfg.method = singlet::gpu::anno::MarkerMethod::UCell;
    else Rcpp::stop("marker_score_cpp: unknown method '%s'. Use 'mlm', 'ulm', 'wsum', or 'ucell'.",
                    method.c_str());

    // Run kernel
    singlet::gpu::anno::MarkerScoreResult res;
    try {
        res = singlet::gpu::anno::marker_score(d, db, cfg, stream);
    } catch (const std::exception& e) {
        ...
        SEXP                model_ptr)
{
    ...
    // Run projection
    singlet::gpu::anno::RefMapResult res;
    try {
        res = singlet::gpu::anno::project_to_reference(d_emb, h->model, stream);
    } catch (const std::exception& e) {

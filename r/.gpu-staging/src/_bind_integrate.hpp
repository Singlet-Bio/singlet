...
//   harmony_cpp  — wraps singlet::gpu::integrate::harmony_gpu
//   bbknn_cpp    — wraps singlet::gpu::integrate::bbknn_gpu
//
// Input format:
//   harmony_cpp:
                 ...
                 cudaStream_t         stream)
{
    ...
    singlet::gpu::core::DeviceMemory<int> d_ids(n_cells);
        ...
        int                 seed)
{
    ...
    // Upload batch IDs
    int n_batches = 0;
    singlet::gpu::core::DeviceMemory<int> d_batch;
    ...
    // Run Harmony kernel
    singlet::gpu::integrate::HarmonyResult res;
    try {
        res = singlet::gpu::integrate::harmony_gpu(d_emb, d_batch, n_batches, cfg, stream);
        ...
        std::string         metric)
{
    ...
    if      (metric == "euclidean") cfg.metric = singlet::gpu::integrate::BBKNNMetric::Euclidean;
    else if (metric == "cosine")    cfg.metric = singlet::gpu::integrate::BBKNNMetric::Cosine;
    else Rcpp::stop("bbknn_cpp: unknown metric '%s'. Use 'euclidean' or 'cosine'.", metric.c_str());

    singlet::gpu::integrate::BBKNNResult res;
    try {
        res = singlet::gpu::integrate::bbknn_gpu(d_emb, d_batch, n_batches, cfg, stream);
    } catch (const std::exception& e) {

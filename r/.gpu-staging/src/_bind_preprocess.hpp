...
//   lognorm_cpp  — wraps singlet::gpu::preprocess::log_normalize
//   hvg_cpp      — wraps singlet::gpu::preprocess::select_hvg
//
...
// ---------------------------------------------------------------------------
inline singlet::gpu::core::DeviceCSC upload_host_csc_to_device(
        const HostCsc& h,
        cudaStream_t   stream)
{
    using singlet::gpu::core::DeviceMemory;
...
inline Rcpp::S4 lognorm_cpp(Rcpp::S4 dgc, double target_sum) {
    ...
    singlet::gpu::preprocess::LogNormResult res;
    try {
        res = singlet::gpu::preprocess::log_normalize(d, cfg, stream);
...
inline Rcpp::List hvg_cpp(Rcpp::S4 dgc, int n_top, std::string flavor) {
    ...
    try {
        res = singlet::gpu::preprocess::select_hvg(d, cfg, stream);
    } catch (const std::exception& e) {

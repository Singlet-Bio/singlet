...
//   pca_cpp  — wraps singlet::gpu::reduce::svd::auto_select
//   nmf_cpp  — wraps singlet::gpu::reduce::nmf::fit
//
...
inline Rcpp::List pca_cpp(Rcpp::S4 dgc, int n_pcs, bool zero_center, int seed) {
    ...
    singlet::gpu::io::PzDeviceMatrix pzdm;
    pzdm.mat             = std::move(d);
    ...
    singlet::gpu::reduce::svd::SvdResult res;
    try {
        res = singlet::gpu::reduce::svd::auto_select(pzdm, n_pcs, cfg);
...
inline Rcpp::List nmf_cpp(Rcpp::S4 dgc, int n_factors, std::string loss, int seed) {
    ...
    // internally.  We use singlet::gpu::core::PinnedPool to get page-locked
    ...
    singlet::gpu::core::PinnedBuffer pb_indptr  = singlet::gpu::core::PinnedPool::acquire(indptr_bytes);
    singlet::gpu::core::PinnedBuffer pb_indices = singlet::gpu::core::PinnedPool::acquire(indices_bytes);
    singlet::gpu::core::PinnedBuffer pb_values  = singlet::gpu::core::PinnedPool::acquire(values_bytes);
    ...
    if (loss == "MSE" || loss == "mse") {
        ...
        cfg.loss.type = singlet::gpu::reduce::nmf::LossType::NB;
    } else {
        Rcpp::stop("nmf_cpp: unknown loss '%s'. Use 'MSE', 'KL', or 'NB'.",
                   loss.c_str());
    }

    singlet::gpu::reduce::nmf::NmfResult res;
    try {
        res = singlet::gpu::reduce::nmf::fit(pzdm, cfg,
                                            /*W_init=*/nullptr,
                                            /*H_init=*/nullptr);
    } catch (const std::exception& e) {

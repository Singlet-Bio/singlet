...
//   umap_cpp  — wraps singlet::gpu::embed::umap
//
        ...
        int                 seed)
{
    ...
    knn.n = n;
    knn.k = k;
    knn.backend_used = singlet::gpu::graph::KnnBackend::Exact;
    ...
    if (init == "random")   cfg.init = singlet::gpu::embed::UmapInit::Random;
    else if (init == "spectral") cfg.init = singlet::gpu::embed::UmapInit::Spectral;
    else Rcpp::stop("umap_cpp: unknown init '%s'. Use 'random' or 'spectral'.",
                    init.c_str());

    // Run UMAP
    singlet::gpu::embed::UmapResult res;
    try {
        res = singlet::gpu::embed::umap(knn, cfg, stream);
    } catch (const std::exception& e) {

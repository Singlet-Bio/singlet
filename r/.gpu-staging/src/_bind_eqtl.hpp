...
//   nebula_cpp — wraps singlet::gpu::eqtl::run_nebula
//
...
//   (snp_ad/snp_dp CSC).  All three come from singlet outputs.  Uploading them
        ...
        std::string         resource)
{
    ...
    if (n_donors > singlet::gpu::eqtl::MAX_DONORS_SHMEM)
        Rcpp::stop("nebula_cpp: n_donors (%d) exceeds MAX_DONORS_SHMEM (%d). "
                   "Downsample donors.", n_donors, singlet::gpu::eqtl::MAX_DONORS_SHMEM);
    ...
    if (!Rf_isNull(covariates)) {
        ...
        n_cov = cov_mat.ncol();
        if (n_cov > singlet::gpu::eqtl::MAX_COV)
            Rcpp::stop("nebula_cpp: n_cov (%d) exceeds MAX_COV (%d).",
                       n_cov, singlet::gpu::eqtl::MAX_COV);
    ...
    singlet::gpu::core::DeviceCSC d_counts, d_snp_ad, d_snp_dp;
    ...
    cfg.chunk_snps  = chunk_snps  > 0 ? chunk_snps  : singlet::gpu::eqtl::DEFAULT_CHUNK_SNPS;
    cfg.chunk_genes = chunk_genes > 0 ? chunk_genes : singlet::gpu::eqtl::DEFAULT_CHUNK_GENES;
    cfg.run_bh_fdr  = run_bh_fdr;
    cfg.fdr_alpha   = static_cast<float>(fdr_alpha);

    // ── Run kernel ─────────────────────────────────────────────────────────────
    singlet::gpu::eqtl::NebulaResult res;
    try {
        res = singlet::gpu::eqtl::run_nebula(

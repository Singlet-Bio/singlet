...
//   discrete_diffusion_train_cpp  — wraps singlet::gpu::generative::discrete_diffusion_train
//   discrete_diffusion_sample_cpp — wraps singlet::gpu::generative::discrete_diffusion_sample
//
// Discrete Diffusion:
...
// ---------------------------------------------------------------------------
// [[Rcpp::export]]
inline Rcpp::XPtr<singlet::gpu::generative::DiscreteDiffusionModel>
discrete_diffusion_train_cpp(
        Rcpp::S4    dgc,
        int         n_latent,
        int         n_steps,
        ...
        int         seed)
{
    ...
    auto* model = new singlet::gpu::generative::DiscreteDiffusionModel();
    try {
        *model = singlet::gpu::generative::discrete_diffusion_train(d, n_genes, n_cells, cfg, stream);
    ...
    return Rcpp::XPtr<singlet::gpu::generative::DiscreteDiffusionModel>(model, true);
}
...
inline Rcpp::List discrete_diffusion_sample_cpp(
        ...
        int  seed)
{
    Rcpp::XPtr<singlet::gpu::generative::DiscreteDiffusionModel> model(model_ptr);
    if (!model)
    ...
    singlet::gpu::generative::DiscreteDiffusionSampleConfig scfg;
    scfg.n_samples = n;
    scfg.seed      = static_cast<uint64_t>(seed);

    singlet::gpu::generative::DiscreteDiffusionSampleResult res;
    try {
        res = singlet::gpu::generative::discrete_diffusion_sample(*model, scfg, stream);
    } catch (const std::exception& e) {

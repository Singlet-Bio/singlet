...
public:
    ...
    CSCMatrix to_csc() const {
        ...
        std::vector<int32_t> col_count(n_barcodes_, 0);
        for (size_t i = 0; i < n; ++i) {
            col_count[cols_[i]]++;
        }

        // Build column start offsets (prefix sum)
        std::vector<int32_t> col_start(n_barcodes_ + 1, 0);
        ...
        std::vector<int32_t> col_pos(col_start.begin(), col_start.begin() + n_barcodes_);
        std::vector<uint32_t> grouped_rows(n);
        std::vector<ValT> grouped_vals(n);
        for (size_t i = 0; i < n; ++i) {
            int32_t slot = col_pos[cols_[i]]++;
        ...
        for (uint32_t c = 0; c < n_barcodes_; ++c) {
            int32_t start = col_start[c];
            int32_t end = col_start[c + 1];
            if (start == end) {

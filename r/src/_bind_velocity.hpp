...
//   velocity_moments_cpp       — wraps singlet::gpu::velocity::compute_moments
//   velocity_prep_compute_cpp  — wraps singlet::gpu::velocity::velocity_prep_compute
//
// Input format:
...
//
// Design note: spliced/unspliced come from singlet's exon_counts.1pz /
// intron_counts.1pz respectively.  The R wrapper (velocity.R) provides a
// read_pz_velocity_sce() helper that loads these assays from a singlet
// output directory.
...
dense_device_to_r_matrix(const singlet::gpu::core::DeviceMemory<float>& d_buf,
        ...
        int                 n_neighbors)
{
    ...
    singlet::gpu::core::DeviceMemory<int> d_knn(nk);
    {
    ...
    // Run moments kernel
    singlet::gpu::velocity::MomentsConfig cfg;
    cfg.n_neighbors = n_neighbors;

    singlet::gpu::velocity::MomentsResult res;
    try {
        res = singlet::gpu::velocity::compute_moments(d_s, d_u, d_knn, n_cells, cfg, stream);
        ...
        bool                compute_velocity)
{
    ...
    singlet::gpu::core::DeviceDense d_Ms, d_Mu;
    ...
    singlet::gpu::velocity::VelocityPrepConfig cfg;
    ...
    singlet::gpu::velocity::VelocityPrepResult res;
    try {
        res = singlet::gpu::velocity::velocity_prep_compute(d_Ms, d_Mu, n_genes, n_cells, cfg, stream);
    } catch (const std::exception& e) {

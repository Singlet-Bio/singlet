...
//   neighbors_cpp  — wraps singlet::gpu::graph::compute_knn
//   leiden_cpp     — wraps singlet::gpu::graph::leiden
//
...
// ---------------------------------------------------------------------------
inline singlet::gpu::core::DeviceDense
                          ...
                          cudaStream_t stream)
{
    ...
    singlet::gpu::core::DeviceDense dense(n_rows, n_cols);
        ...
        int                 seed)
{
    ...
    singlet::gpu::core::DeviceDense d_emb;
    ...
    if      (metric == "euclidean") cfg.metric = singlet::gpu::graph::DistanceMetric::L2;
    else if (metric == "cosine")    cfg.metric = singlet::gpu::graph::DistanceMetric::Cosine;
    else if (metric == "inner")     cfg.metric = singlet::gpu::graph::DistanceMetric::Inner;
    else Rcpp::stop("neighbors_cpp: unknown metric '%s'. Use 'euclidean', 'cosine', or 'inner'.",
                    metric.c_str());

    // Run kNN
    singlet::gpu::graph::KnnResult knn;
    try {
        knn = singlet::gpu::graph::compute_knn(d_emb, cfg, stream);
        ...
        std::string         weight_function)
{
    ...
    knn.n = n;
    knn.k = k;
    knn.backend_used = singlet::gpu::graph::KnnBackend::Exact;  // provenance only

    knn.row_offsets = singlet::gpu::core::DeviceMemory<int>(n + 1);
    knn.neighbors   = singlet::gpu::core::DeviceMemory<int>(nk);
    knn.distances   = singlet::gpu::core::DeviceMemory<float>(nk);
    ...
    if      (weight_function == "connectivity") cfg.weight_function = singlet::gpu::graph::LeidenWeight::Connectivity;
    else if (weight_function == "inverse")      cfg.weight_function = singlet::gpu::graph::LeidenWeight::Inverse;
    else if (weight_function == "gaussian")     cfg.weight_function = singlet::gpu::graph::LeidenWeight::Gaussian;
    else Rcpp::stop("leiden_cpp: unknown weight_function '%s'. Use 'connectivity', 'inverse', or 'gaussian'.",
                    weight_function.c_str());

    // Run Leiden
    singlet::gpu::graph::LeidenResult res;
    try {
        res = singlet::gpu::graph::leiden(knn, cfg, stream);
    } catch (const std::exception& e) {

...
//   mt_lineage_call_clones_cpp — wraps singlet::gpu::anno::mt_lineage_call_clones
//
// Input format:
...
//
// Design note: alt and depth come from singlet's mt_alleles.1pz (which stores
// alt allele counts) and an associated depth matrix.  The R wrapper (lineage.R)
// provides read_pz_mt_sce() to load these into an SCE from a singlet output
        ...
        int      seed)
{
    ...
    singlet::gpu::core::DeviceCSC d_alt, d_dep;
    ...
    singlet::gpu::anno::MtLineageResult res;
    try {
        res = singlet::gpu::anno::mt_lineage_call_clones(d_alt, d_dep, n_sites, n_cells, cfg, stream);

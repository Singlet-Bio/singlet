...
//
//   hdr.confidence <= lib1fq::Confidence::LOW (=1)
...
static bool should_reject_r2_short(lib1fq::Confidence conf, int r2_len, bool is_atac) {
    if (is_atac) return false;  // ATAC short-R2 is expected
    return conf <= lib1fq::Confidence::LOW
        && r2_len > 0
        && r2_len < 50;
}
...
static void test_confidence_enum_values() {
    ...
    CHECK(static_cast<uint8_t>(lib1fq::Confidence::NONE)   == 0, "NONE == 0");
    CHECK(static_cast<uint8_t>(lib1fq::Confidence::LOW)    == 1, "LOW == 1");
    CHECK(static_cast<uint8_t>(lib1fq::Confidence::MEDIUM) == 2, "MEDIUM == 2");
    CHECK(static_cast<uint8_t>(lib1fq::Confidence::HIGH)   == 3, "HIGH == 3");
}
...
static void test_reject_low_conf_short_r2() {
    ...
    CHECK(should_reject_r2_short(lib1fq::Confidence::LOW, 30, false),
          "LOW conf + 30bp R2 → reject");
    CHECK(should_reject_r2_short(lib1fq::Confidence::LOW, 49, false),
          "LOW conf + 49bp R2 → reject (boundary)");
    CHECK(should_reject_r2_short(lib1fq::Confidence::NONE, 30, false),
...
static void test_pass_cases() {
    ...
    CHECK(!should_reject_r2_short(lib1fq::Confidence::LOW, 50, false),
          "LOW conf + 50bp R2 → allow (at threshold)");
    CHECK(!should_reject_r2_short(lib1fq::Confidence::LOW, 91, false),
          "LOW conf + 91bp R2 → allow");

    // High / medium confidence → always allow regardless of R2 length
    CHECK(!should_reject_r2_short(lib1fq::Confidence::MEDIUM, 30, false),
          "MEDIUM conf + 30bp R2 → allow");
    CHECK(!should_reject_r2_short(lib1fq::Confidence::HIGH, 30, false),
          "HIGH conf + 30bp R2 → allow");
    CHECK(!should_reject_r2_short(lib1fq::Confidence::MANUAL, 30, false),
          "MANUAL conf + 30bp R2 → allow");

    // r2_len == 0 (unknown) → allow (no length info, cannot decide)
    CHECK(!should_reject_r2_short(lib1fq::Confidence::LOW, 0, false),
          "LOW conf + r2_len=0 → allow (unknown R2 len)");

    // ATAC mode → always allow (ATAC R2 barcode is legitimately short)
    CHECK(!should_reject_r2_short(lib1fq::Confidence::LOW, 16, true),
          "LOW conf + 16bp R2 + is_atac → allow");
    CHECK(!should_reject_r2_short(lib1fq::Confidence::LOW, 30, true),
...
static void test_boundary_exact_50() {
    ...
    CHECK(!should_reject_r2_short(lib1fq::Confidence::LOW, 50, false),
          "r2_len=50 == 50 → pass");
    CHECK(!should_reject_r2_short(lib1fq::Confidence::LOW, 51, false),
          "r2_len=51 > 50 → pass");
}

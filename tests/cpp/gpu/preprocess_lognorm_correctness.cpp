...
//
// Correctness harness for singlet::gpu::preprocess::log_normalize.
//
...
TEST_F(LognormTest, TinySynthetic_TotalCount) {
    ...
    singlet::gpu::preprocess::LogNormResult res =
        singlet::gpu::preprocess::log_normalize(d_mat, cfg, stream_);
    cudaStreamSynchronize(stream_);
...
TEST_F(LognormTest, EdgeCase_HugeCount_KahanNoOverflow) {
    ...
    ASSERT_NO_THROW(singlet::gpu::preprocess::log_normalize(d_mat, cfg, stream_));
    cudaStreamSynchronize(stream_);
...
TEST_F(LognormTest, DeferredModes_NotImplemented) {
    ...
    {
        singlet::gpu::preprocess::LogNormConfig cfg{};
        cfg.method = singlet::gpu::preprocess::LogNormMethod::ScranDeconvolution;
    ...
    {
        singlet::gpu::preprocess::LogNormConfig cfg{};
        cfg.method = singlet::gpu::preprocess::LogNormMethod::Downsample;
        EXPECT_THROW(
            singlet::gpu::preprocess::log_normalize(d_mat, cfg, stream_),
            std::exception);
    }
...
TEST_F(LognormTest, Gsm4037629_TotalCount) {
    ...
    // T and all s_j values are validated by checking the 500-column subset.
    {
        ...
        singlet::gpu::preprocess::LogNormResult res =
            singlet::gpu::preprocess::log_normalize(pz.mat, cfg, stream_);
        cudaStreamSynchronize(stream_);

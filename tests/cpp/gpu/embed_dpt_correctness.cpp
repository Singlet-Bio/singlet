...
TEST(DptTest, Dpt_LinearTrajectory_MonotonicPseudotime)
{
    ...
    singlet::gpu::embed::DptConfig cfg;
    cfg.root_cell   = ROOT;
    cfg.n_eigenvecs = 10;

    auto res = singlet::gpu::embed::dpt(knn, cfg);
...
TEST(DptTest, Dpt_MemoryGuard_RejectsTooLarge)
{
    ...
    singlet::gpu::graph::KnnResult fake_knn;
    fake_knn.n = N_LARGE;
    fake_knn.k = 1;
    fake_knn.backend_used = singlet::gpu::graph::KnnBackend::Exact;

    // Tiny buffers — guard fires before any kernel touches them.
    fake_knn.row_offsets = singlet::gpu::core::DeviceMemory<int>(2);
    fake_knn.neighbors   = singlet::gpu::core::DeviceMemory<int>(1);
    fake_knn.distances   = singlet::gpu::core::DeviceMemory<float>(1);
    ...
    std::string err_msg;
    try {
        auto res = singlet::gpu::embed::dpt(fake_knn, cfg);

...
//
// Correctness harness for singlet::gpu::anno::call_clones (anno/mt_lineage.h).
//
...
std::vector<int> download_int(
        const singlet::gpu::core::DeviceMemory<int>& d,
...
TEST(MtLineageCorrectness, MtLineage_TinySynthetic_VsMQuad) {
    ...
    auto pred = singlet::gpu::anno::call_clones(d_alt, d_depth, cfg, stream);
    cudaStreamSynchronize(stream);
...
TEST(MtLineageCorrectness, MtLineage_GSM4037629_RealData) {
    ...
    singlet::gpu::core::DeviceCSC d_alt =
        singlet::gpu::io::load_pz(kGsmMtPath, stream).mat;
    ...
    // This is the safest fallback when mt_depth.1pz is not yet written by singlet.
    singlet::gpu::core::DeviceCSC d_depth;
    static const char* kGsmDepthPath =
        "/mnt/projects/debruinz_project/singlet_pipeline/"
        "quant/scrna/GSE127/GSE127918/GSM4037629/mt_depth.1pz";
    if (fs::exists(kGsmDepthPath)) {
        d_depth = singlet::gpu::io::load_pz(kGsmDepthPath, stream).mat;
    } else {
        ...
        d_depth = singlet::gpu::io::load_pz(kGsmMtPath, stream).mat;
    }
...
TEST(MtLineageCorrectness, MtLineage_Determinism_BitIdentical) {
    ...
    // Run 1.
    auto pred1 = singlet::gpu::anno::call_clones(d_alt, d_depth, cfg, stream);
    cudaStreamSynchronize(stream);
    auto labels1 = download_int(pred1.labels, static_cast<size_t>(kTinyCells), stream);

    // Run 2 — same inputs, same config, same seed.
    auto pred2 = singlet::gpu::anno::call_clones(d_alt, d_depth, cfg, stream);
    cudaStreamSynchronize(stream);

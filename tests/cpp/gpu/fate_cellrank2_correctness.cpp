...
//
// Correctness harness for singlet::gpu::fate::cellrank2.
//
...
// DeviceCSR compat: the test uses singlet::gpu::fate::DeviceCSR; we alias it to
// singlet::gpu::core::DeviceCSC (the real factornet type is the same regardless
// of whether the test treats it as CSR or CSC).
namespace singlet::gpu { namespace fate {
    using DeviceCSR = singlet::gpu::core::DeviceCSC;
} }
...
// Test uses: auto ctx = singlet::gpu::core::GPUContext::create(stream);
...
namespace singlet::gpu { namespace graph {
...
// The test always declares cfg as singlet::gpu::fate::CellRank2Config.
...
singlet::gpu::fate::DeviceCSR upload_csr(const HostCSR& h, cudaStream_t stream) {
...
TEST(Cellrank2, TinySynthetic_VsPython) {
    ...
    singlet::gpu::fate::DeviceCSR d_T = upload_csr(syn.T, stream);
    singlet::gpu::fate::CellRank2Config cfg;
    ...
    singlet::gpu::fate::CellRank2Result res =
        singlet::gpu::fate::run_absorption(ctx, d_T, cfg);
...
TEST(Cellrank2, GSM_RealData) {
    ...
    knn_cfg.k = 50;
    auto d_knn = singlet::gpu::graph::compute_exact(ctx, d_csc, knn_cfg);

    // Convert kNN to row-stochastic transition matrix (uniform weights).
    auto d_T = singlet::gpu::fate::knn_to_transition(ctx, d_knn);

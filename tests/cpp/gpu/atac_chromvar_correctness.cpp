...
//
// Correctness harness for singlet::gpu::atac::chromvar.
//
...
// actual singlet::gpu::atac API in chromvar.h.  ALL bridges at global scope.
// =============================================================================
...
// ---------------------------------------------------------------------------
singlet::gpu::atac::ChromVARConfig make_config(uint64_t seed = kSeed,
                                              int n_bg = kTinyNBg,
                                              int n_perm = kPermCount)
{
    singlet::gpu::atac::ChromVARConfig cfg;
...
protected:
    ...
    singlet::gpu::core::Handles handles_{};
    ...
    void SetUp() override {
        ...
        handles_ = singlet::gpu::core::Handles::create(stream_);
        ensure_refs_tmp();
    }

    void TearDown() override {
        if (stream_) {
            singlet::gpu::core::Handles::destroy(handles_);
...
TEST_F(ChromVARTest, ChromVAR_TinySynthetic_VsR) {
    ...
    DevCSC accessibility = singlet::gpu::io::host_dense_to_device_csc(
    ...
    singlet::gpu::atac::ChromVARResult result =
        singlet::gpu::atac::compute_chromvar(
            accessibility,
    ...
    singlet::gpu::atac::ChromVARResult::free(result, stream_);
}
...
TEST_F(ChromVARTest, ChromVAR_GSM_RealData_Atac) {
    ...
    singlet::gpu::core::DeviceCSC fragments =
        singlet::gpu::io::load_pz(kGsmFragmentsPath, stream_).mat;
    ...
    singlet::gpu::atac::ChromVARResult::free(result, stream_);
    singlet::gpu::core::device_csc_free_noop(fragments, stream_);
}
...
TEST_F(ChromVARTest, ChromVAR_VariabilityRanking) {
    ...
    singlet::gpu::atac::ChromVARResult::free(result, stream_);
    singlet::gpu::core::device_csc_free_noop(accessibility, stream_);
}
...
TEST_F(ChromVARTest, ChromVAR_Determinism_BitIdentical) {
    ...
    // Run 1
    DevCSC acc1 = singlet::gpu::io::host_dense_to_device_csc(
        counts_flat.data(), kTinyPeaks, kTinyCells, stream_);
    singlet::gpu::atac::ChromVARResult res1 =
        singlet::gpu::atac::compute_chromvar(
    ...
    cudaMemcpy(dev1.data(), res1.deviations_dev,
               dev_elems * sizeof(float), cudaMemcpyDeviceToHost);
    singlet::gpu::atac::ChromVARResult::free(res1, stream_);
    singlet::gpu::core::device_csc_free_noop(acc1, stream_);

    // Run 2 — same inputs, same seed
    DevCSC acc2 = singlet::gpu::io::host_dense_to_device_csc(
        counts_flat.data(), kTinyPeaks, kTinyCells, stream_);
    singlet::gpu::atac::ChromVARResult res2 =
        singlet::gpu::atac::compute_chromvar(
    ...
    cudaMemcpy(dev2.data(), res2.deviations_dev,
               dev_elems * sizeof(float), cudaMemcpyDeviceToHost);
    singlet::gpu::atac::ChromVARResult::free(res2, stream_);
    singlet::gpu::core::device_csc_free_noop(acc2, stream_);

...
//
// Correctness harness for singlet::gpu::eqtl::nebula_eqtl.
//
...
// Placed at global scope to avoid shadowing the real singlet::gpu namespace.
// =============================================================================

namespace singlet::gpu { namespace eqtl {
...
TEST(Nebula, GSM_RealMultisample) {
    ...
    auto d_counts = singlet::gpu::io::load_pz(kGsmCountsPath, stream).mat;
    auto d_snp_ad = singlet::gpu::io::load_pz(kGsmSnpAdPath,  stream).mat;
    auto d_snp_dp = singlet::gpu::io::load_pz(kGsmSnpDpPath,  stream).mat;
    ASSERT_EQ(cudaStreamSynchronize(stream), cudaSuccess);
    ...
    singlet::gpu::core::DeviceMemory<int32_t> d_donor_id(n_cells);
...
TEST(Nebula, Determinism_BitIdentical) {
    ...
    // Run 1.
    singlet::gpu::eqtl::NebulaResult r1 =
        singlet::gpu::eqtl::nebula_eqtl(
            d_counts, d_snp_ad, d_snp_dp,
            d_donor_id.get(), kTinyDonors,
            nullptr, 0,
    ...
    // Run 2.
    singlet::gpu::eqtl::NebulaResult r2 =
        singlet::gpu::eqtl::nebula_eqtl(
            d_counts, d_snp_ad, d_snp_dp,
            d_donor_id.get(), kTinyDonors,
            nullptr, 0,

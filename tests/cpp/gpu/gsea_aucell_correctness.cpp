...
//
// Correctness harness for singlet::gpu::gsea::aucell.
//
...
// ---------------------------------------------------------------------------
std::vector<float>
download_aucell_scores(const singlet::gpu::gsea::AUCellResult& res,
                       cudaStream_t stream)
{
...
// ---------------------------------------------------------------------------
singlet::gpu::anno::GeneSetDB make_planted_genesets(int G, int n_sets,
                                                    int genes_per_set)
{
...
TEST(AUCell, TinyPlanted_VsR) {
    ...
    auto our_res = singlet::gpu::gsea::aucell(d_mat, db, cfg, stream);
...
TEST(AUCell, Determinism_BitIdentical) {
    ...
    auto res1      = singlet::gpu::gsea::aucell(d_mat1, db, cfg, stream);
    auto scores1   = download_aucell_scores(res1, stream);

    // Run 2.
    auto d_mat2    = upload_csc(csc_h, stream);
    auto res2      = singlet::gpu::gsea::aucell(d_mat2, db, cfg, stream);
...
TEST(AUCell, LargeCellCount_TilesCorrectly) {
    ...
    singlet::gpu::core::DeviceMemory<float>   d_values(nnz_approx);
    singlet::gpu::core::DeviceMemory<int32_t> d_indptr(static_cast<size_t>(kLargeCells) + 1);
    singlet::gpu::core::DeviceMemory<int32_t> d_indices(nnz_approx);
    ...
    singlet::gpu::core::DeviceMemory<float>   dv(actual_nnz);
    singlet::gpu::core::DeviceMemory<int32_t> dp(static_cast<size_t>(kLargeCells) + 1);
    singlet::gpu::core::DeviceMemory<int32_t> di(actual_nnz);
    ...
    singlet::gpu::core::DeviceCSC d_mat;
    ...
    ASSERT_NO_THROW(res = singlet::gpu::gsea::aucell(d_mat, db, cfg, stream));
    ASSERT_EQ(cudaStreamSynchronize(stream), cudaSuccess);

...
//
// Correctness harness for singlet::gpu::reduce::svd adapters.
//
...
//   2. Call singlet::gpu::reduce::svd::{backend}(m, cfg)  → result_adapter.
...
TEST(SVD_RoundTrip, BackendName) {                                             \
    using namespace singlet::gpu::reduce::svd;                                  \

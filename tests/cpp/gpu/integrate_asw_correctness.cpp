...
TEST(AswTest, Asw_PerfectlySeparatedClusters_HighScore)
{
    ...
    singlet::gpu::integrate::AswConfig cfg;
    cfg.deterministic = true;
    auto res = singlet::gpu::integrate::asw(knn, d_lbl.get(), N_LABELS, cfg);
...
TEST(AswTest, Asw_Determinism_BitIdentical)
{
    ...
    auto res1 = singlet::gpu::integrate::asw(knn1, d_lbl.get(), N_LABELS, cfg);
    auto h1   = d2h_float(res1.silhouette, N);

    // Run 2 (re-upload).
    auto knn2 = build_knn_result(N, K, make_row_offsets(N, K), h_nbr, h_dist);
    auto res2 = singlet::gpu::integrate::asw(knn2, d_lbl.get(), N_LABELS, cfg);

...
# The script does NOT import singlet — it uses scanpy's built-in 10x reader
# path after decoding the .1pz via singlet.io (if available) or by reading
# the pre-written CSC binary produced by the C++ harness (fallback path).
#
# Fallback path (no singlet.io): the C++ harness writes a CSCC binary at
...
def parse_args():
    ...
    p.add_argument("--pz-path",   required=False, default="",
                   help="Path to exon_counts.1pz (used if singlet.io available)")
    p.add_argument("--csc-bin",   required=False, default="",
                   help="Pre-written CSCC binary from C++ (fallback if no singlet.io)")
...
def load_anndata(args):
    """
    Try to load via singlet.io; fall back to --csc-bin.
    Returns AnnData with raw integer counts in .X (CSC float32).
    """
    # Primary path: singlet.io Python wrapper
    if args.pz_path:
        try:
            import singlet.io as sio
            ...
            print(f"[e2e_ref] Loaded via singlet.io: {ad.shape}", flush=True)
            return ad
        except Exception as e:
            print(f"[e2e_ref] singlet.io unavailable ({e}); "
    ...
    raise RuntimeError("No data source available: supply --pz-path (singlet.io) "

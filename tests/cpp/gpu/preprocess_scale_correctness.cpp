...
//   singlet::gpu::preprocess::scale()
//   singlet::gpu::preprocess::regress_out()
//
...
static singlet::gpu::core::DeviceCSC upload_csc(const HostCSC& h, cudaStream_t stream)
{
    singlet::gpu::core::DeviceCSC d;
    d.rows = h.n_genes;
    d.cols = h.n_cells;
    d.nnz  = h.nnz();
    d.values      = singlet::gpu::core::DeviceMemory<float>(h.nnz());
    d.col_ptr     = singlet::gpu::core::DeviceMemory<int>(static_cast<size_t>(h.n_cells) + 1);
    d.row_indices = singlet::gpu::core::DeviceMemory<int>(h.nnz());
...
TEST_F(ScaleTest, TinySynthetic_ScaleCorrectness) {
    ...
    singlet::gpu::core::DeviceMemory<float> d_mean(N_GENES);
    singlet::gpu::core::DeviceMemory<float> d_std (N_GENES);
    ...
    auto d_out = singlet::gpu::preprocess::scale(d_mat, d_mean, d_std, cfg, stream_);
...
TEST_F(ScaleTest, EdgeCase_ZeroVarianceGene) {
    ...
    singlet::gpu::core::DeviceMemory<float> d_mean(1);
    singlet::gpu::core::DeviceMemory<float> d_std (1);
...
TEST_F(ScaleTest, RegressOut_ZeroCorrelation) {
    ...
    singlet::gpu::core::DeviceMemory<float> d_X(static_cast<size_t>(N_GENES) * N_CELLS);
    singlet::gpu::core::DeviceMemory<float> d_C(static_cast<size_t>(N_CELLS) * P);

...
TEST(PearsonResidualsTest, Tiny_Synthetic_VsClosedForm)
{
    ...
    singlet::gpu::preprocess::PearsonResidualsConfig cfg;
    cfg.theta = 100.f;
    auto d_var    = singlet::gpu::preprocess::pearson_residual_variance(pz, cfg);
...
TEST(PearsonResidualsTest, Determinism_BitIdentical_Same_Seed)
{
    ...
    auto d1  = singlet::gpu::preprocess::pearson_residual_variance(pz1, cfg);
    auto h1  = d2h(d1, M);

    auto pz2 = upload_csc(csc);
    auto d2  = singlet::gpu::preprocess::pearson_residual_variance(pz2, cfg);
    auto h2  = d2h(d2, M);
...
TEST(PearsonResidualsTest, AllZeros_Input_AllZerosOutput)
{
    ...
    singlet::gpu::io::PzDeviceMatrix pz;
    pz.mat = singlet::gpu::core::DeviceCSC(M, N, 0);

...
//
// Correctness harness for singlet::gpu::generative::discrete_diffusion_fit.
//
...
inline DiscreteDiffusionResultCompat discrete_diffusion_fit(
    ...
    int                                              n_samples = 50)
{
    auto base = singlet::gpu::generative::train_discrete_diffusion(counts, cfg, stream);
    cudaStreamSynchronize(stream);

    // Sample n_samples cells from the trained model.
    auto d_sampled = singlet::gpu::generative::sample_cells(base, n_samples, cfg, stream);
    cudaStreamSynchronize(stream);
...
namespace singlet::gpu { namespace generative {
...
} } // namespace singlet::gpu::generative
...
TEST(DiscreteDiffusion, Diffusion_TinySynthetic_TrainsAndSamples) {
    ...
    using namespace singlet::gpu::generative;
...
TEST(DiscreteDiffusion, Diffusion_GSM4037629_RealData_PerGeneMarginal) {
    ...
    DeviceCSC d_counts = singlet::gpu::io::load_pz(kGsmExonPath, stream).mat;
    cudaStreamSynchronize(stream);
...
TEST(DiscreteDiffusion, Diffusion_Determinism_BitIdentical) {
...

    {
        cudaStream_t stream;
        cudaStreamCreate(&stream);
        singlet::gpu::core::DeviceCSC d = upload_csc(h, stream);

...
TEST(MlmTest, Mlm_TinyClosedForm)
{
    ...
    singlet::gpu::enrich::MlmConfig cfg;
    cfg.ridge = 1e-6f;
    auto res  = singlet::gpu::enrich::mlm(pz, dW.get(), P, cfg);
...
TEST(MlmTest, Mlm_Determinism_BitIdentical)
{
    ...
    auto r1  = singlet::gpu::enrich::mlm(pz1, dW1.get(), P, cfg);
    auto h1  = d2h(r1.scores, N * P);

    auto pz2 = upload_csc(csc);
    auto dW2 = upload_W(W_h);
    auto r2  = singlet::gpu::enrich::mlm(pz2, dW2.get(), P, cfg);

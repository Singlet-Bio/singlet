...
std::vector<float> cpu_score_genes(
    ...
    const singlet::gpu::enrich::ScoreGenesConfig& cfg)
{
...
TEST(ScoreGenesTest, ScoreGenes_TinyClosedForm)
{
    ...
    auto res = singlet::gpu::enrich::score_genes(pz, gene_sets, cfg);
...
TEST(ScoreGenesTest, ScoreGenes_Determinism_SameSeed)
{
    ...
    singlet::gpu::enrich::ScoreGenesConfig cfg_a;
    cfg_a.ctrl_size = 5;
    cfg_a.n_bins    = 5;
    cfg_a.seed      = 7777;

    singlet::gpu::enrich::ScoreGenesConfig cfg_b = cfg_a;  // same seed
    singlet::gpu::enrich::ScoreGenesConfig cfg_c = cfg_a;
    cfg_c.seed = 8888;  // different seed

    // Run 1: seed=7777
    auto pz1  = upload_csc(csc);
    auto res1 = singlet::gpu::enrich::score_genes(pz1, gene_sets, cfg_a);
    auto h1   = d2h(res1.scores, N * 2);

    // Run 2: seed=7777 (same → should be bit-identical)
    auto pz2  = upload_csc(csc);
    auto res2 = singlet::gpu::enrich::score_genes(pz2, gene_sets, cfg_b);
    auto h2   = d2h(res2.scores, N * 2);

    // Run 3: seed=8888 (different → should differ in at least some elements)
    auto pz3  = upload_csc(csc);
    auto res3 = singlet::gpu::enrich::score_genes(pz3, gene_sets, cfg_c);

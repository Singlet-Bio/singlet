...
//
// Correctness harness for singlet::gpu::preprocess::velocity_prep.
//
...
std::vector<uint8_t> download_u8(
        const singlet::gpu::core::DeviceMemory<uint8_t>& d,
        size_t count, cudaStream_t stream)
{
...
TEST_F(VelocityPrepTest, VelocityPrep_TinyPlanted_VsScvelo) {
    ...
    singlet::gpu::core::DeviceCSC d_s = upload_csc(d.spliced,   stream);
    singlet::gpu::core::DeviceCSC d_u = upload_csc(d.unspliced, stream);
    ...
    auto result = singlet::gpu::preprocess::velocity_prep(d_s, d_u, nullptr, cfg, stream);
    cudaStreamSynchronize(stream);
...
// GSM4037629 — the two singlet outputs unique to this library.
...
TEST_F(VelocityPrepTest, VelocityPrep_GSM4037629_RealData) {
    ...
    auto d_spliced   = singlet::gpu::io::load_pz(kGsmSplicedPath,   stream).mat;
    auto d_unspliced = singlet::gpu::io::load_pz(kGsmUnsplicedPath, stream).mat;
    cudaStreamSynchronize(stream);
    ...
    auto result = singlet::gpu::preprocess::velocity_prep(
...
TEST_F(VelocityPrepTest, VelocityPrep_Determinism_BitIdentical) {
    ...
    // Run 1.
    {
        auto d_s = upload_csc(d.spliced,   stream);
        auto d_u = upload_csc(d.unspliced, stream);
        auto r1 = singlet::gpu::preprocess::velocity_prep(d_s, d_u, nullptr, cfg, stream);
        cudaStreamSynchronize(stream);
        ...
        auto r2 = singlet::gpu::preprocess::velocity_prep(d_s2, d_u2, nullptr, cfg, stream);
        cudaStreamSynchronize(stream);

...
//
// Correctness harness for singlet::gpu::fate::cospar.
//
...
// PzDeviceLoader compat (singlet::gpu::io).
...
TEST(CosparCorrectness, Cospar_TinySynthetic_VsPython) {
    ...
    singlet::gpu::core::DeviceCSC d_expr = upload_csc(syn.expression, stream);

    // Upload time_point and clone_id.
    singlet::gpu::core::DeviceMemory<int32_t> d_tp(kTinyCells);
    singlet::gpu::core::DeviceMemory<int32_t> d_cl(kTinyCells);
    ...
    // Run GPU kernel.
    using namespace singlet::gpu::fate;
    CosparConfig cfg;
    cfg.n_iter      = 50;
    cfg.n_knn       = 10;
    cfg.lambda1     = 0.05f;
...
TEST(CosparCorrectness, Cospar_PotencyMonotone) {
    ...
    singlet::gpu::core::DeviceCSC d_expr = upload_csc(tree.expression, stream);

    singlet::gpu::core::DeviceMemory<int32_t> d_tp(kTreeCells);
    singlet::gpu::core::DeviceMemory<int32_t> d_cl(kTreeCells);

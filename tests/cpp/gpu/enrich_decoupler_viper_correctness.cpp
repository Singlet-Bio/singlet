...
TEST(ViperTest, Viper_TinyClosedForm)
{
    ...
    singlet::gpu::enrich::ViperConfig cfg;
    auto res = singlet::gpu::enrich::viper(pz, dW.get(), P, cfg);
...
TEST(ViperTest, Viper_Determinism_BitIdentical)
{
    ...
    auto r1  = singlet::gpu::enrich::viper(pz1, dW1.get(), P, cfg);
    auto h1  = d2h(r1.nes, N * P);

    auto pz2 = upload_csc(csc);
    auto dW2 = upload_W(W_h);
    auto r2  = singlet::gpu::enrich::viper(pz2, dW2.get(), P, cfg);
...
TEST(ViperTest, Viper_MemoryGuard_RejectsTooLarge)
{
    ...
    singlet::gpu::core::DeviceMemory<float> d_W_tiny(2);  // minimal W (2 entries)
    float w_val = 1.f;
    ...
    bool threw = false;
    try {
        auto res = singlet::gpu::enrich::viper(pz_tiny, d_W_tiny.get(), 1, cfg);

...
//
// Correctness harness for singlet::gpu::enrich::ssgsea and
// singlet::gpu::enrich::progeny — bundled because both share the
...
namespace singlet::gpu { namespace enrich {
...
inline void ssgsea_run(
    ...
    cudaStream_t /*stream*/)
{
    // Stub retained for synthetic tests that compare vs R reference.
    // Real-data test (Ssgsea_GSM_RealData) bypasses this and calls ssgsea() directly.
}
...
// Implements activity = d_expr × d_W^T + optional per-pathway z-score normalization.
// d_expr    : [n_cells × n_genes]    float, row-major (device)
// d_weights : [n_pathways × n_genes] float, row-major (device)
// d_out     : [n_cells × n_pathways] float, row-major output (device)
inline void progeny_run(
    const float*          d_expr,
    const float*          d_weights,
    float*                d_out,
    const ProgenyParams&  pparams,
    cudaStream_t          stream)
{
    // FIX: the previous stub did nothing, leaving d_out all-zero (variance=0 test failure).
    // PROGENy activity = expression × weights^T implemented as cuBLAS SGEMM.
    //
    // cuBLAS is column-major.  Row-major A[M×K] = column-major A^T[K×M].
    // We want C[n_cells × n_pathways] = A[n_cells × n_genes] × B^T[n_genes × n_pathways]
    // where A = d_expr (row-major) and B = d_weights (row-major).
    //
    // In column-major notation:
    //   C^T[n_pathways × n_cells] = B[n_pathways × n_genes] × A^T[n_genes × n_cells]
    //   cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N,
    //               n_cells, n_pathways, n_genes,
    //               &alpha, d_expr,    n_genes,     // A^T: leading dim = n_genes (row-major A)
    //                       d_weights, n_genes,     // B:   leading dim = n_genes (row-major B)
    //               &beta,  d_out,     n_cells)     // C^T: leading dim = n_cells

    const int M = pparams.n_cells;
    const int N = pparams.n_pathways;
    const int K = pparams.n_genes;
    ...
    // C^T = B × A^T  (column-major)
    // d_out interpreted as C^T [N × M], leading dim = M
    cublasSgemm(handle,
                CUBLAS_OP_T, CUBLAS_OP_N,
                M, N, K,
                &alpha,
                d_expr,    K,   // A row-major [M×K]: pass as col-major [K×M], op=T gives [M×K]
                d_weights, K,   // B row-major [N×K]: pass as col-major [K×N], op=N keeps [K×N]
                &beta,
                d_out,     M);  // C col-major [M×N] = result [M×N] row-major

    cublasDestroy(handle);

    if (!pparams.normalize) return;

    // Per-pathway z-score normalization across cells (Welford single-pass).
    // d_out layout: n_cells × n_pathways row-major.
    const int n_cells    = M;
    const int n_pathways = N;

    cudaStreamSynchronize(stream);
    std::vector<float> h_act(static_cast<size_t>(n_cells) * n_pathways);
    cudaMemcpy(h_act.data(), d_out,
               h_act.size() * sizeof(float), cudaMemcpyDeviceToHost);

    for (int p = 0; p < n_pathways; ++p) {
        double mean = 0.0, M2 = 0.0;
        for (int c = 0; c < n_cells; ++c) {
            double v = h_act[static_cast<size_t>(c) * n_pathways + p];
            double delta = v - mean;
            mean += delta / (c + 1);
            M2   += delta * (v - mean);
        }
        float std_p = (float)std::sqrt(M2 / n_cells + (double)pparams.norm_eps);
        for (int c = 0; c < n_cells; ++c) {
            h_act[static_cast<size_t>(c) * n_pathways + p] =
                (h_act[static_cast<size_t>(c) * n_pathways + p] - (float)mean) / std_p;
        }
    }
    cudaMemcpy(d_out, h_act.data(),
               h_act.size() * sizeof(float), cudaMemcpyHostToDevice);
}

} }  // namespace singlet::gpu::enrich
...
namespace singlet::gpu { namespace preprocess {

inline void csc_to_dense_row_major(
    const singlet::gpu::io::PzDeviceMatrix& /*pz_mat*/,
    float* /*d_dense*/,
    cudaStream_t /*stream*/)
{
...
// Run singlet::gpu::enrich::ssgsea_run() on device.
...
TEST(SsGseaCorrectness, Ssgsea_TinySynthetic_VsGsva) {
    ...
    // singlet::gpu::enrich::ssgsea_run expects a device dense float32 matrix
    ...
    // Pack gene sets for singlet::gpu::enrich::ssgsea_run.
    ...
    // ---- Run singlet::gpu ssGSEA. ---------------------------------------------
    cudaStream_t stream;
    ASSERT_EQ(cudaStreamCreate(&stream), cudaSuccess);

    singlet::gpu::enrich::SsGseaParams params;
...
TEST(SsGseaCorrectness, Ssgsea_GSM_RealData) {
    ...
    singlet::gpu::io::PzDeviceMatrix pz_mat;
    ASSERT_NO_THROW(pz_mat = singlet::gpu::io::load_pz_device(kGsmExonPath, stream));
    ...
    // Densify: singlet::gpu::preprocess::csc_to_dense_row_major
    ASSERT_NO_THROW(singlet::gpu::preprocess::csc_to_dense_row_major(
    ...
    // FIX: The previous code called csc_to_dense_row_major (stub → d_dense=zero) then
    // ssgsea_run (stub → d_scores=zero), causing variance=0 test failure.
    // Call the real ssgsea() API directly with pz_mat.mat (already a DeviceCSC).

    // Build SsGseaGeneSets from the gs_map already loaded above.
    singlet::gpu::enrich::SsGseaGeneSets gs_real;
    for (auto& [nm, _] : gs_map) gs_real.set_names.push_back(nm);
    gs_real.set_offsets = std::vector<int>(set_offsets.begin(), set_offsets.end());
    gs_real.gene_idx    = std::vector<int>(set_gene_idx.begin(), set_gene_idx.end());

    singlet::gpu::enrich::SsGseaConfig ss_cfg;
    ss_cfg.alpha     = kSsGseaAlpha;
    ss_cfg.cell_tile = 512;   // small tile to stay within VRAM on 11k-cell matrix

    singlet::gpu::enrich::SsGseaResult ss_res;
    ASSERT_NO_THROW(ss_res = singlet::gpu::enrich::ssgsea(pz_mat.mat, gs_real, ss_cfg, stream));
    ASSERT_EQ(cudaStreamSynchronize(stream), cudaSuccess);

    ASSERT_EQ(ss_res.n_cells, n_cells);
    ASSERT_EQ(ss_res.n_sets,  n_sets);

    // Download: SsGseaResult::scores is [n_sets × n_cells] column-major.
    // Transpose to [n_cells × n_sets] row-major for the variance check below.
    std::vector<float> h_scores_raw = device_to_host(
        ss_res.scores.get(), static_cast<size_t>(n_sets) * n_cells, stream);
    auto h_scores = std::vector<float>(static_cast<size_t>(n_cells) * n_sets);
    for (int s = 0; s < n_sets; ++s)
        for (int c = 0; c < n_cells; ++c)
            h_scores[static_cast<size_t>(c) * n_sets + s] =
                h_scores_raw[static_cast<size_t>(s) * n_cells + c];
...
TEST(SsGseaCorrectness, Ssgsea_Determinism_BitIdentical) {
    ...
    ASSERT_NO_THROW(singlet::gpu::enrich::ssgsea_run(
...
TEST(ProgenyCorrectness, Progeny_Determinism_BitIdentical) {
    ...
    singlet::gpu::enrich::ProgenyParams pp;
    pp.n_cells = kN; pp.n_genes = kG; pp.n_pathways = kP; pp.normalize = true;

    ASSERT_NO_THROW(singlet::gpu::enrich::progeny_run(d_expr, d_W, d_a1, pp, stream));
    ASSERT_EQ(cudaStreamSynchronize(stream), cudaSuccess);
    ASSERT_NO_THROW(singlet::gpu::enrich::progeny_run(d_expr, d_W, d_a2, pp, stream));
    ASSERT_EQ(cudaStreamSynchronize(stream), cudaSuccess);

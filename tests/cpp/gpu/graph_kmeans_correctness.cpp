...
//
// Correctness harness for singlet::gpu::graph::kmeans (Lloyd's algorithm).
//
// Tests:
...
TEST_F(KmeansFixture, Kmeans_TwoClusters_RecoverPlanted) {
    ...
    singlet::gpu::graph::KmeansConfig cfg;
    cfg.seed = 1234u;
    auto result = singlet::gpu::graph::kmeans(d_X.get(), D, N, K, cfg, stream_);
...
TEST_F(KmeansFixture, Kmeans_Determinism_SameSeed) {
    ...
    auto r1 = singlet::gpu::graph::kmeans(d_X.get(), D, N, K, cfg, stream_);
    auto r2 = singlet::gpu::graph::kmeans(d_X.get(), D, N, K, cfg, stream_);

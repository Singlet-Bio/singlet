...
//
// Correctness harness for singlet::gpu::cna::numbat_cna.
//
...
namespace singlet::gpu { namespace cna {
...
struct PzDeviceLoader {
    ...
    core::DeviceCSC load_csc(cudaStream_t s) const {
        return singlet::gpu::io::load_pz(path_, s).mat;
    }
    ...
    core::DeviceCSC load(const std::string& p, cudaStream_t s) const {
        return singlet::gpu::io::load_pz(p, s).mat;
    }
};
} }
...
static singlet::gpu::cna::NumbatCnaConfig default_config() {
    singlet::gpu::cna::NumbatCnaConfig cfg;
    ...
    cfg.smooth_window  = 5;
    cfg.min_cells      = 10;
    cfg.reference_mode = singlet::gpu::cna::NumbatCnaConfig::ReferenceMode::BulkMean;
    return cfg;
}
...
TEST(NumbatCna, Numbat_TinySynthetic_VsR) {
    ...
    singlet::gpu::cna::NumbatCnaConfig cfg = default_config();

    // Output buffers (device): cna_states (n_cells × n_segments), posteriors
    singlet::gpu::cna::NumbatCnaOutput gpu_out =
        singlet::gpu::cna::numbat_cna(
...
TEST(NumbatCna, Numbat_GSM_RealTumor) {
    ...
    singlet::gpu::io::PzDeviceLoader loader(kTumorSamplePath, stream);
    const auto csc = loader.load();

    // Log-normalize (cycle 2) before CNA calling
    singlet::gpu::preprocess::LognormConfig lnorm_cfg;
    lnorm_cfg.target_sum = 1e4f;
    auto lnorm_csc = singlet::gpu::preprocess::lognorm(csc, lnorm_cfg, stream);
...
TEST(NumbatCna, Numbat_Determinism_BitIdentical) {
    ...
    auto run_once = [&](uint64_t seed) {
        ...
        singlet::gpu::cna::NumbatCnaOutput out =
            singlet::gpu::cna::numbat_cna(
                d_expr, kTinyCells, kTinyGenes,
                d_chr, d_pos,
                cfg, stream);

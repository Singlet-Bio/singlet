...
singlet::gpu::reduce::nmf::NmfConfig base_cfg(int iters = kRegIter)
{
...
float frob(const singlet::gpu::reduce::nmf::DenseMatrix& M)
{
    float acc = 0.f;
...
float sparsity(const singlet::gpu::reduce::nmf::DenseMatrix& W, float thr = 1e-3f)
{
    int cnt = 0;
...
float max_offdiag_gram(const singlet::gpu::reduce::nmf::DenseMatrix& W)
{
    int m = W.m, k = W.n;
    // Column norms
...
bool is_valid_nmf_factor(const singlet::gpu::reduce::nmf::DenseMatrix& M)
{
    for (float v : M.data)
...
TEST(NmfTest, Regularization_L1_IncreasesSparsity)
{
    ...
    auto cfg0 = base_cfg();
    const auto r0 = singlet::gpu::reduce::nmf::fit(pz, cfg0);
    ...
    auto cfg1 = base_cfg();
    cfg1.W.L1 = 0.1f;
    const auto r1 = singlet::gpu::reduce::nmf::fit(pz, cfg1);
...
TEST(NmfTest, Regularization_L2_ShrinksNorm)
{
    ...
    auto cfg2 = base_cfg();
    cfg2.W.L2 = 0.1f;
    const auto r2 = singlet::gpu::reduce::nmf::fit(pz, cfg2);
...
TEST(NmfTest, Regularization_Ortho_ReducesOffDiagGram)
{
    ...
    auto cfg3 = base_cfg();
    cfg3.W.ortho = 0.1f;
    const auto r3 = singlet::gpu::reduce::nmf::fit(pz, cfg3);
...
TEST(NmfTest, Regularization_CombinedConvergesNoNaN)
{
    ...
    const auto r = singlet::gpu::reduce::nmf::fit(pz, cfg);

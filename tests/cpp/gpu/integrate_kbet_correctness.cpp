...
TEST(KbetTest, Kbet_PerfectMixing_LowChi2)
{
    ...
    singlet::gpu::integrate::KbetConfig cfg;
    cfg.deterministic = true;
    auto res = singlet::gpu::integrate::kbet(knn, d_bat.get(), N_BATCHES, cfg);
...
TEST(KbetTest, Kbet_Determinism_BitIdentical)
{
    ...
    auto res1 = singlet::gpu::integrate::kbet(knn1, d_bat.get(), N_BATCHES, cfg);
    auto h1   = d2h_float(res1.chi2, N);
    auto p1   = d2h_float(res1.pvalue, N);

    // Run 2 (re-upload).
    auto knn2 = build_knn_result(N, K, make_row_offsets(N, K), h_nbr);
    auto res2 = singlet::gpu::integrate::kbet(knn2, d_bat.get(), N_BATCHES, cfg);

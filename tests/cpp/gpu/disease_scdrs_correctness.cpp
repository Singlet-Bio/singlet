...
//
// Correctness harness for singlet::gpu::disease::scdrs.
//
...
// ---------------------------------------------------------------------------
static singlet::gpu::core::DeviceCSC dense_to_device_csc(
...
static ScDrsHostResult download_scdrs_result(
        const singlet::gpu::disease::ScDrsResult& res) {
    ScDrsHostResult h;
...
TEST(ScDRSCorrectness, Scdrs_TinySynthetic_VsPython) {
    ...
    // -----------------------------------------------------------------------
    // Run GPU kernel: singlet::gpu::disease::score()
    // -----------------------------------------------------------------------
    ...
    std::vector<singlet::gpu::disease::DiseaseGeneSet> kernel_sets;
    ...
    singlet::gpu::core::DeviceCSC expr_csc =
    ...
    singlet::gpu::disease::ScDrsResult dev_result =
        singlet::gpu::disease::score(expr_csc, gene_names, kernel_sets, cfg, ctx);
...
TEST(ScDRSCorrectness, Scdrs_GSM_RealData) {
    ...
    singlet::gpu::io::PzDeviceMatrix dev_mat =
        singlet::gpu::io::load_pz(kGsmExprPath);
    ...
    std::vector<singlet::gpu::disease::DiseaseGeneSet> kernel_sets;
    {
        ...
        std::map<std::string, singlet::gpu::disease::DiseaseGeneSet> ds_map;
    ...
    singlet::gpu::core::GPUContext& ctx = singlet::gpu::core::default_context();
    singlet::gpu::disease::ScDrsResult dev_result =
        singlet::gpu::disease::score(dev_mat.mat, gene_names, kernel_sets, cfg, ctx);
...
TEST(ScDRSCorrectness, Scdrs_Determinism_BitIdentical) {
    ...
    auto run_once = [&]() -> ScDrsHostResult {
        ...
        if (gpu_available()) {
            ...
            cudaStreamDestroy(s);
            singlet::gpu::core::GPUContext& ctx = singlet::gpu::core::default_context();
            singlet::gpu::disease::ScDrsResult dev_r =
                singlet::gpu::disease::score(csc, gene_names_local, kernel_sets, cfg, ctx);

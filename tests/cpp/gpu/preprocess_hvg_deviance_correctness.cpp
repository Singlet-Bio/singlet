...
//
// Correctness harness for singlet::gpu::preprocess::deviance_feature_selection.
...
TEST_F(DevianceHvgTest, Test2_SinglePlantedSpikeGene) {
    ...
    // Run GPU kernel
    singlet::gpu::core::DeviceCSC d_mat = upload_csc(h2, stream_);
...
TEST_F(DevianceHvgTest, Test3_RealData_GSM4037629_vs_Scry) {
    ...
    ASSERT_NO_THROW({
        pz = singlet::gpu::io::load_pz(kGsm4037629GeneCountsPath, stream_);
...
TEST_F(DevianceHvgTest, Test5_DeterminismIdempotent) {
    ...
    // Run 1
    singlet::gpu::preprocess::DevianceHvgResult res1;
    ASSERT_NO_THROW({
        res1 = singlet::gpu::preprocess::deviance_feature_selection(d_mat, cfg, stream_);
    }) << "Run 1 threw";

    HostDevianceResult hr1 = copy_deviance_result(res1, TOP_N, N_GENES, stream_);

    // Run 2 — identical input, identical config
    singlet::gpu::preprocess::DevianceHvgResult res2;
    ASSERT_NO_THROW({
        res2 = singlet::gpu::preprocess::deviance_feature_selection(d_mat, cfg, stream_);
    }) << "Run 2 threw";

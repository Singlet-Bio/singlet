...
//
// Correctness harness for singlet::gpu::preprocess::select_hvg.
//
...
HostHvgResult copy_hvg_result(const singlet::gpu::preprocess::HvgResult& res,
                               int    top_n,
                               uint32_t m,
                               cudaStream_t stream)
{
...
struct HvgTestContext {
    singlet::gpu::preprocess::HvgFlavor flavor;
...
TEST_F(HvgTest, Tiny_SeuratV3_Top50) {
    ...
    HvgTestContext ctx{};
    ctx.flavor          = singlet::gpu::preprocess::HvgFlavor::SeuratV3;
...
TEST_F(HvgTest, Tiny_PearsonResiduals_Top50) {
    ...
    HvgTestContext ctx{};
    ctx.flavor          = singlet::gpu::preprocess::HvgFlavor::PearsonResiduals;
...
TEST_F(HvgTest, Gsm4037629_SeuratV3_Top2000) {
    ...
    ASSERT_NO_THROW({
        pz = singlet::gpu::io::load_pz(kGsm4037629Path, stream_);
...
TEST_F(HvgTest, EdgeCase_ZeroVarGenes_Excluded) {
    ...
    for (auto flavor : {singlet::gpu::preprocess::HvgFlavor::SeuratV3,
                         singlet::gpu::preprocess::HvgFlavor::PearsonResiduals})
    {
        singlet::gpu::preprocess::HvgConfig cfg{};
        cfg.flavor = flavor;
        cfg.top_n  = 3;  // only 3 eligible genes (2–4)

        singlet::gpu::preprocess::HvgResult res;
        EXPECT_NO_THROW({
            res = singlet::gpu::preprocess::select_hvg(d_mat, cfg, stream_);
...
TEST_F(DevianceHvgTest, Uniform_NullBinomial_Smoke) {
    ...
    singlet::gpu::preprocess::DevianceHvgConfig cfg{};
    cfg.top_n          = TOP_N;
    cfg.use_poisson    = false;
    cfg.min_gene_total = 1.f;

    singlet::gpu::preprocess::DevianceHvgResult res;
    ASSERT_NO_THROW({
        res = singlet::gpu::preprocess::deviance_feature_selection(d_mat, cfg, stream_);
...
TEST_F(DevianceHvgTest, WithLibSizes_MatchesAutoComputed) {
    ...
    singlet::gpu::core::DeviceMemory<float> d_ls(N_CELLS);
    ...
    singlet::gpu::preprocess::DevianceHvgResult r_auto, r_lib;
    ASSERT_NO_THROW({
        r_auto = singlet::gpu::preprocess::deviance_feature_selection(
                     d_mat, cfg, stream_);
    });
    ASSERT_NO_THROW({
        r_lib  = singlet::gpu::preprocess::deviance_feature_selection_with_lib_sizes(
...
HostDevResult copy_dev_result_hvg(
        const singlet::gpu::preprocess::DevianceHvgResult& res,
...
TEST_F(DevianceHvgTest, HvgDeviance2_RealData_GSM4037629_vs_Scry)
{
    ...
    singlet::gpu::preprocess::DevianceHvgResult res;
    ASSERT_NO_THROW({
        res = singlet::gpu::preprocess::deviance_feature_selection(pz.mat, cfg, stream_);
...
TEST_F(DevianceHvgTest, HvgDeviance3_LargeScale_100k_vs_Scry)
{
    ...
    // GPU kernel — time it.
    singlet::gpu::core::DeviceCSC d_tiled = upload_csc(h_tiled, stream_);
    ...
    ASSERT_NO_THROW({
        res = singlet::gpu::preprocess::deviance_feature_selection(d_tiled, cfg, stream_);
...
TEST_F(DevianceHvgTest, HvgDeviance4_PoissonBinomialAgreement)
{
    ...
    singlet::gpu::preprocess::DevianceHvgConfig cfg_b, cfg_p;
    cfg_b.top_n = cfg_p.top_n = TOP_N;
    cfg_b.min_gene_total = cfg_p.min_gene_total = 0.0f;
    cfg_b.use_poisson = false;
    cfg_p.use_poisson = true;

    singlet::gpu::preprocess::DevianceHvgResult res_b, res_p;
    ASSERT_NO_THROW({
        res_b = singlet::gpu::preprocess::deviance_feature_selection(d_mat, cfg_b, stream_);
    }) << "deviance_feature_selection (binomial) threw on HvgDeviance4";
    ASSERT_NO_THROW({
        res_p = singlet::gpu::preprocess::deviance_feature_selection(d_mat, cfg_p, stream_);
...
TEST_F(DevianceHvgTest, HvgDeviance5_StreamingAssociativity)
{
    ...
    singlet::gpu::core::DeviceCSC d_full = upload_csc(h_full, stream_);
    singlet::gpu::preprocess::DevianceHvgConfig cfg;
    cfg.top_n          = TOP_N;
    cfg.min_gene_total = 0.0f;
    cfg.use_poisson    = false;

    singlet::gpu::preprocess::DevianceHvgResult res_full;
    ASSERT_NO_THROW({
        res_full = singlet::gpu::preprocess::deviance_feature_selection(
            d_full, cfg, stream_);

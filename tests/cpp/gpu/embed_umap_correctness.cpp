...
//
// Correctness harness for singlet::gpu::embed::umap.
//
...
std::vector<float> copy_embedding_to_host(
        const singlet::gpu::embed::UmapResult& result) {
    int n    = result.n;
...
//
// Wraps the host kNN arrays into a singlet::gpu::graph::KnnResult by uploading
// neighbors and distances to device memory.
// The KnnResult holds n * k entries in flat row-major order.
// =============================================================================

singlet::gpu::graph::KnnResult make_knn_result_on_device(
    ...
    knn.neighbors  = singlet::gpu::core::DeviceMemory<int>(n_elems);
    knn.distances  = singlet::gpu::core::DeviceMemory<float>(n_elems);
...
TEST_F(UmapCorrectnessTest, Umap_TinySynthetic_TrustworthinessHigh) {
    ...
    umap_cfg.seed         = 42;
    auto result = singlet::gpu::embed::umap(knn, umap_cfg, nullptr);
...
TEST_F(UmapCorrectnessTest, Umap_Determinism_RandomInit_BitIdentical) {
    ...
    // First run.
    auto result1 = singlet::gpu::embed::umap(knn, umap_cfg, nullptr);
    auto h_emb1  = copy_embedding_to_host(result1);

    // Second run — same config, same knn.
    auto result2 = singlet::gpu::embed::umap(knn, umap_cfg, nullptr);
...
TEST_F(UmapCorrectnessTest, Umap_RequireKnnGraph_Throws) {
    singlet::gpu::graph::KnnResult empty_knn;
    ...
    EXPECT_THROW(
        { auto result = singlet::gpu::embed::umap(empty_knn, umap_cfg, nullptr); },
        std::exception

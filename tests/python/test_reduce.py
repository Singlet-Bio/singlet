...
"""
Cycle 19 correctness tests for singlet.gpu.reduce (svd.py + nmf.py).
...
def test_pca_vs_scanpy(gsm4037629_path):
    ...
    Procedure:
      ...
      3. singlet.gpu.reduce.pca(adata_gpu, n_comps=50, inplace=True).
...
def test_pca_backend_auto_select(gsm4037629_path):
    ...
    singlet.gpu.preprocess.normalize_total(adata_auto, inplace=True)
    singlet.gpu.preprocess.log1p(adata_auto, inplace=True)

    adata_rand = _load_gpu_adata(gsm4037629_path)
    singlet.gpu.preprocess.normalize_total(adata_rand, inplace=True)
    singlet.gpu.preprocess.log1p(adata_rand, inplace=True)

    # backend='auto' must not raise.
    singlet.gpu.reduce.pca(adata_auto, n_comps=_N_COMPS, backend="auto", inplace=True)
    ...
    singlet.gpu.reduce.pca(adata_rand, n_comps=_N_COMPS, backend="randomized", inplace=True)
...
def test_nmf_basic_smoke(gsm4037629_path):
    ...
    n_factors = 10
    ret = singlet.gpu.reduce.nmf(adata, n_factors=n_factors, max_iter=10, inplace=True)
...
def test_nmf_writes_obsm_X_nmf(gsm4037629_path):
    ...
    n_factors = 10
    singlet.gpu.reduce.nmf(adata, n_factors=n_factors, max_iter=10, inplace=True)
...
def test_nmf_chunked_smoke(gsm4037629_path):
    ...
    result = singlet.gpu.reduce.nmf_chunked(
        paths=[pz_dir, pz_dir],
        n_factors=n_factors,
        chunk_cols=5000,
        max_iter=5,
    ...
    if H is None:
        H = getattr(result, "loadings", None)
    if hasattr(H, "get"):
        H = H.get()
    H = np.asarray(H)
...
def test_pca_inplace_vs_copy(gsm4037629_path):
    ...
    singlet.gpu.preprocess.normalize_total(adata_ip, inplace=True)
    singlet.gpu.preprocess.log1p(adata_ip, inplace=True)

    ret = singlet.gpu.reduce.pca(adata_ip, n_comps=_N_COMPS, inplace=True)
    assert ret is None, (
    ...
    singlet.gpu.preprocess.normalize_total(adata_orig, inplace=True)
    singlet.gpu.preprocess.log1p(adata_orig, inplace=True)

    adata_copy = singlet.gpu.reduce.pca(adata_orig, n_comps=_N_COMPS, inplace=False, copy=True)

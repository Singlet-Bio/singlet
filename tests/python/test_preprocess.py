...
"""
Cycle 19 correctness tests for singlet.gpu.preprocess (lognorm.py + hvg.py).
...
def test_normalize_total_vs_scanpy(gsm4037629_path):
    ...
    Procedure:
      ...
      3. Run singlet.gpu.preprocess.normalize_total(adata_gpu, inplace=True).
...
def test_normalize_total_inplace_vs_copy(gsm4037629_path):
    ...
    # --- inplace=True path ---
    ret = singlet.gpu.preprocess.normalize_total(adata, inplace=True)
    assert ret is None, (
    ...
    adata2_normed = singlet.gpu.preprocess.normalize_total(adata2, inplace=False, copy=True)
...
def test_normalize_total_layer_param(gsm4037629_path):
    ...
    ret = singlet.gpu.preprocess.normalize_total(adata, layer="raw", inplace=True)
...
def test_log1p_base_param(gsm4037629_path):
    ...
    singlet.gpu.preprocess.normalize_total(adata_nat, inplace=True)
    singlet.gpu.preprocess.normalize_total(adata_b2, inplace=True)

    singlet.gpu.preprocess.log1p(adata_nat, inplace=True)          # natural log
    singlet.gpu.preprocess.log1p(adata_b2, base=2.0, inplace=True) # log2
...
def test_highly_variable_genes_vs_scanpy_seurat_v3(gsm4037629_path):
    ...
    Procedure:
      ...
      2. Run singlet.gpu.preprocess.highly_variable_genes(n_top_genes=2000, flavor='seurat_v3').
...
def test_drop_in_replacement_for_scanpy_pp_normalize_total(gsm4037629_path):
    ...
    Steps:
      1. Import scanpy.
      2. Replace sc.pp.normalize_total with singlet.gpu.preprocess.normalize_total.
    ...
    try:
        sc.pp.normalize_total = singlet.gpu.preprocess.normalize_total

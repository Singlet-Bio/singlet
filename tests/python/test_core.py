...
Correctness reference: ``singlet.io.read_matrix`` (the singlet pure-Python
...
# These are the exact strings the singlet pipeline embedded in the file's
...
def test_load_pz_basic(gsm4037629_path):
    ...
    The design doc says 'rows == 30000 (or whatever singlet wrote)' — we use
...
def test_metadata_fields(gsm4037629_path):
    """Verify that singlet-embedded TLV user_kv fields are accessible via
    the Python binding's Metadata struct.

    Fields checked:
      - gsm_id  (known constant)
      - gse_id  (known constant)
      - protocol  (non-empty string; singlet auto-detected 10x_v3)
    ...
    non-empty for protocol/organism so the test survives minor singlet
...
def test_cuda_array_interface_zero_copy(gsm4037629_path):
    ...
    Bit identity holds because the data path is:
      singlet pz_reader.h  ─► pinned host buffer ─► cudaMemcpyAsync ─► device
    ...
    # Singlet scipy reference
    try:
        from singlet.io import read_matrix as singlet_read_matrix
    except ImportError:
        pytest.skip(
            "singlet.io not importable — cannot generate scipy reference. "
            "Install singlet Python package to run this test."
        )

    sp_csc, _ = singlet_read_matrix(str(pz_path))
    ...
    ), "indptr mismatch between device view and singlet scipy reference"

    assert np.array_equal(
        csc_host.indices.astype(np.int64),
        sp_csc.indices.astype(np.int64),
    ), "indices mismatch between device view and singlet scipy reference"

    assert np.array_equal(
        csc_host.data.astype(np.float32),
        sp_csc.data.astype(np.float32),
    ), "data mismatch between device view and singlet scipy reference"
...
def test_anndata_roundtrip(gsm4037629_path):
    ...
    Checks:
      ...
      - adata.uns['singlet']['gsm_id'] == 'GSM4037629'
...
def test_to_host_explicit_copy(gsm4037629_path):
    ...
    Correctness: element-wise identity between the host copy and the singlet
    scipy reference loaded via singlet.io.read_matrix.

    The .to_host() call must:
      - Return a scipy.sparse.csr_matrix (CSR, not CSC — per design doc).
      - Have the same shape as (m.rows, m.cols).
      - Contain bit-identical values to the singlet reference.
    """
    ...
    ), "to_host() indptr does not match singlet scipy reference"

    assert np.array_equal(
        host_csr.indices.astype(np.int64),
        sp_csr_ref.indices.astype(np.int64),
    ), "to_host() indices do not match singlet scipy reference"

    assert np.array_equal(
        host_csr.data.astype(np.float32),
        sp_csr_ref.data.astype(np.float32),
    ), "to_host() data does not match singlet scipy reference"

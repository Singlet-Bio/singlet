...
`star_main_impl()` is the renamed `main()` of STAR 2.7.11b (singlet-lite variant). It accepts the same command-line arguments as the STAR binary. singlify calls it in a forked child process, which writes unsorted BAM to stdout (a pipe) that the parent reads into the pileup engine.

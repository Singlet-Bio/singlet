...
// Forward declaration so the convenience wrapper below can reference
// SparseAccumulator<ValT>::CSCMatrix without forcing every translation
// unit that includes pz_writer.h to also include sparse_accumulator.h.
// Translation units that actually instantiate write_1pz_csc must still
// include sparse_accumulator.h before this header — but read-only callers
// (like the pz_reader.h Python/R bindings) get a parse-clean experience.
template<typename ValT> class SparseAccumulator;

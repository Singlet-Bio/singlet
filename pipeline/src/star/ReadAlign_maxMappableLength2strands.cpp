...
uint ReadAlign::maxMappableLength2strands(uint pieceStartIn, uint pieceLengthIn, uint iDir, uint iSA1, uint iSA2, uint& maxLbest, uint iFrag) {
    ...
    for (uint iDist = 0; iDist < min(pieceLengthIn, P.pGe.gSAsparseD); iDist++) {  // cycle through different distances
        ...
        // S3: SA narrowing via minimizer index — DEAD END (see git log)
        // Tested 2026-04-11: narrowing changes SJ.out.tab and adds 41% overhead.
        // Root causes:
        //   1. Correctness: minimizer of read != minimizer at true SA position for
        //      many uniquely-mapped reads → false exclusions → different alignments.
        //   2. Performance: binary search in 4.8GB mmap (~28 cache misses/query)
        //      adds ~40s overhead for 40M reads, exceeding any SA range reduction.
        // Fix needed: (a) mmap-copy to anonymous RAM for in-cache lookups, and
        //             (b) only narrow when 14-mer SA range > threshold (e.g., > 500).
        // Guard left here so the index still loads and machinery is in place.
        (void)mapGen.minimizerIdx;  // suppress unused warning
#endif

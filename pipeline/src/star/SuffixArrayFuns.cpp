...
uint maxMappableLength(Genome &mapGen, char** s, uint S, uint N, uint i1, uint i2, bool dirR, uint& L, uint* indStartEnd)
{
...
#ifdef SA_INIT_GENOME_PREFETCH
    // Pre-read SA entries for both boundaries (should be in L1/L2 from
    // SA_BOUNDARY_PREFETCH issued in maxMappableLength2strands).
    // Decode genome positions and issue prefetches so the genome data
    // is arriving in DRAM while we set up the first compareSeqToGenome.
    // This gives G[gpos2] ~100ns of lead time and G[gpos1] ~50ns.
    {
        uint SAv1 = mapGen.SA[i1];
        uint SAv2 = mapGen.SA[i2];
        bool dG1 = (SAv1 >> mapGen.GstrandBit) == 0;
        bool dG2 = (SAv2 >> mapGen.GstrandBit) == 0;
        uint64 gp1 = dG1 ? ((SAv1 & mapGen.GstrandMask) + L)
                         : (mapGen.nGenome - 1 - (SAv1 & mapGen.GstrandMask) - L);
        uint64 gp2 = dG2 ? ((SAv2 & mapGen.GstrandMask) + L)
                         : (mapGen.nGenome - 1 - (SAv2 & mapGen.GstrandMask) - L);
#ifdef GENOME_PACKED
        __builtin_prefetch(mapGen.gPacked.G_packed + (gp1 >> 5), 0, 0);
        __builtin_prefetch(mapGen.gPacked.G_packed + (gp2 >> 5), 0, 0);
#else
        __builtin_prefetch(mapGen.G + gp1, 0, 0);
        __builtin_prefetch(mapGen.G + gp2, 0, 0);
#endif
        // Also prefetch genome for first midpoint
        if (i1 + 1 < i2) {
            uint mid = medianUint2(i1, i2);
            uint SAvm = mapGen.SA[mid];
            bool dGm = (SAvm >> mapGen.GstrandBit) == 0;
            uint64 gpm = dGm ? ((SAvm & mapGen.GstrandMask) + L)
                             : (mapGen.nGenome - 1 - (SAvm & mapGen.GstrandMask) - L);
#ifdef GENOME_PACKED
            __builtin_prefetch(mapGen.gPacked.G_packed + (gpm >> 5), 0, 0);
#else
            __builtin_prefetch(mapGen.G + gpm, 0, 0);
#endif
        }
    }
#endif
    ...
    while (i1+1<i2) {//main binary search loop
        ...
        if (i1+1<i2) {
            ...
            __builtin_prefetch(mapGen.SA.charArray + (uint64_t)i3_next * mapGen.SA.wordLength / 8, 0, 0);
#ifdef SA_INIT_GENOME_PREFETCH
            // Also pre-read SA for next midpoint and prefetch its genome position.
            // SA[i3_next] should arrive from the prefetch above (~100ns latency).
            // Meanwhile, issue genome prefetch so it's in flight during the comparison.
            uint SAv_next = mapGen.SA[i3_next];
            bool dG_next = (SAv_next >> mapGen.GstrandBit) == 0;
            uint64 gp_next = dG_next ? ((SAv_next & mapGen.GstrandMask) + L)
                                     : (mapGen.nGenome - 1 - (SAv_next & mapGen.GstrandMask) - L);
#ifdef GENOME_PACKED
            __builtin_prefetch(mapGen.gPacked.G_packed + (gp_next >> 5), 0, 0);
#else
            __builtin_prefetch(mapGen.G + gp_next, 0, 0);
#endif
#endif
        }
#endif

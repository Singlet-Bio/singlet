...
void Genome::genomeLoad() {  // allocate and load Genome
    ...
    *(P.inOut->logStdOut) << timeMonthDayTime(rawtime) << " ..... loading genome\n"
    ...
    P.inOut->logMain << "nGenome=" << nGenome << ";  nSAbyte=" << nSAbyte << endl
                     << flush;
    P.inOut->logMain << "GstrandBit=" << int(GstrandBit) << "   SA number of indices=" << nSA << endl
                     << flush;
    ...
    if ((pGe.gLoad == "LoadAndKeep" ||
        ...
        if (!sharedMemory->IsAllocator()) {
            ...
            P.inOut->logMain << "Using shared memory for genome. key=0x" << hex << shmKey << dec << ";   shmid=" << sharedMemory->GetId() << endl
                             << flush;
        };
        ...
        try {
            ...
            errOut << "Possible cause 2: not enough virtual memory allowed with ulimit. SOLUTION: run ulimit -v " << nGenome + L + L + SA.lengthByte + SAi.lengthByte + 2000000000 << endl
                   << flush;
...
#ifdef MINIMIZER_SA_INDEX
    {
        string minimizerPath = pGe.gDir + "/minimizerIndex.bin";
        struct stat st;
        if (stat(minimizerPath.c_str(), &st) == 0) {
            try {
                minimizerIdx = new MinimizerSAIndex();
                minimizerIdx->mmap_load(minimizerPath);
                P.inOut->logMain << "Minimizer index: loaded " << minimizerIdx->size()
                                 << " records (k=" << minimizerIdx->k()
                                 << " w=" << minimizerIdx->w()
                                 << " T=" << minimizerIdx->T()
                                 << ") from " << minimizerPath << "\n"
                                 << flush;
            } catch (const std::exception& e) {
                P.inOut->logMain << "WARNING: minimizer index load failed: " << e.what() << "\n"
                                 << flush;
                delete minimizerIdx;
                minimizerIdx = nullptr;
            }
        } else {
            P.inOut->logMain << "Minimizer index: not found at " << minimizerPath << " (optional, skipping)\n"
                             << flush;
        }
    }
#endif
...
// Hot path uses G_packed exclusively. This reduces TLB and cache pollution.
    ...
    if (P.alignIntronMax == 0 && P.alignMatesGapMax == 0) {
        ...
        P.inOut->logMain << "To accommodate alignIntronMax=" << P.alignIntronMax << " and alignMatesGapMax=" << P.alignMatesGapMax << ", redefined winFlankNbins=" << P.winFlankNbins << " and winAnchorDistNbins=" << P.winAnchorDistNbins << endl;
    };

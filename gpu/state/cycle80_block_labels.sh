#!/bin/bash
#SBATCH --job-name=sg_c80_block_labels
#SBATCH --partition=gpu
#SBATCH --time=00:45:00
#SBATCH --cpus-per-task=8
#SBATCH --mem=48G
#SBATCH --gres=gpu:1
#SBATCH --output=/mnt/home/debruinz/Singlet-AI/singlet-gpu/state/cycle80_block_labels_%j.log
...
BUILD_DIR=/mnt/home/debruinz/Singlet-AI/singlet-gpu/build_cycle80_block
SRC_DIR=/mnt/home/debruinz/Singlet-AI/singlet-gpu

echo "=== Cycle 80/81: block-labels fix — TinyPlanted planted-signal recovery ==="
...
# --- PRIMARY: Ttest TinyPlanted (expect Jaccard ~0.9+, LFCSpearman ~0.98, PvalRank 0.95) ---
echo ""
echo "--- ctest: Ttest.*TinyPlanted ---"
ctest --test-dir "$BUILD_DIR" -R "Ttest.*TinyPlanted" \
      --output-on-failure -V 2>&1 | tee /tmp/c80bl_ttest_${SLURM_JOB_ID}.txt
TTEST_TINY_EXIT=$?
echo "TTEST_TINY_EXIT=$TTEST_TINY_EXIT"

# --- PRIMARY: Wilcoxon TinyPlanted (expect Jaccard ~1.0, LFCSpearman 1.0, PvalRank 1.0) ---
echo ""
echo "--- ctest: Wilcoxon.*TinyPlanted ---"
ctest --test-dir "$BUILD_DIR" -R "Wilcoxon.*TinyPlanted" \
      --output-on-failure -V 2>&1 | tee /tmp/c80bl_wil_${SLURM_JOB_ID}.txt
WIL_TINY_EXIT=$?
...
ctest --test-dir "$BUILD_DIR" -R "Test83_RealDataSized_PostNormalize_NoCrash" \
      --output-on-failure -V 2>&1 | tee /tmp/c80bl_test83_${SLURM_JOB_ID}.txt
TEST83_EXIT=$?
...
ctest --test-dir "$BUILD_DIR" -R "Wilcoxon_GSM4037629_RealDataPlanted" \
      --output-on-failure -V 2>&1 | tee /tmp/c80bl_rdp_${SLURM_JOB_ID}.txt
RDP_EXIT=$?
echo "RDP_EXIT=$RDP_EXIT"

echo ""
echo "=== KEY METRICS: Ttest TinyPlanted ==="
grep -E "Jaccard|Spearman|rho|PASSED|FAILED|PASS|FAIL|intersection" /tmp/c80bl_ttest_${SLURM_JOB_ID}.txt | head -40 || true

echo ""
echo "=== KEY METRICS: Wilcoxon TinyPlanted ==="
grep -E "Jaccard|Spearman|rho|PASSED|FAILED|PASS|FAIL|intersection" /tmp/c80bl_wil_${SLURM_JOB_ID}.txt | head -40 || true

echo ""
echo "=== REGRESSION Test83 verdict ==="
grep -E "PASSED|FAILED|PASS|FAIL|NaN|CUDA error" /tmp/c80bl_test83_${SLURM_JOB_ID}.txt | head -10 || true

echo ""
echo "=== REGRESSION RealDataPlanted verdict ==="
grep -E "Jaccard|Spearman|PASSED|FAILED|PASS|FAIL" /tmp/c80bl_rdp_${SLURM_JOB_ID}.txt | head -20 || true

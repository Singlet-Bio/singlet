...
- **CYCLE-154-CHOSEN-NEXT-FROM-QUEUE**: scrublet iter-1 closed FAIL with partial progress (T1 Spearman 0.24→0.54, T2 regression PASS→FAIL exposed deeper score-inflation bug). Pivoting to CYCLE-122 enrichment zero-output diagnosis OR Phase E bench backfill OR fresh port — picking next iteration. Scrublet deferred to CYCLE-148.1 with Phase B research dispatch first.

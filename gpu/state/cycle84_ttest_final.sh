#!/bin/bash
#SBATCH --job-name=sg_cycle84_ttest_final
#SBATCH --partition=gpu
#SBATCH --time=00:45:00
#SBATCH --cpus-per-task=8
#SBATCH --mem=48G
#SBATCH --gres=gpu:1
#SBATCH --output=/mnt/home/debruinz/Singlet-AI/singlet-gpu/state/cycle84_ttest_final_%j.log
...
BUILD_DIR=/mnt/home/debruinz/Singlet-AI/singlet-gpu/build_cycle84_verify
SRC_DIR=/mnt/home/debruinz/Singlet-AI/singlet-gpu

echo "=== Cycle 84: ttest PvalRankSpearman constant-vector fix + RealDataPlanted ==="
...
echo ""
echo "--- ctest: Ttest TinyPlanted (all 3 metrics — Fix A: PvalRankSpearman constant-vector) ---"
ctest --test-dir "$BUILD_DIR" \
      -R "Ttest_TinyPlanted" \
      --output-on-failure -V 2>&1 | tee /tmp/c84_tinyplanted_${SLURM_JOB_ID}.txt
TTEST_TINY_EXIT=$?
echo "TTEST_TINY_EXIT=$TTEST_TINY_EXIT"

echo ""
echo "--- ctest: Ttest GSM4037629 RealDataPlanted (Fix B: planted signal) ---"
ctest --test-dir "$BUILD_DIR" \
      -R "Ttest_GSM4037629_RealDataPlanted" \
      --output-on-failure -V 2>&1 | tee /tmp/c84_realplanted_${SLURM_JOB_ID}.txt
TTEST_REAL_EXIT=$?
...
ctest --test-dir "$BUILD_DIR" \
      ...
      --output-on-failure -V 2>&1 | tee /tmp/c84_wilcoxon_${SLURM_JOB_ID}.txt
WILCOXON_EXIT=$?
echo "WILCOXON_EXIT=$WILCOXON_EXIT"

echo ""
echo "--- ctest: Test83 + Wilcoxon RealDataPlanted (no-regression) ---"
ctest --test-dir "$BUILD_DIR" \
      -R "Test83_RealDataSized_PostNormalize_NoCrash|Wilcoxon_GSM4037629_RealDataPlanted" \
      --output-on-failure -V 2>&1 | tee /tmp/c84_regression_${SLURM_JOB_ID}.txt
REGRESSION_EXIT=$?
...
grep -E "Jaccard|LFCSpearman|PvalRank|Spearman|PASSED|FAILED|PASS|FAIL" \
    /tmp/c84_tinyplanted_${SLURM_JOB_ID}.txt | head -30 || true

echo ""
echo "=== KEY METRICS: RealDataPlanted ==="
grep -E "Jaccard|LFCSpearman|PvalRank|Spearman|PASSED|FAILED|PASS|FAIL" \
    /tmp/c84_realplanted_${SLURM_JOB_ID}.txt | head -30 || true

echo ""
echo "=== KEY METRICS: Wilcoxon regression ==="
grep -E "PASSED|FAILED|PASS|FAIL" \
    /tmp/c84_wilcoxon_${SLURM_JOB_ID}.txt | head -10 || true

echo ""
echo "=== KEY METRICS: Test83 + Wilcoxon RealDataPlanted regression ==="
grep -E "PASSED|FAILED|PASS|FAIL" \
    /tmp/c84_regression_${SLURM_JOB_ID}.txt | head -10 || true

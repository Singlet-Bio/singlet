...
//        c. PCA on RNA HVG submatrix  (factornet randomized_svd_gpu, n_pcs_rna=30).
//        d. PCA on ADT matrix         (factornet randomized_svd_gpu, n_pcs_adt=20).
...
//     L1122-1130: D2H download combined matrices for factornet SVD — one-time SVD setup.
//       factornet::randomized_svd_gpu_dense requires host input pointer.
//       CYCLE-5-FOLLOWUP-FACTORNET-STREAM-OVERLOAD tracks removing this transfer.
...
//
// port:factornet svd/randomized_gpu.cuh (via singlet-gpu reduce/svd/randomized.h adapter)
             ...
             cudaStream_t stream)
{
    ...
    // ── Step 5: Per-modality PCA via factornet randomized SVD ──────────────────
    // factornet::svd::randomized_svd_gpu_dense takes a HOST dense matrix pointer
    // and returns SVDResult with U as an Eigen (CPU) matrix (m × k).
    // factornet creates its own GPUContext internally.
    //
    // We must:
    //   a) Sync stream to ensure device matrices are fully written.
    //   b) Download combined_rna and combined_adt to host.
    //   c) Call randomized_svd_gpu_dense (host pointer → GPU internally → host result).
    ...
    // This is justified: factornet SVD does not expose a device-pointer entry point
    // for dense input (only for sparse CSC via randomized_svd_gpu). The dense variant
    // requires host input. CYCLE-5-FOLLOWUP-FACTORNET-STREAM-OVERLOAD tracks adding
    // a device-dense entry point to factornet.
    cudaStreamSynchronize(stream);
    ...
    // SVD configs.
    ::factornet::SVDConfig<float> svd_cfg_rna;
    svd_cfg_rna.k_max   = k_rna;
    svd_cfg_rna.seed    = cfg.seed;
    svd_cfg_rna.max_iter = 3;    // q=3 power iterations per design doc

    ::factornet::SVDConfig<float> svd_cfg_adt;
    svd_cfg_adt.k_max   = k_adt;
    svd_cfg_adt.seed    = cfg.seed + 1;
    svd_cfg_adt.max_iter = 3;

    // Run per-modality PCA (factornet creates its own GPUContext internally).
    // WHY NOT randomized_svd_gpu (sparse): combined matrices are dense (simulated
    // cells have been densified). randomized_svd_gpu_dense is the correct entry point.
    auto svd_rna = ::factornet::svd::randomized_svd_gpu_dense<float>(
        h_combined_rna.data(), n_total, n_hvg, svd_cfg_rna);
    auto svd_adt = ::factornet::svd::randomized_svd_gpu_dense<float>(
        h_combined_adt.data(), n_total, n_tags, svd_cfg_adt);

    // svd_rna.U: Eigen host matrix (n_total × k_rna), col-major (Eigen default).
    // svd_adt.U: Eigen host matrix (n_total × k_adt).
    ...
    // Build interleaved host buffer then upload once (H2D, one-time).
    {
        ...
        for (int i = 0; i < n_total; ++i) {
            // Copy RNA PCs for row i (Eigen col-major → row-major).
            for (int j = 0; j < k_rna; ++j)
                h_joint[(size_t)i * k_joint + j] = svd_rna.U(i, j);
            // Copy ADT PCs for row i.
            for (int j = 0; j < k_adt; ++j)
                h_joint[(size_t)i * k_joint + k_rna + j] = svd_adt.U(i, j);
        }

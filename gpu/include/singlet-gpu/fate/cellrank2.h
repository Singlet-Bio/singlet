    ...
    for (int restart = 0; restart < max_restarts; restart++) {
        ...
        for (int j = 0; j < gmres_m; j++) {
            ...
            if (j + 1 < gmres_m) {
                ...
                // Both this kernel and dscale_by_device_inv_kernel are in singlet_gpu::fate::detail.
        ...
        int          n_genes_expr    = 0)
{
    ...
    // = R_dense. This reuses cuSPARSE and avoids a custom kernel.
    {
        ...
        cusparseSpMM_bufferSize(ctx.cusparse, CUSPARSE_OPERATION_NON_TRANSPOSE,
        ...
        cusparseSpMM(ctx.cusparse, CUSPARSE_OPERATION_NON_TRANSPOSE,
                     CUSPARSE_OPERATION_NON_TRANSPOSE,
                     &alpha_r, R_mat, I_mat, &beta_r, R_dense_mat,
    ...
    for (int t_start = 0; t_start < n_terminals; t_start += batch_sz) {
        ...
        for (int t = t_start; t < t_end; t++) {
            ...
            double rel_res = detail::gmres_solve_one_rhs(
                ctx.cusparse, ctx.cublas, ImQ_desc,
                d_x.get(), d_b.get(),
                d_V.get(), d_w.get(),

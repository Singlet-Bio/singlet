...
//
// Benchmarks singlet_gpu::io::load() (our_manual and our_auto variants) against:
...
base = Path("/mnt/projects/debruinz_project/singlify_pipeline/quant/scrna")
...
static OurResult bench_our_loader(const std::string& pz_path,
    ...
    // Build config
    singlet_gpu::io::PzLoadConfig cfg;
    if (manual_config) {
    ...
    using Timer = singlet_gpu::bench::BenchTimer;
    using MemTracker = singlet_gpu::bench::PeakMemTracker;
    ...
    singlet_gpu::io::PzLoadResult last_result;
    ...
    for (int iter = 0; iter < WARMUP + TIMED; ++iter) {
        ...
        singlet_gpu::io::PzLoadResult r = manual_config
            ? singlet_gpu::io::load(pz_path, cfg)
            : singlet_gpu::io::load(pz_path);
...
// BenchRow factory
// ---------------------------------------------------------------------------
static singlet_gpu::bench::BenchRow make_row(
    ...
    double sota_mem)
{
    ...
    row.date    = singlet_gpu::bench::today_iso();
    row.feature = feature;
    row.scale   = scale;
    row.impl    = impl;
    row.wall_ms = wall_p50;
    row.mem_mb  = mem_mb;
    if (n_cols > 0) row.cells_per_sec = singlet_gpu::bench::throughput(n_cols, wall_p50);
    row.sota_wall = sota_wall;
    row.sota_mem  = sota_mem;
    row.commit    = singlet_gpu::bench::git_short_sha();
    return row;
}
...
int main(int argc, char* argv[]) {
    ...
    if (!gpu_available()) {
        singlet_gpu::bench::skip("io_pz_device_loader_bench", "no CUDA device");
        return 0;
    }
    ...
    std::printf("Date: %s\n", singlet_gpu::bench::today_iso().c_str());
    std::fflush(stdout);
    ...
    if (promote) {
        ...
        if (ofs.is_open()) {
            ...
            ofs << "\n**Notes**: Feature 0 `singlet_gpu::io::load()` dominates on "
                << dominates_on
            ...
            ofs << "**Commit**: " << singlet_gpu::bench::git_short_sha() << "\n";
        }

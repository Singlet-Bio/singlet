...
//
// Performance benchmark: singlet_gpu::de::wilcoxon_de vs scanpy rank_genes_groups.
//
        ...
        int n, int d, uint64_t seed, cudaStream_t stream)
{
    ...
    cudaMemcpyAsync(dev.get(), host.data(), host.size() * sizeof(float),
                    cudaMemcpyHostToDevice, stream);
    cudaStreamSynchronize(stream);
    singlet_gpu::core::DeviceDense mat;
    mat.rows = n; mat.cols = d;
    mat.data = std::move(dev);
    return mat;
}

// Write CSC binary (dump_csc.h format) for the Python reference.
// Magic = 0x43535343, then n_rows, n_cols (uint32), nnz (uint64),
...
singlet_gpu::bench::RefResult run_wilcoxon_reference(
        const singlet_gpu::core::DeviceCSC&        mat,
        const singlet_gpu::core::DeviceMemory<int>& d_labels,
...
int main()
{
    ...
    leiden_cfg.seed       = 42;
    singlet_gpu::graph::LeidenResult leiden_result;
    int n_clusters = 0;
    try {
        leiden_result = singlet_gpu::graph::leiden(knn_result, leiden_cfg, stream);
        cudaStreamSynchronize(stream);
    ...
    // ── Wilcoxon config ────────────────────────────────────────────────────────
    singlet_gpu::de::WilcoxonConfig wil_cfg;
    wil_cfg.n_bins       = 4096;
    wil_cfg.top_n        = 100;
    ...
    for (int i = 0; i < kWarmup; ++i) {
        auto res = singlet_gpu::de::wilcoxon_de(mat, leiden_result.labels, n_clusters, wil_cfg, stream);
        cudaStreamSynchronize(stream);
        (void)res;
    }

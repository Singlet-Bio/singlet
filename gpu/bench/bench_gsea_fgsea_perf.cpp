...
//
// Performance benchmark: singlet_gpu::gsea::fgsea + singlet_gpu::gsea::aucell
...
singlet_gpu::anno::GeneSetDB make_synthetic_genesets(
...
singlet_gpu::core::DeviceMemory<float> make_stats_device(
        int n_genes, uint64_t seed, cudaStream_t stream)
{
    ...
    singlet_gpu::core::DeviceMemory<float> d(n_genes);
...
singlet_gpu::core::DeviceCSC make_tiny_csc(
        int n_genes, int n_cells, float density, uint64_t seed, cudaStream_t stream)
{
    ...
    singlet_gpu::core::DeviceMemory<int>   d_cp(n_cells + 1);
    singlet_gpu::core::DeviceMemory<int>   d_ri(nnz);
    singlet_gpu::core::DeviceMemory<float> d_vl(nnz);
    ...
    cudaMemcpyAsync(d_vl.data(), vals.data(), nnz*sizeof(float),
                    cudaMemcpyHostToDevice, stream);
    cudaStreamSynchronize(stream);
    return singlet_gpu::core::DeviceCSC(n_genes, n_cells, nnz,
...
std::pair<float, float> run_fgsea_sota(
        const singlet_gpu::core::DeviceMemory<float>& d_stats,
...
int main()
{
    ...
    // ── fgsea config ──────────────────────────────────────────────────────────
    singlet_gpu::gsea::FgseaConfig fgsea_cfg;
    ...
    for (int i = 0; i < BENCH_WARMUP_ITERS; ++i) {
        auto r = singlet_gpu::gsea::fgsea(d_stats, genesets, fgsea_cfg, stream);
        cudaStreamSynchronize(stream);
        (void)r;
    }
    ...
    // ── AUCell config ─────────────────────────────────────────────────────────
    singlet_gpu::gsea::AUCellConfig aucell_cfg;
    ...
    for (int i = 0; i < BENCH_WARMUP_ITERS; ++i) {
        auto r = singlet_gpu::gsea::aucell(aucell_mat, genesets, aucell_cfg, stream);
        cudaStreamSynchronize(stream);
        (void)r;
    }

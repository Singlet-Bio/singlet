...
static void write_csc_bin(const std::string& p, const float* hv, const int* hp,
                           const int* hi, int nr, int nc, int64_t nnz) {
    std::ofstream f(p, std::ios::binary);
    uint32_t mg=0x43535343u,nru=nr,ncu=nc; uint64_t nz=nnz;
    f.write((char*)&mg,4);f.write((char*)&nru,4);f.write((char*)&ncu,4);f.write((char*)&nz,8);
...
static ScaleResult run_scale(const char* label, int n_genes, int n_cells, int n_clusters,
    ...
    for (int i=0;i<3;++i){ auto r2=singlet_gpu::de::wilcoxon_de(mat,d_labels,n_clusters,cfg,stream); cudaStreamSynchronize(stream);(void)r2; }
    ...
    for (int i=0;i<5;++i){
        ...
        auto r2=singlet_gpu::de::wilcoxon_de(mat,d_labels,n_clusters,cfg,stream);

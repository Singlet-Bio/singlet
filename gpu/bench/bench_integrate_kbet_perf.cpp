...
// singlet-gpu/bench/bench_integrate_kbet_perf.cpp
//
// CYCLE-171 Phase E — integrate/kbet standalone kernel benchmark.
//
// Benchmarks integrate::kbet() (from include/singlet-gpu/integrate/kbet.h)
// against a manual numpy CPU baseline at two medium scales:
//
//   Scale "10k":  n_cells=10000, n_pcs=50, k=10, n_batches=4
//   Scale "30k":  n_cells=30000, n_pcs=50, k=15, n_batches=4
//
// GPU kernel: one block per cell.  Thread 0 serially scans k neighbors,
//   builds int local_count[n_batches] in shared memory, computes chi2
//   = Σ_b (obs-exp)²/max(exp,1), then p-value via Wilson-Hilferty cube-root
...
// Reference: Büttner M et al. (2019) Nat Methods 16:43-49.
//
// Protocol:
//   kNN build — single untimed warmup (Exact, KnnBackend::Exact, L2, k=10/15).
//   kBET timing — 2 warmup + 5 timed via cudaEvent (bench::BenchTimer).
//   Memory       — bench::PeakMemTracker (cudaMemGetInfo delta).
//   Batch labels — round-robin: batch[c] = c % n_batches (device-side).
//
...
// ---------------------------------------------------------------------------
// make_round_robin_batch — device int[n] with batch[c] = c % n_batches.
// Filled on the CPU and uploaded once (batch labels are constant across all
// kBET bench iterations — only the kbet() kernel execution is timed).
// ---------------------------------------------------------------------------
static core::DeviceMemory<int> make_round_robin_batch(int n, int n_batches,
                                                       cudaStream_t stream)
{
    std::vector<int> h_batch(n);
    for (int c = 0; c < n; ++c) h_batch[c] = c % n_batches;

    core::DeviceMemory<int> d_batch(static_cast<size_t>(n));
    cudaMemcpyAsync(d_batch.get(), h_batch.data(), n * sizeof(int),
                    cudaMemcpyHostToDevice, stream);
    cudaStreamSynchronize(stream);
    return d_batch;
}
...
// ---------------------------------------------------------------------------
// bench_kbet — 2 warmup + 5 timed iterations of kbet().
// kNN result and batch labels are pre-built (untimed).
// Returns median wall_ms and peak mem_mb via out params.
// ---------------------------------------------------------------------------
static void bench_kbet(
    ...
    double&                         out_mem_mb)
{
    ...
    for (int iter = 0; iter < WARMUP + TIMED; ++iter) {
        ...
        {
            integrate::KbetConfig cfg{};
            auto result = integrate::kbet(knn, d_batch.get(), n_batches,
                                          cfg, stream);
            (void)result;
        }
...
int main() {
    if (!gpu_available()) {
        std::puts("NO GPU — skipping Cycle 171 integrate/kbet bench.");
        return 0;
    }
    ...
    for (const auto& s : scales) {
        ...
        // Build kNN graph (untimed warmup — not part of kBET timing).
        ...
        core::DeviceMemory<int> d_batch =
            make_round_robin_batch(s.n_cells, N_BATCHES, stream);

        std::printf("[bench] Timing %s kbet() (n_batches=%d k=%d)...\n",
                    s.name, N_BATCHES, s.k);
        std::fflush(stdout);

        double wall_ms = 0.0, mem_mb = 0.0;
        bench_kbet(knn, d_batch, N_BATCHES, stream, wall_ms, mem_mb);

        // CSV row.
        std::printf("%s,%d,%d,%d,%d,%.3f,%.1f\n",
                    s.name, s.n_cells, s.n_pcs, s.k, N_BATCHES,
                    wall_ms, mem_mb);
        std::fflush(stdout);
    ...
    std::printf("[bench] Cycle 171 integrate/kbet perf bench complete.\n");
    return 0;
}

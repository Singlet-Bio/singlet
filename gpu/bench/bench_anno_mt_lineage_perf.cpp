...
//
// Performance benchmark: singlet_gpu::anno::call_clones (mt_lineage.h)
//   vs mgatk R (cycle 15).
//
...
singlet_gpu::core::DeviceCSC make_mt_csc(
        ...
        uint64_t seed, cudaStream_t stream)
{
    ...
    return singlet_gpu::core::DeviceCSC(n_sites, n_cells, nnz,
...
int main()
{
    ...
    singlet_gpu::core::DeviceCSC alt_csc, dep_csc;
    ...
    for (int i = 0; i < BENCH_WARMUP_ITERS; ++i) {
        try {
            auto r = singlet_gpu::anno::call_clones(alt_csc, dep_csc, cfg, stream);
            cudaStreamSynchronize(stream);
            (void)r;
        } catch (const std::exception& ex) {

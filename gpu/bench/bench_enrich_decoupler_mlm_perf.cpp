...
// singlet-gpu/bench/bench_enrich_decoupler_mlm_perf.cpp
//
// CYCLE-165 Phase E — enrich/decoupler_mlm standalone kernel benchmark.
//
// Benchmarks decoupler_mlm::mlm() (from include/singlet-gpu/enrich/decoupler_mlm.h)
...
// Per §J.6 audit: NOT at risk of dense-n×n bug (SpMM O(nnz × p); solve O(p^2 × n)).
//
...
// ---------------------------------------------------------------------------
// bench_mlm — 2 warmup + 5 timed iterations of mlm().
// Returns median wall_ms and peak mem_mb.
// ---------------------------------------------------------------------------
static void bench_mlm(
    ...
    double&                   out_mem_mb)
{
    ...
    for (int iter = 0; iter < WARMUP + TIMED; ++iter) {
        ...
        {
            enrich::MlmConfig cfg{};
            auto result = enrich::mlm(pzmat, d_W, n_pathways, cfg, stream);
            (void)result;
        }
...
int main() {
    if (!gpu_available()) {
        std::puts("NO GPU — skipping Cycle 165 decoupler_mlm bench.");
        return 0;
    }
    ...
    for (const auto& s : scales) {
        ...
        std::printf("[bench] Timing %s method=mlm...\n", s.name);
        std::fflush(stdout);

        double wall_ms = 0.0, mem_mb = 0.0;
        bench_mlm(pzmat, d_W_buf.get(), n_pathways, stream, wall_ms, mem_mb);
        ...
        row.feature       = "enrich/decoupler_mlm";
    ...
    std::printf("[bench] Cycle 165 decoupler_mlm perf bench complete.\n");
    return 0;
}

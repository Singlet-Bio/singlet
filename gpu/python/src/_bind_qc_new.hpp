...
struct PyDoubletScoreResult {
    singlet_gpu::core::DeviceMemory<float>   score;
...
struct PyOmniDoubletResult {
    ...
    singlet_gpu::core::DeviceMemory<uint8_t> doublet_call;
    singlet_gpu::core::DeviceMemory<float>   simulated_scores;
    float threshold_used          = 0.0f;
    int   n_predicted_doublets    = 0;
    int   n_cells                 = 0;
    int   n_sim                   = 0;
...
inline void bind_qc_new(py::module_& m) {
    ...
    m.def("omni_doublet",
        ...
        {
            cudaStream_t s = resolve_stream_qc(stream);
            const singlet_gpu::core::DeviceCSC& rna = *rna_counts.csc;
            const singlet_gpu::core::DeviceCSC& adt = *adt_counts.csc;
            ...
            auto r = singlet_gpu::qc::omni_doublet(rna, adt, cfg, s);

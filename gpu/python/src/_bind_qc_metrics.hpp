...
// ---------------------------------------------------------------------------
struct PyQcResult {
    singlet_gpu::core::DeviceMemory<float> n_umis;        // [n_cells]
    singlet_gpu::core::DeviceMemory<int>   n_genes;       // [n_cells]
    singlet_gpu::core::DeviceMemory<float> pct_mt;        // [n_cells]
    singlet_gpu::core::DeviceMemory<float> pct_ribo;      // [n_cells]
    singlet_gpu::core::DeviceMemory<float> gene_mean;     // [n_genes]
    singlet_gpu::core::DeviceMemory<float> gene_var;      // [n_genes]
    singlet_gpu::core::DeviceMemory<int>   gene_n_cells;  // [n_genes]
    int n_cells       = 0;
    int n_genes_total = 0;
...
// ---------------------------------------------------------------------------
inline singlet_gpu::qc::QcResult make_qc_view(const PyQcResult& py_r) {
    singlet_gpu::qc::QcResult v;
    // Non-owning wraps: destructor is a no-op (owns_=false in factornet impl).
    v.n_umis       = singlet_gpu::core::DeviceMemory<float>::wrap(
                         const_cast<float*>(py_r.n_umis.get()), py_r.n_umis.size());
    v.n_genes      = singlet_gpu::core::DeviceMemory<int>::wrap(
                         const_cast<int*>(py_r.n_genes.get()), py_r.n_genes.size());
    v.pct_mt       = singlet_gpu::core::DeviceMemory<float>::wrap(
                         const_cast<float*>(py_r.pct_mt.get()), py_r.pct_mt.size());
    v.pct_ribo     = singlet_gpu::core::DeviceMemory<float>::wrap(
                         const_cast<float*>(py_r.pct_ribo.get()), py_r.pct_ribo.size());
    v.gene_mean    = singlet_gpu::core::DeviceMemory<float>::wrap(
                         const_cast<float*>(py_r.gene_mean.get()), py_r.gene_mean.size());
    v.gene_var     = singlet_gpu::core::DeviceMemory<float>::wrap(
                         const_cast<float*>(py_r.gene_var.get()), py_r.gene_var.size());
    v.gene_n_cells = singlet_gpu::core::DeviceMemory<int>::wrap(
...
inline void bind_qc_metrics(py::module_& m) {
    ...
    m.def("filter_cells",
           ...
           py::object stream_obj) -> py::object
        {
            ...
            singlet_gpu::qc::FilterConfig cfg;
            ...
            singlet_gpu::qc::QcResult qc_view = make_qc_view(qc);

            auto out_csc = singlet_gpu::qc::filter_cells(*mat.csc, qc_view, cfg, s);

            auto py_out = std::make_shared<PyDeviceCsc>(
                std::make_shared<singlet_gpu::core::DeviceCSC>(std::move(out_csc)),
                s);
    ...
    m.def("filter_genes",
           ...
           py::object stream_obj) -> py::object
        {
            ...
            auto out_csc = singlet_gpu::qc::filter_genes(*mat.csc, qc_view, cfg, s);

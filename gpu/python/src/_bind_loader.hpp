...
//
// pybind11 bindings for singlet_gpu::io::load_pz and the PzDeviceMatrix
// result struct.
//
// Design:
...
//
// Wraps singlet_gpu::core::DeviceCSC via std::shared_ptr so Python's GC
...
inline void bind_pz_device_matrix(py::module_& m) {
    ...
    .def_property_readonly("meta",
        [](const PyPzDeviceMatrix& self) -> const singlet_gpu::core::Metadata& {
            return self.meta;
        },
    ...
    m.def("load_pz",
        [](const std::string& path, bool keep_host_pinned) -> std::shared_ptr<PyPzDeviceMatrix> {
            ...
            singlet_gpu::io::PzDeviceMatrix result =
                singlet_gpu::io::load_pz(path, /*stream=*/nullptr, keep_host_pinned);

            // Wrap DeviceCSC in a shared_ptr so cupy views can share ownership.
            auto dev_csc = std::make_shared<PyDeviceCsc>(
                std::make_shared<singlet_gpu::core::DeviceCSC>(std::move(result.mat)),

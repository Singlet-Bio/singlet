...
struct PyCsiGepResult {
    int chosen_k = 0;
    singlet_gpu::core::DeviceMemory<float> consensus_programs;  // chosen_k × n_genes
    singlet_gpu::core::DeviceMemory<float> program_usage;       // n_cells × chosen_k
...
inline void bind_nmf_new(py::module_& m) {
    ...
    m.def("csi_gep",
           ...
           uint64_t seed) -> py::object
        {
            ...
            auto r = singlet_gpu::reduce::nmf::csi_gep(csc, cfg, s);

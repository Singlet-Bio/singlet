...
//
// Wraps singlet_gpu::enrich::score_genes (cycle 129, Phase E benchmark:
...
struct PyScoreGenesResult {
    singlet_gpu::core::DeviceMemory<float> scores;  // n_cells × n_sets, col-major
    int          n_cells = 0;
    int          n_sets  = 0;
...
inline void bind_score_genes(py::module_& m) {
    ...
    m.def("score_genes",
           ...
           py::object    stream) -> py::object
        {
            ...
            auto r = singlet_gpu::enrich::score_genes(pz, gene_sets, cfg, s);

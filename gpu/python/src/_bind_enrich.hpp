...
struct PySsGseaResult {
    singlet_gpu::core::DeviceMemory<float> scores;  // [n_sets × n_cells] col-major
    int   n_sets  = 0;
    int   n_cells = 0;
...
struct PyProgenyResult {
    singlet_gpu::core::DeviceMemory<float> activity;  // [n_cells × n_pathways] row-major
    int   n_cells    = 0;
    int   n_pathways = 0;
...
inline void bind_enrich(py::module_& m) {
    ...
    m.def("ssgsea",
           ...
           uint64_t seed) -> py::object
        {
            ...
            const singlet_gpu::core::DeviceCSC& csc = *mat.csc;

            // Build SsGseaGeneSets from Python lists.
            singlet_gpu::enrich::SsGseaGeneSets gsets;
            ...
            singlet_gpu::enrich::SsGseaConfig cfg;
            cfg.alpha     = alpha;
            ...
            auto r = singlet_gpu::enrich::ssgsea(csc, gsets, cfg, s);
    ...
    m.def("progeny",
           ...
           uint64_t /*seed*/) -> py::object
        {
            ...
            singlet_gpu::enrich::PROGENyWeights weights =
                singlet_gpu::enrich::load_progeny_weights_tsv(
                    weights_tsv, gene_names, name_to_idx);

            // Upload weights to GPU (mutates weights.d_weights + gpu_ready).
            singlet_gpu::enrich::upload_progeny_weights(weights, s);
            ...
            singlet_gpu::enrich::PROGENyConfig cfg;
            cfg.normalize  = normalize;
            cfg.norm_eps   = norm_eps;
            cfg.cell_batch = cell_batch;

            auto r = singlet_gpu::enrich::progeny(csc, weights, cfg, s, cusparse_h);

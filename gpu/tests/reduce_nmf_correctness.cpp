...
//
// Correctness harness for singlet_gpu::reduce::nmf adapters and
// singlet_gpu::io::PzDataLoader.
//
...
// Call singlet_gpu::reduce::nmf::fit(m, cfg) AND
...
TEST(NMF_RoundTrip, Fit_MSE) {
    ...
    const auto result_adapter = singlet_gpu::reduce::nmf::fit(m, cfg);
...
TEST(NMF_RoundTrip, CvFit) {
    ...
    const auto result_adapter = singlet_gpu::reduce::nmf::cv_fit(m, cfg);
...
TEST(NMF_Init, Random_Shape) {
    const auto W = singlet_gpu::reduce::nmf::init_random(
...
TEST(NMF_Init, Deflation_ReusesSvd) {
    ...
    const auto [W, H] = singlet_gpu::reduce::nmf::init_deflation(m, kSmallK, cfg);
...
TEST(NMF_RequireHostPinned, Throws) {
    ...
    EXPECT_THROW(singlet_gpu::reduce::nmf::fit(m, cfg), std::runtime_error)
...
TEST(PzDataLoader, NextForward) {
    ...
    singlet_gpu::io::PzDataLoader loader(kGsm4037629Path, kChunkCols);
...
TEST(NMF_Chunked, SmokeTest) {
    ...
    const auto result = singlet_gpu::reduce::nmf::chunked_fit(loader, cfg);
...
TEST(NMF_Graph, MultiModal_SmokeTest) {
    ...
    // Build the graph.
    // Node type aliases live in singlet_gpu::reduce::nmf::graph (not ::nmf directly).
    using namespace singlet_gpu::reduce::nmf::graph;
    ...
    net.compile();
    const auto graph_result = singlet_gpu::reduce::nmf::graph::fit_graph(net, m);
...
TEST(NMF_EdgeCase, CvFit_ZeroHoldout) {
    ...
    EXPECT_NO_THROW(singlet_gpu::reduce::nmf::cv_fit(m, cfg))
...
TEST(NMF_EdgeCase, ChunkedSingleChunk) {
    ...
    singlet_gpu::io::PzDataLoader loader(kGsm4037629Path, /*chunk_cols=*/1000000);

    auto cfg = make_nmf_config(/*rank=*/5, /*max_iter=*/5);

    EXPECT_NO_THROW(singlet_gpu::reduce::nmf::chunked_fit(loader, cfg))

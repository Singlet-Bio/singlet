...
//
// Correctness harness for singlet_gpu::comm::CellChat.
//
...
// ---------------------------------------------------------------------------
singlet_gpu::core::DeviceCSC host_dense_to_device_csc(
    const float* data, int n_genes, int n_cells, cudaStream_t stream)
{
    ...
    d.cols = n_cells;
    d.nnz  = nnz;
    d.col_ptr    = singlet_gpu::core::DeviceMemory<int>(n_cells + 1);
    d.row_indices= singlet_gpu::core::DeviceMemory<int>(std::max(nnz, 1));
    d.values     = singlet_gpu::core::DeviceMemory<float>(std::max(nnz, 1));
...
// ---------------------------------------------------------------------------
singlet_gpu::comm::CellChatResult compute_cellchat(
    const singlet_gpu::core::DeviceCSC& expr,
    const int*   cell_types_ptr,
    int          n_cells,
    int          n_types,
    const LRPair* lr_db_ptr,
    int          n_lr,
    const singlet_gpu::comm::CellChatConfig& cfg,
    ...
    cudaStream_t                     /*stream*/)
{
    ...
    return singlet_gpu::comm::cellchat_gpu(
...
// ---------------------------------------------------------------------------
singlet_gpu::comm::CellChatConfig make_config(uint64_t seed    = kSeed,
...
protected:
    ...
    singlet_gpu::core::GPUContext* ctx_ptr_ = nullptr;
    cudaStream_t                   stream_{};

    void SetUp() override {
        if (!gpu_available()) GTEST_SKIP() << "No CUDA device available.";
        ctx_ptr_ = &singlet_gpu::core::default_context();
...
TEST_F(CellChatTest, CellChat_TinySynthetic_VsR) {
    ...
    singlet_gpu::comm::CellChatResult result =
        compute_cellchat(
            expr_csc,
            cell_types.data(),
            kTinyCells,
...
TEST_F(CellChatTest, CellChat_GSM_RealData) {
    ...
    using DevCSC = singlet_gpu::core::DeviceCSC;
    DevCSC expr_csc = singlet_gpu::io::load_pz(kGsmExprPath, stream_).mat;

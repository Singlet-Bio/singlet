...
TEST(UlmTest, Ulm_TinyClosedForm)
{
    ...
    singlet_gpu::enrich::UlmConfig cfg;
    auto res  = singlet_gpu::enrich::ulm(pz, d_W.get(), P, cfg);
...
TEST(UlmTest, Ulm_Determinism)
{
    ...
    auto res1 = singlet_gpu::enrich::ulm(pz1, dW1.get(), P, cfg);
    auto h1   = d2h(res1.scores, N * P);

    auto pz2  = upload_csc(csc);
    auto dW2  = upload_W(W_h);
    auto res2 = singlet_gpu::enrich::ulm(pz2, dW2.get(), P, cfg);

...
//   singlet_gpu::io::load() → factornet::gpu::SparseMatrixGPU<float>
//
// Tests:
...
TEST_F(PzDeviceLoaderTest, RealSample_Dimensions) {
    ...
    singlet_gpu::io::PzLoadResult res = singlet_gpu::io::load(kSamplePath);
...
TEST_F(PzDeviceLoaderTest, RealSample_MetadataKeys) {
    ...
    // These fields are written by singlify into every .1pz.
...
TEST_F(PzDeviceLoaderTest, ChunkIteratorVsFullLoad) {
    singlet_gpu::io::PzLoadResult full = singlet_gpu::io::load(kSamplePath);
    ...
    singlet_gpu::io::PzChunkIterator iter(kSamplePath, nullptr, 2000);

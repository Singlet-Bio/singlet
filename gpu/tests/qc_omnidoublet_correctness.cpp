...
//
// Correctness harness for singlet_gpu::qc::omnidoublet.
//
...
// singlet_gpu::DeviceCSC resolves to the real type, and rewrite upload_csc to
...
struct PzDeviceLoader {
    PzLoaded load(const std::string& path, cudaStream_t stream) const {
        PzLoaded out;
        out.pz     = singlet_gpu::io::load_pz(path, stream);
...
// ---------------------------------------------------------------------------
// Upload a HostCSC to device and wrap as singlet_gpu DeviceCSC.
...
// ---------------------------------------------------------------------------
singlet_gpu::DeviceCSC upload_csc(const HostCSC& h, cudaStream_t stream) {
    // Use the synchronous host-data constructor:
...
void free_device_csc(singlet_gpu::DeviceCSC& csc, cudaStream_t stream) {
...
TEST(Omnidoublet, TinySynthetic_VsPython) {
    ...
    singlet_gpu::qc::OmniDoubletConfig cfg;
    ...
    singlet_gpu::qc::OmniDoubletResult result =
        singlet_gpu::qc::omnidoublet(d_rna, d_adt, cfg, stream);
...
TEST(Omnidoublet, MultimodalFeatures_NonZero) {
    ...
    auto result_a = singlet_gpu::qc::omnidoublet(d_rna_a, d_adt_a, cfg, stream);
    ASSERT_EQ(cudaStreamSynchronize(stream), cudaSuccess);
    ...
    auto result_b  = singlet_gpu::qc::omnidoublet(d_rna_b, d_adt_b, cfg, stream);
    ASSERT_EQ(cudaStreamSynchronize(stream), cudaSuccess);
...
TEST(Omnidoublet, Determinism_BitIdentical) {
    ...
    auto run_once = [&]() -> std::vector<double> {
        ...
        auto result = singlet_gpu::qc::omnidoublet(d_rna, d_adt, cfg, stream);

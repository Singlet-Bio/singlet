...
//
// Correctness harness for singlet_gpu::qc::calculate_qc_metrics(),
...
// ---------------------------------------------------------------------------
static singlet_gpu::core::DeviceCSC upload(const HostCSC& h) {
    singlet_gpu::core::DeviceCSC d(h.n_genes, h.n_cells, h.nnz);
    if (h.nnz > 0) {
...
TEST(QcMetrics, TinySynthetic_NUmis) {
    ...
    singlet_gpu::core::DeviceMemory<uint8_t> d_is_mt(200), d_is_ribo(200);
    d_is_mt.upload(is_mt_h.data(), 200);
    d_is_ribo.upload(is_ribo_h.data(), 200);

    auto res = singlet_gpu::qc::calculate_qc_metrics(
...
TEST(QcMetrics, EdgeCase_EmptyColumn) {
    ...
    singlet_gpu::core::DeviceMemory<uint8_t> d_is_mt(10), d_is_ribo(10);
...
TEST(QcMetrics, EdgeCase_ZeroVarianceGene) {
    ...
    singlet_gpu::core::DeviceMemory<uint8_t> d_is_mt(3), d_is_ribo(3);
...
TEST(FilterCells, DimensionsCorrect) {
    ...
    singlet_gpu::core::DeviceMemory<uint8_t> d_is_mt(50), d_is_ribo(50);
    d_is_mt.upload(is_mt_h.data(), 50);
    d_is_ribo.upload(is_ribo_h.data(), 50);

    auto qc = singlet_gpu::qc::calculate_qc_metrics(
        d.col_ptr.get(), d.row_indices.get(), d.values.get(),
    ...
    auto out = singlet_gpu::qc::filter_cells(d, qc, cfg, nullptr);
...
TEST(FilterGenes, MinCells) {
    ...
    auto out = singlet_gpu::qc::filter_genes(d, qc, cfg, nullptr);

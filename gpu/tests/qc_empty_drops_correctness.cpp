...
//
// Correctness harness for singlet_gpu::qc::empty_drops.
//
// Tests:
...
// Upload a HostCSC to a singlet_gpu::io::PzDeviceMatrix.
singlet_gpu::io::PzDeviceMatrix upload_csc(const HostCSC& h, cudaStream_t stream) {
...
std::vector<float> download_f32(const singlet_gpu::core::DeviceMemory<float>& d,
...
std::vector<uint8_t> download_u8(const singlet_gpu::core::DeviceMemory<uint8_t>& d,
                                  size_t count, cudaStream_t stream) {
...
TEST_F(EmptyDropsFixture, EmptyDrops_AllEmpty_NoCellsCalled) {
    ...
    singlet_gpu::qc::EmptyDropsConfig cfg;
    cfg.lower  = 100;
    cfg.niters = 100;
    cfg.seed   = 0ULL;

    auto result = singlet_gpu::qc::empty_drops(pz, cfg, stream_);
...
TEST_F(EmptyDropsFixture, EmptyDrops_Determinism_SameSeed) {
    ...
    auto result_a = singlet_gpu::qc::empty_drops(pz, cfg, stream_);
    auto result_b = singlet_gpu::qc::empty_drops(pz, cfg, stream_);
...
TEST_F(EmptyDropsFixture, EmptyDrops_FdrThreshold_ControlsCalls) {
    ...
    singlet_gpu::qc::EmptyDropsConfig cfg_hi;
    cfg_hi.lower     = 100;
    cfg_hi.niters    = 500;
    cfg_hi.fdr_thresh = 0.01f;
    cfg_hi.seed      = 0ULL;

    singlet_gpu::qc::EmptyDropsConfig cfg_lo = cfg_hi;
    cfg_lo.fdr_thresh = 0.001f;

    auto result_hi = singlet_gpu::qc::empty_drops(pz, cfg_hi, stream_);
    auto result_lo = singlet_gpu::qc::empty_drops(pz, cfg_lo, stream_);

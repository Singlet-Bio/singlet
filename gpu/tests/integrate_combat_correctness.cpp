...
// GPU implementation via singlet_gpu::integrate::combat().
//
// Tests:
...
singlet_gpu::core::DeviceMemory<int> upload_batch(const std::vector<int>& b)
{
    singlet_gpu::core::DeviceMemory<int> d(b.size());
...
TEST(CombatTest, Combat_SingleBatch_Identity)
{
    ...
    singlet_gpu::integrate::CombatConfig cfg;
    auto res = singlet_gpu::integrate::combat(pz, d_bat.get(), 1, cfg);
...
TEST(CombatTest, Combat_TwoBatches_RemovesShift)
{
    ...
    singlet_gpu::integrate::CombatConfig cfg;
    auto res = singlet_gpu::integrate::combat(pz, d_bat.get(), 2, cfg);
    auto out = d2h(res.X_adj, M * N);
...
TEST(CombatTest, Combat_Determinism)
{
    ...
    auto res1 = singlet_gpu::integrate::combat(pz1, db1.get(), NB, cfg);
    auto h1   = d2h(res1.X_adj, M * N);

    auto pz2  = upload_csc(csc);
    auto db2  = upload_batch(batch_h);
    auto res2 = singlet_gpu::integrate::combat(pz2, db2.get(), NB, cfg);
...
TEST(CombatTest, Combat_MemoryGuard)
{
    ...
    singlet_gpu::io::PzDeviceMatrix pz;
    pz.mat = singlet_gpu::core::DeviceCSC(M_BIG, N_BIG, /*nnz=*/0);
    ...
    singlet_gpu::integrate::CombatConfig cfg;
    EXPECT_THROW(
        singlet_gpu::integrate::combat(pz, d_bat.get(), NB, cfg),
        std::runtime_error);

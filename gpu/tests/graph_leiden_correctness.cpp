...
//
// Correctness harness for singlet_gpu::graph::leiden and leiden_multi.
//
...
// singlet_gpu::graph::KnnResult.  The KnnResult owns DeviceMemory<int> and
// DeviceMemory<float> as in cycle 8.
// =============================================================================

singlet_gpu::graph::KnnResult upload_knn_to_device(
...
TEST(Leiden, TinySynthetic_BlockStochastic_ARI) {
    ...
    // Run Leiden.
    using namespace singlet_gpu::graph;
    LeidenConfig cfg;
    cfg.resolution      = 1.0f;
    cfg.seed            = 42;
...
TEST(Leiden, RealEmbedding_AriVsScanpy) {
    ...
    auto knn = singlet_gpu::graph::compute_knn(embed_dev, knn_cfg, stream);
    ...
    auto result = singlet_gpu::graph::leiden(knn, cfg, stream);

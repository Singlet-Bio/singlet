...
// -------------------------------------------------------------------------
singlet_gpu::core::SparseSymmetricCSR make_csr(
    int n, int nnz,
    const std::vector<float>& h_val,
    const std::vector<int>&   h_row,
    const std::vector<int>&   h_col)
{
    singlet_gpu::core::SparseSymmetricCSR A;
    A.n   = n;
    A.nnz = nnz;
...
// -------------------------------------------------------------------------
singlet_gpu::core::SparseSymmetricCSR build_tridiagonal(
    int n,
...
// -------------------------------------------------------------------------
singlet_gpu::core::SparseSymmetricCSR build_random_sparse_symmetric(
...
TEST_F(SparseEigTest, TinyEigSymmetric)
{
    ...
    singlet_gpu::core::SparseEigConfig<float> cfg;
    cfg.n_components = K;
    cfg.max_iter     = 300;
    cfg.tolerance    = 1e-7f;
    cfg.seed         = static_cast<uint64_t>(kSeed);

    auto result = singlet_gpu::core::top_k_eigsh_lobpcg(A, cfg, nullptr);
...
TEST_F(SparseEigTest, Determinism_BitIdentical)
{
    ...
    auto r1 = singlet_gpu::core::top_k_eigsh_lobpcg(A1, cfg, nullptr);
    auto r2 = singlet_gpu::core::top_k_eigsh_lobpcg(A2, cfg, nullptr);

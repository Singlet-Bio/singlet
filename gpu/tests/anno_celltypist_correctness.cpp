...
//
// Correctness harness for singlet_gpu::anno::celltypist_predict.
//
// Tests:
...
TEST_F(CelltypistFixture, Celltypist_TinyClosedForm) {
    ...
    auto result = singlet_gpu::anno::celltypist_predict(
...
TEST_F(CelltypistFixture, Celltypist_Determinism_BitIdentical) {
    ...
    auto r1 = singlet_gpu::anno::celltypist_predict(
        d_Z.get(), F, C, d_W.get(), d_b.get(), K, {}, stream_);
    auto r2 = singlet_gpu::anno::celltypist_predict(
        d_Z.get(), F, C, d_W.get(), d_b.get(), K, {}, stream_);

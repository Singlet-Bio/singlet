...
//
// Correctness harness for singlet_gpu::spatial::flash_deconv.
//
...
struct PzDeviceLoader {
    ...
    core::DeviceCSC load_csc(cudaStream_t stream) const {
        return singlet_gpu::io::load_pz(path_, stream).mat;
    }
};
} }
...
TEST_F(FlashDeconvTest, FlashDeconv_TinySynthetic_VsCell2Location) {
    ...
    singlet_gpu::spatial::FlashDeconvConfigCompat cfg;
    cfg.sketch_k = kSketchK;
    cfg.seed     = kSeed;

    auto result = singlet_gpu::spatial::flash_deconv_fit(
...
TEST_F(FlashDeconvTest, FlashDeconv_Determinism_BitIdentical) {
    ...
    auto B1 = make_B_dev();
    auto r1 = singlet_gpu::spatial::flash_deconv_fit(
        Y_dev1, B1, nullptr, kTinyTypes, kTinyGenes, cfg, stream_);

    auto B2 = make_B_dev();
    auto r2 = singlet_gpu::spatial::flash_deconv_fit(
...
TEST_F(FlashDeconvTest, FlashDeconv_SpatialRegularization_SmoothsAbundance) {
    ...
    singlet_gpu::spatial::FlashDeconvConfigCompat cfg_base;
    cfg_base.sketch_k       = 30;
    cfg_base.spatial_lambda = 0.0f;
    cfg_base.seed           = kSeed;

    auto B1   = make_B_dev();
    auto r_base = singlet_gpu::spatial::flash_deconv_fit(
        Y_dev_base, B1, nullptr, kSpatTypes, kSpatGenes, cfg_base, stream_);

    // Run with spatial regularization.
    singlet_gpu::spatial::FlashDeconvConfigCompat cfg_reg;
    cfg_reg.sketch_k       = 30;
    ...
    auto B2   = make_B_dev();
    auto r_reg = singlet_gpu::spatial::flash_deconv_fit(

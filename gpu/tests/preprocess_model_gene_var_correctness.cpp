...
std::vector<float> d2h_f(const singlet_gpu::core::DeviceMemory<float>& d, int n)
{
...
std::vector<int> d2h_i(const singlet_gpu::core::DeviceMemory<int>& d, int n)
{
    std::vector<int> h(n);
...
TEST(ModelGeneVarTest, Tiny_Closed_Form)
{
    ...
    singlet_gpu::preprocess::ModelGeneVarConfig cfg;
    cfg.n_top = M; // keep all genes
    auto res  = singlet_gpu::preprocess::model_gene_var(pz, cfg);

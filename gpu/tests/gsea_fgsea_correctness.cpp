...
//
// Correctness harness for singlet_gpu::gsea::fgsea.
//
...
// ---------------------------------------------------------------------------
singlet_gpu::anno::GeneSetDB make_planted_genesets(int m, int n_sets,
                                                    int genes_per_set) {
...
TEST(Fgsea, TinyPlanted_VsR) {
    ...
    singlet_gpu::gsea::FgseaResult our_res =
        singlet_gpu::gsea::fgsea(d_stats, db, cfg, stream);
    ...
    std::map<std::string, singlet_gpu::gsea::PathwayResult> our_map;
...
TEST(Fgsea, RealMSigDB_TopHallmarks) {
    ...
    auto our_res = singlet_gpu::gsea::fgsea(d_stats, db, cfg, stream);
    cudaStreamDestroy(stream);
    ...
    std::vector<std::string> our_top, ref_top;
    {
        ...
        std::sort(sorted_ours.begin(), sorted_ours.end(),
                  [](const singlet_gpu::gsea::PathwayResult& a,
                     const singlet_gpu::gsea::PathwayResult& b){
...
TEST(Fgsea, AdaptivePermutation_StopsEarly) {
    ...
    auto res = singlet_gpu::gsea::fgsea(d_stats, db, cfg, stream);
    cudaStreamDestroy(stream);
...
TEST(Fgsea, Determinism_BitIdentical) {
    ...
    auto res1 = singlet_gpu::gsea::fgsea(d_stats1, db, cfg, stream);

    auto d_stats2 = upload_stats(stats, stream);
    auto res2 = singlet_gpu::gsea::fgsea(d_stats2, db, cfg, stream);
    ...
    auto sort_by_name = [](singlet_gpu::gsea::FgseaResult& r) {

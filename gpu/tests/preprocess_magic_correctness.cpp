...
singlet_gpu::graph::SnnResult build_snn_result(
    int n, int nnz,
...
{
    singlet_gpu::graph::SnnResult res;
    res.n   = n;
    res.nnz = nnz;

    res.row_offsets = singlet_gpu::core::DeviceMemory<int>(n + 1);
    res.col_indices = singlet_gpu::core::DeviceMemory<int>(nnz > 0 ? nnz : 1);
    res.weights     = singlet_gpu::core::DeviceMemory<float>(nnz > 0 ? nnz : 1);
...
singlet_gpu::graph::SnnResult build_identity_graph(int n)
{
...
TEST(MagicImputeTest, Identity_Graph_NoOp)
{
    ...
    singlet_gpu::preprocess::MagicConfig cfg;
    cfg.t = 3;
    auto result = singlet_gpu::preprocess::magic_impute(pz, graph, cfg);
...
TEST(MagicImputeTest, Determinism_SameInput_SameOutput)
{
    ...
    auto res1   = singlet_gpu::preprocess::magic_impute(pz1, graph1, cfg);
    ...
    auto res2   = singlet_gpu::preprocess::magic_impute(pz2, graph2, cfg);
...
TEST(MagicImputeTest, MemoryGuard_RejectsTooLarge)
{
    ...
    singlet_gpu::io::PzDeviceMatrix fake_pz;
    ...
    fake_pz.mat.col_ptr     = singlet_gpu::core::DeviceMemory<int32_t>(1);
    fake_pz.mat.row_indices = singlet_gpu::core::DeviceMemory<int32_t>(1);
    fake_pz.mat.values      = singlet_gpu::core::DeviceMemory<float>(1);
    ...
    singlet_gpu::graph::SnnResult fake_graph;
    fake_graph.n   = N_FAKE;
    fake_graph.nnz = 0;
    fake_graph.row_offsets = singlet_gpu::core::DeviceMemory<int>(N_FAKE + 1);
    fake_graph.col_indices = singlet_gpu::core::DeviceMemory<int>(1);
    fake_graph.weights     = singlet_gpu::core::DeviceMemory<float>(1);
    ...
    EXPECT_THROW(
        singlet_gpu::preprocess::magic_impute(fake_pz, fake_graph, cfg),
        std::runtime_error)

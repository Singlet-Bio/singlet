...
//
// Correctness harness for singlet_gpu::graph::compute_knn + compute_snn.
//
...
//
// The real singlet_gpu::core::DeviceDense type is defined in core/types.h.
...
//   const singlet_gpu::core::DeviceDense&
...
singlet_gpu::core::DeviceDense upload_dense(const HostDense& h, cudaStream_t stream) {
...
copy_knn_result_to_host(const singlet_gpu::graph::KnnResult& res,
                        cudaStream_t stream) {
    int n = res.n;
    int k = res.k;
...
TEST_F(KnnCorrectnessTest, Knn_Exact_TinySynthetic_BitIdenticalToScikit) {
    ...
    auto result = singlet_gpu::graph::compute_knn(d_emb, cfg, stream_);
    ASSERT_EQ(result.n, kTinyN);
    ASSERT_EQ(result.k, kTinyK);
    ASSERT_EQ(result.backend_used, singlet_gpu::graph::KnnBackend::Exact);
...
TEST_F(KnnCorrectnessTest, Knn_AutoSelect_RoutingTable) {
    ...
    auto result = singlet_gpu::graph::compute_knn(d_emb, cfg, stream_);
    EXPECT_EQ(result.backend_used, singlet_gpu::graph::KnnBackend::Exact)
...
TEST_F(KnnCorrectnessTest, Knn_ReturnSquared_NoSqrt) {
    ...
    singlet_gpu::graph::KnnConfig cfg_normal;
    cfg_normal.k              = kTinyK;
    cfg_normal.backend        = singlet_gpu::graph::KnnBackend::Exact;
    cfg_normal.return_squared = false;

    auto res_normal = singlet_gpu::graph::compute_knn(d_emb, cfg_normal, stream_);
    auto [h_nb_n, h_dist_n] = copy_knn_result_to_host(res_normal, stream_);

    // Squared run.
    singlet_gpu::graph::KnnConfig cfg_sq;
    cfg_sq.k              = kTinyK;
    cfg_sq.backend        = singlet_gpu::graph::KnnBackend::Exact;
    cfg_sq.return_squared = true;

    auto res_sq = singlet_gpu::graph::compute_knn(d_emb, cfg_sq, stream_);
...
TEST_F(KnnCorrectnessTest, Knn_CagraBackend_RecallAtK) {
    ...
    auto res_exact = singlet_gpu::graph::compute_knn(d_emb, cfg_exact, stream_);
    auto [h_nb_ex, h_dist_ex] = copy_knn_result_to_host(res_exact, stream_);

    // CAGRA run (direct build+search, no HNSW conversion).
    singlet_gpu::graph::KnnConfig cfg_cagra;
    cfg_cagra.k       = kMedK;
    cfg_cagra.backend = singlet_gpu::graph::KnnBackend::Cagra;
    cfg_cagra.seed    = kSeed;
    // Use defaults: graph_degree=64, intermediate_graph_degree=128, search_width=1, itopk=auto.

    auto res_cagra = singlet_gpu::graph::compute_knn(d_emb, cfg_cagra, stream_);
    ASSERT_EQ(res_cagra.backend_used, singlet_gpu::graph::KnnBackend::Cagra);
...
TEST_F(KnnCorrectnessTest, Knn_Determinism_RepeatedRunsBitIdentical) {
    ...
    // First run.
    auto res1 = singlet_gpu::graph::compute_knn(d_emb, cfg, stream_);
    auto [h_nb1, h_dist1] = copy_knn_result_to_host(res1, stream_);

    // Second run — same input, same config.
    auto res2 = singlet_gpu::graph::compute_knn(d_emb, cfg, stream_);
...
TEST_F(KnnCorrectnessTest, Snn_Jaccard_TinySynthetic_MatchesNumpyReference) {
    ...
    auto knn_res = singlet_gpu::graph::compute_knn(d_emb, knn_cfg, stream_);
    ...
    singlet_gpu::graph::SnnConfig snn_cfg;
    snn_cfg.prune_threshold = 1.0f / 15.0f;
    singlet_gpu::graph::SnnResult snn_res =
        singlet_gpu::graph::compute_snn(knn_res, snn_cfg, stream_);

...
std::vector<int> d2h_int(const singlet_gpu::core::DeviceMemory<int>& d, int n)
{
    std::vector<int> h(n);
...
TEST(LisiTest, Lisi_PerfectMixing_HighScore)
{
    ...
    singlet_gpu::integrate::LisiConfig cfg;
    cfg.deterministic = true;
    auto res = singlet_gpu::integrate::lisi(knn, d_lbl.get(), N_LABELS, cfg);
...
TEST(LisiTest, Lisi_Determinism_BitIdentical)
{
    ...
    auto res1 = singlet_gpu::integrate::lisi(knn1, d_lbl.get(), N_LABELS, cfg);
    auto h1   = d2h_float(res1.lisi, N);

    // Run 2 (re-upload).
    auto knn2 = build_knn_result(N, K, make_row_offsets(N, K), h_nbr, h_dist);
    auto res2 = singlet_gpu::integrate::lisi(knn2, d_lbl.get(), N_LABELS, cfg);

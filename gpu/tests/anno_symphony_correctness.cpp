...
//
// Correctness harness for singlet_gpu::anno::symphony_predict.
//
// Tests:
...
TEST_F(SymphonyFixture, Symphony_TinyClosedForm) {
    ...
    auto result = singlet_gpu::anno::symphony_predict(
        d_Z.get(), F, C,
        d_W.get(), P,
        d_C.get(), K,
        d_P.get(), L,
...
TEST_F(SymphonyFixture, Symphony_Determinism_BitIdentical) {
    ...
    auto r1 = singlet_gpu::anno::symphony_predict(
        d_Z.get(), F, C, d_W.get(), P, d_C.get(), K,
        d_P.get(), L, d_mu.get(), d_sigma.get(), {}, stream_);
    auto r2 = singlet_gpu::anno::symphony_predict(
        d_Z.get(), F, C, d_W.get(), P, d_C.get(), K,
        d_P.get(), L, d_mu.get(), d_sigma.get(), {}, stream_);

...
//
// Correctness harness for singlet_gpu::spatial::stagate_fit.
//
...
namespace singlet_gpu { namespace spatial {
...
} } // namespace singlet_gpu::spatial

// Map StagateResult to StagateResultCompat globally
// so test code that declares `singlet_gpu::spatial::StagateResultCompat result = singlet_gpu::spatial::stagate_fit(...)` compiles.
...
// The tests use `singlet_gpu::spatial::StagateResultCompat result = singlet_gpu::spatial::stagate_fit(...)` — since stagate_fit
...
std::vector<int32_t> download_i32(
        const singlet_gpu::core::DeviceMemory<int32_t>& d,
        size_t count, cudaStream_t stream)
{
...
TEST(Stagate, Stagate_TinySynthetic_VsPyG) {
    ...
    singlet_gpu::core::DeviceMemory<float> d_coords(
    ...
    using namespace singlet_gpu::spatial;
    StagateConfig cfg;
    cfg.d_embed      = kDEmbed;
    ...
    singlet_gpu::spatial::StagateResultCompat result = singlet_gpu::spatial::stagate_fit(d_counts, d_coords, kTinySpots, cfg, stream);
    cudaStreamSynchronize(stream);
...
TEST(Stagate, Stagate_DLPFC_VsBenchmark) {
    ...
    singlet_gpu::spatial::StagateResultCompat result = singlet_gpu::spatial::stagate_fit(d_counts, d_coords, kDlfpcSpots, cfg, stream);
    cudaStreamSynchronize(stream);
...
TEST(Stagate, Stagate_Determinism_BitIdentical) {
    ...
    auto upload_coords = [&]() {
        singlet_gpu::core::DeviceMemory<float> d(
            static_cast<size_t>(kTinySpots) * 2);

...
//
// Correctness harness for singlet_gpu::qc::doublet_score.
//
...
// actual singlet_gpu::qc and singlet_gpu::io APIs.  All at global scope.
// =============================================================================
    ...
    cudaStream_t           stream)
{
    ...
    DoubletScoreResult result = ::singlet_gpu::qc::doublet_score(embedding, cfg, stream);
...
// (Production code uses singlet_gpu::reduce::pca; this test fixture avoids
...
// Run singlet_gpu::qc::doublet_score on a PCA projection of the counts.
...
TEST_F(DoubletFixture, Doublet_TinySynthetic_VsScrublet) {
    ...
    singlet_gpu::core::DeviceMemory<float> d_scores(kTinyCells);
    singlet_gpu::qc::doublet_score(
        d_pca.get(),
...
TEST_F(DoubletFixture, Doublet_GSM4037629_RealData) {
    ...
    singlet_gpu::io::PzDeviceLoader loader(kGsmCountsPath);
    ...
    // (Production would call singlet_gpu::reduce::pca directly on the device.)
    ...
    singlet_gpu::core::DeviceMemory<float> d_scores(n_cells);
    singlet_gpu::qc::doublet_score(
        d_pca.get(),
    ...
    singlet_gpu::qc::DoubletResult result;
    singlet_gpu::qc::doublet_score_with_result(
        d_pca.get(),
        static_cast<int>(n_cells),
        static_cast<int>(pca.n_cols),
        d_scores.get(),
...
TEST_F(DoubletFixture, Doublet_AutoThreshold_ROC) {
    ...
    singlet_gpu::core::DeviceMemory<float> d_scores(kRocCells);
    singlet_gpu::qc::DoubletResult result;
    singlet_gpu::qc::doublet_score_with_result(
        d_pca.get(),
        kRocCells,
...
TEST_F(DoubletFixture, Doublet_Determinism_BitIdentical) {
    ...
    // First run.
    singlet_gpu::core::DeviceMemory<float> d_scores_a(kTinyCells);
    singlet_gpu::qc::doublet_score(
    ...
    singlet_gpu::core::DeviceMemory<float> d_scores_b(kTinyCells);
    singlet_gpu::qc::doublet_score(
        d_pca.get(), kTinyCells, static_cast<int>(pca.n_cols),
...
TEST_F(DoubletFixture, Doublet_NSynth_Sensitivity) {
    ...
    for (double frac : kNSynthFracs) {
        ...
        singlet_gpu::core::DeviceMemory<float> d_scores(kSensCells);
        singlet_gpu::qc::doublet_score(

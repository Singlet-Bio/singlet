...
//
// Correctness harness for singlet_gpu::network::hdwgcna.
//
...
TEST_F(HdwgcnaTest, Hdwgcna_TinySynthetic_VsR) {
    ...
    singlet_gpu::network::HdwgcnaParams params;
    ...
    singlet_gpu::DeviceCSC dev_csc =
        singlet_gpu::testing::dense_to_device_csc(expr.data(), kTinyCells, kTinyGenes);

    singlet_gpu::network::HdwgcnaResult result =
        singlet_gpu::network::run_hdwgcna(dev_csc, params);
...
TEST_F(HdwgcnaTest, Hdwgcna_GSM_RealData) {
    ...
    // Load .1pz → device CSC
    singlet_gpu::io::PzDeviceLoader loader;
    singlet_gpu::DeviceCSC raw_csc = loader.load(kRealDataPath);

    // Log-normalize (reuse cycle 2)
    singlet_gpu::preprocess::LognormParams lp;
    lp.target_sum = 1e4f;
    singlet_gpu::DeviceCSC lognorm_csc =
        singlet_gpu::preprocess::lognorm(raw_csc, lp);

    // Select top HVG (reuse cycle 3)
    singlet_gpu::preprocess::HvgParams hvg_p;
    hvg_p.n_top_genes = kRealTopHVG;
    singlet_gpu::DeviceCSC hvg_csc =
        singlet_gpu::preprocess::select_hvg(lognorm_csc, hvg_p);
    ...
    singlet_gpu::network::HdwgcnaResult result =
        singlet_gpu::network::run_hdwgcna(hvg_csc, params);
...
TEST_F(HdwgcnaTest, Hdwgcna_Determinism_BitIdentical) {
    ...
    // Run once
    singlet_gpu::network::HdwgcnaResult r1 =
        singlet_gpu::network::run_hdwgcna(dev_csc, params);

    // Run again (same device CSC, same params)
    singlet_gpu::network::HdwgcnaResult r2 =
        singlet_gpu::network::run_hdwgcna(dev_csc, params);

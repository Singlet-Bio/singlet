...
std::vector<float> d2h_float(const singlet_gpu::core::DeviceMemory<float>& d, size_t n)
{
...
singlet_gpu::io::PzDeviceMatrix build_pz(
...
TEST(DendrogramTest, Dendrogram_TinyClosedForm)
{
    ...
    auto res = singlet_gpu::embed::dendrogram(pz, d_label_mem.get(), K);
...
TEST(DendrogramTest, Dendrogram_EmptyCluster_Handled)
{
    ...
    singlet_gpu::embed::DendrogramResult res;
    ASSERT_NO_THROW({
        res = singlet_gpu::embed::dendrogram(pz, d_label_mem.get(), K);
    });

...
// ---------------------------------------------------------------------------
HostCSC device_csc_to_host(const singlet_gpu::core::DeviceCSC& d,
                             cudaStream_t stream) {
    HostCSC h;
...
static E2ePipelineOutputs run_pipeline(uint64_t seed = kSeed) {
    ...
    // Stage 1 — Load .1pz
    // -----------------------------------------------------------------------
    singlet_gpu::core::GPUContext ctx(0);   // device 0
    cudaStream_t stream = ctx.stream();

    auto dcsc = singlet_gpu::io::load_pz(kGsmPath, ctx);
    ...
    // -----------------------------------------------------------------------
    singlet_gpu::preprocess::LognormConfig lncfg;
    lncfg.method = singlet_gpu::preprocess::LognormMethod::TotalCount;
    auto lnorm_result = singlet_gpu::preprocess::lognorm(dcsc, lncfg, ctx);
    ...
    // Stage 3 — HVG
    // -----------------------------------------------------------------------
    singlet_gpu::preprocess::HvgConfig hvgcfg;
    hvgcfg.n_top_genes = kNHvg;
    hvgcfg.flavor      = singlet_gpu::preprocess::HvgFlavor::SeuratV3;
    auto hvg_result = singlet_gpu::preprocess::select_hvg(dcsc, hvgcfg, ctx);
    ...
    singlet_gpu::reduce::svd::RandomizedSvdConfig svdcfg;
    svdcfg.k            = kNPcs;
    svdcfg.n_iterations = 4;
    svdcfg.seed         = seed;
    auto svd_result = singlet_gpu::reduce::svd::randomized(
    ...
    // -----------------------------------------------------------------------
    singlet_gpu::graph::KnnConfig knncfg;
    knncfg.k       = kKnn;
    knncfg.metric  = singlet_gpu::graph::KnnMetric::Euclidean;
    knncfg.backend = singlet_gpu::graph::KnnBackend::Auto;
    knncfg.seed    = static_cast<uint64_t>(seed);
    // PCA cell embedding: svd_result.U (n_cells × k, row-major on device)
    auto knn_result = singlet_gpu::graph::compute_knn(
    ...
    // Stage 6 — Leiden
    // -----------------------------------------------------------------------
    singlet_gpu::graph::LeidenConfig leidencfg;
    leidencfg.resolution = kLeidenRes;
    leidencfg.seed       = static_cast<uint64_t>(seed);
    auto leiden_result = singlet_gpu::graph::leiden(knn_result, leidencfg, ctx);
    ...
    // Stage 7 — UMAP
    // -----------------------------------------------------------------------
    singlet_gpu::embed::UmapConfig umapcfg;
    umapcfg.n_components = 2;
    umapcfg.seed         = static_cast<uint64_t>(seed);
    auto umap_result = singlet_gpu::embed::umap(knn_result, umapcfg, ctx);
    ...
    // Find top-2 clusters.
    {
        ...
        singlet_gpu::de::WilcoxonConfig decfg;
        decfg.group_a_label = clust_a;
        decfg.group_b_label = clust_b;
        auto de_result = singlet_gpu::de::wilcoxon_de(
            lnorm_result.normalized,
    ...
    singlet_gpu::anno::MarkerScoreConfig mscfg;
    mscfg.method = singlet_gpu::anno::MarkerScoreMethod::UCell;
    ...
    auto ms_result = singlet_gpu::anno::marker_score(
        lnorm_result.normalized,
        gene_sets,
        mscfg,
        ctx);
...
TEST_F(IntegrationE2eTest, Integration_E2e_Full_GSM4037629) {
    ...
    // (fallback path — in case singlify.io is not installed in the Python env).
    // -----------------------------------------------------------------------
    ...
    {
        singlet_gpu::core::GPUContext ctx2(0);
        auto dcsc_raw = singlet_gpu::io::load_pz(kGsmPath, ctx2);
    ...
    if (!ref_ok) {
        ...
        EXPECT_EQ(our.n_cells, 20866) << "Stage 1: expected 20866 cells (singlify v0.3)";
...
TEST_F(IntegrationE2eTest, Integration_E2e_KnnWrapperValidates) {
    ...
    singlet_gpu::core::GPUContext ctx(0);

    // Upload to device.
    singlet_gpu::core::DeviceMemory<float> d_X(static_cast<size_t>(kN) * kD);
    ...
    singlet_gpu::graph::KnnConfig cfg_auto;
    cfg_auto.k       = kK;
    cfg_auto.metric  = singlet_gpu::graph::KnnMetric::Euclidean;
    cfg_auto.backend = singlet_gpu::graph::KnnBackend::Auto;
    cfg_auto.seed    = kSeed;
    auto knn_auto = singlet_gpu::graph::compute_knn(d_X.get(), kN, kD, cfg_auto, ctx);

    // Path B: compute_knn with Exact backend (direct call — no routing).
    singlet_gpu::graph::KnnConfig cfg_exact;
    cfg_exact.k       = kK;
    cfg_exact.metric  = singlet_gpu::graph::KnnMetric::Euclidean;
    cfg_exact.backend = singlet_gpu::graph::KnnBackend::Exact;
    cfg_exact.seed    = kSeed;
    auto knn_exact = singlet_gpu::graph::compute_knn(d_X.get(), kN, kD, cfg_exact, ctx);
...
TEST_F(IntegrationE2eTest, Integration_E2e_HarmonyPciePassive) {
    ...
    // Run stages 1–4 only (the PCIe-sensitive path: load → lognorm → HVG → SVD).
    {
        ...
        singlet_gpu::preprocess::LognormConfig lncfg;
        lncfg.method = singlet_gpu::preprocess::LognormMethod::TotalCount;
        auto lnorm  = singlet_gpu::preprocess::lognorm(dcsc, lncfg, ctx);
        singlet_gpu::preprocess::HvgConfig hvgcfg;
        hvgcfg.n_top_genes = kNHvg;
        hvgcfg.flavor      = singlet_gpu::preprocess::HvgFlavor::SeuratV3;
        auto hvg    = singlet_gpu::preprocess::select_hvg(dcsc, hvgcfg, ctx);
        singlet_gpu::reduce::svd::RandomizedSvdConfig svdcfg;
        svdcfg.k = kNPcs; svdcfg.seed = kSeed;
        auto svd    = singlet_gpu::reduce::svd::randomized(

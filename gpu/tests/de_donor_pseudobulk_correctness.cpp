...
//             exploits singlify donor_assignments.tsv directly)
//
// Correctness harness for singlet_gpu::de::donor_pseudobulk_de.
//
...
// ---------------------------------------------------------------------------
// Parse donor_assignments.tsv (singlify output).
...
TEST(DonorPseudobulk, TinyPlanted_VsDESeq2) {
    ...
    singlet_gpu::core::DeviceCSC d_mat = upload_csc(h, stream);

    singlet_gpu::core::DeviceMemory<int> d_clusters(kTinyCells);
    singlet_gpu::core::DeviceMemory<int> d_donors  (kTinyCells);
    ...
    singlet_gpu::de::DonorPseudobulkResult result =
        singlet_gpu::de::donor_pseudobulk_de(
...
//
// Load GSM4037629 exon_counts.1pz + donor_assignments.tsv from singlify.
...
TEST(DonorPseudobulk, GSM4037629_RealData) {
    ...
    singlet_gpu::core::DeviceCSC d_mat = singlet_gpu::io::load_pz(kGsmCountsPath, stream).mat;
    ASSERT_EQ(cudaStreamSynchronize(stream), cudaSuccess);
    ...
    // Read donor assignments from singlify output.
    ...
    // Upload labels.
    singlet_gpu::core::DeviceMemory<int> d_clusters(n_cells);
    singlet_gpu::core::DeviceMemory<int> d_donors  (n_cells);
    ASSERT_EQ(cudaMemcpyAsync(d_clusters.get(), cluster_labels.data(),
                               n_cells * sizeof(int),
                               cudaMemcpyHostToDevice, stream), cudaSuccess);
    ASSERT_EQ(cudaMemcpyAsync(d_donors.get(), donor_labels.data(),
...
TEST(DonorPseudobulk, MinCellsFilter_DropsSparseGroups) {
    ...
    singlet_gpu::core::DeviceMemory<int> d_clusters(kC);
    singlet_gpu::core::DeviceMemory<int> d_donors  (kC);
    ...
    singlet_gpu::de::DonorPseudobulkResult res_filtered =
        singlet_gpu::de::donor_pseudobulk_de(
    ...
    singlet_gpu::de::DonorPseudobulkResult res_unfiltered =
        singlet_gpu::de::donor_pseudobulk_de(
            d_mat, d_clusters, kK, d_donors, kD, cfg, stream);
    ASSERT_EQ(cudaStreamSynchronize(stream), cudaSuccess);
...
TEST(DonorPseudobulk, DispersionShrinkage_Convergence) {
    ...
    singlet_gpu::de::DonorPseudobulkConfig cfg_shrink;
    cfg_shrink.apeglm_shrinkage = true;
    cfg_shrink.seed = kSeed;

    singlet_gpu::de::DonorPseudobulkResult res_shrink =
        singlet_gpu::de::donor_pseudobulk_de(
    ...
    singlet_gpu::de::DonorPseudobulkConfig cfg_noshrink;
    cfg_noshrink.apeglm_shrinkage = false;
    cfg_noshrink.seed = kSeed;

    singlet_gpu::de::DonorPseudobulkResult res_noshrink =
        singlet_gpu::de::donor_pseudobulk_de(
            d_mat, d_clusters, kTinyClusters, d_donors, kTinyDonors,
            cfg_noshrink, stream);
    ASSERT_EQ(cudaStreamSynchronize(stream), cudaSuccess);
...
TEST(DonorPseudobulk, Determinism_BitIdentical) {
    ...
    auto run_kernel = [&](cudaStream_t stream)
        -> singlet_gpu::de::DonorPseudobulkResult
    {
        singlet_gpu::core::DeviceCSC d_mat = upload_csc(h, stream);
        singlet_gpu::core::DeviceMemory<int> d_clusters(kTinyCells);
        singlet_gpu::core::DeviceMemory<int> d_donors  (kTinyCells);
        ...
        return singlet_gpu::de::donor_pseudobulk_de(
            d_mat, d_clusters, kTinyClusters, d_donors, kTinyDonors, cfg, stream);
    };

    singlet_gpu::de::DonorPseudobulkResult r1 = run_kernel(s1);
    ASSERT_EQ(cudaStreamSynchronize(s1), cudaSuccess);

    singlet_gpu::de::DonorPseudobulkResult r2 = run_kernel(s2);
...
TEST(DonorPseudobulk, FewDonorsErrors) {
    ...
    singlet_gpu::core::DeviceMemory<int> d_clusters(kSmallCells);
    singlet_gpu::core::DeviceMemory<int> d_donors  (kSmallCells);
    ASSERT_EQ(cudaMemcpyAsync(d_clusters.get(), clusters.data(),

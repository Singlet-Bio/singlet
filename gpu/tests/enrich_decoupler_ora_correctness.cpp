...
TEST(OraTest, Ora_TinyClosedForm)
{
    ...
    singlet_gpu::enrich::OraConfig cfg;
    cfg.top_k = 4;
    auto res = singlet_gpu::enrich::ora(pz, sets, cfg);
...
TEST(OraTest, Ora_Determinism)
{
    ...
    auto res1 = singlet_gpu::enrich::ora(pz1, sets, cfg);
    auto h1   = d2h(res1.scores, N_CELLS * 3);

    auto pz2  = upload_csc(csc);
    auto res2 = singlet_gpu::enrich::ora(pz2, sets, cfg);

...
//
// Correctness harness for singlet_gpu::perturbation::perturb_graph_fit.
//
...
struct PerturbGraphResultCompat {
    ...
    singlet_gpu::core::DeviceMemory<float> pred_held_out;
...
inline PerturbGraphResultCompat perturb_graph_fit(
    ...
    const singlet_gpu::core::DeviceMemory<int32_t>&  d_labels,
    const singlet_gpu::core::DeviceMemory<float>&    d_doses,
    const singlet_gpu::perturbation::PerturbGraphConfig& cfg,
    cudaStream_t /*stream*/)
{
    ...
    auto base = singlet_gpu::perturbation::train_perturb_graph(
...
inline singlet_gpu::core::DeviceMemory<float> perturb_graph_predict_doses(
    const PerturbGraphResultCompat::ModelHandle&,
    int, const std::vector<float>&, cudaStream_t) {
    return singlet_gpu::core::DeviceMemory<float>();
}

// Stub: perturb_graph_encode_zc
inline singlet_gpu::core::DeviceMemory<float> perturb_graph_encode_zc(
    const PerturbGraphResultCompat::ModelHandle&,
    const singlet_gpu::core::DeviceCSC&, cudaStream_t) {
    return singlet_gpu::core::DeviceMemory<float>();
}
...
TEST(PerturbGraph, PerturbGraph_TinySynthetic_VsCPA) {
    ...
    using namespace singlet_gpu::perturbation;
    ...
    singlet_gpu::core::DeviceMemory<float>   d_doses (kTinyCells);
...
TEST(PerturbGraph, PerturbGraph_HeldOutPerturbation_Smoke) {
    ...
    singlet_gpu::core::DeviceCSC d_counts = upload_csc(h_train, stream);

    singlet_gpu::core::DeviceMemory<int32_t> d_labels(n_train);
    singlet_gpu::core::DeviceMemory<float>   d_doses (n_train);
...
TEST(PerturbGraph, PerturbGraph_DoseResponse_Monotone) {
    ...
    singlet_gpu::core::DeviceCSC d_counts = upload_csc(h, stream);
    singlet_gpu::core::DeviceMemory<int32_t> d_labels(kDoseCells);
    singlet_gpu::core::DeviceMemory<float>   d_doses (kDoseCells);
    cudaMemcpyAsync(d_labels.get(), labels.data(),
    ...
    singlet_gpu::core::DeviceMemory<float> d_dose_preds =
...
inline PerturbGraphState perturb_graph_init(
    const singlet_gpu::core::DeviceCSC&,
    const singlet_gpu::core::DeviceMemory<int32_t>&,
    const singlet_gpu::core::DeviceMemory<float>&,
    const singlet_gpu::perturbation::PerturbGraphConfig&,
    cudaStream_t) { return {}; }
...
TEST(PerturbGraph, PerturbGraph_GradientCheck_FiniteDiff) {
    ...
    singlet_gpu::core::DeviceCSC d_counts = upload_csc(h, stream);
    singlet_gpu::core::DeviceMemory<int32_t> d_labels(kGradCells);
    singlet_gpu::core::DeviceMemory<float>   d_doses (kGradCells);
    cudaMemcpyAsync(d_labels.get(), labels.data(),
...
TEST(PerturbGraph, PerturbGraph_AdversarialLoss_Disentangle) {
    ...
    singlet_gpu::core::DeviceCSC d_counts = upload_csc(h, stream);
    singlet_gpu::core::DeviceMemory<int32_t> d_labels(kAdvCells);
    singlet_gpu::core::DeviceMemory<float>   d_doses (kAdvCells);
    cudaMemcpyAsync(d_labels.get(), labels.data(),
    ...
    singlet_gpu::core::DeviceMemory<float> d_zc =
...
TEST(PerturbGraph, PerturbGraph_GNNExtension_UnseenPerturbation) {
    ...
    singlet_gpu::core::DeviceCSC d_train = upload_csc(h_train, stream);
    singlet_gpu::core::DeviceMemory<int32_t> d_labels(n_train);
    singlet_gpu::core::DeviceMemory<float>   d_doses (n_train);
    cudaMemcpyAsync(d_labels.get(), train_labels.data(),
                    n_train * sizeof(int32_t), cudaMemcpyHostToDevice, stream);

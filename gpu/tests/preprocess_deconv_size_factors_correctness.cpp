...
//   singlet_gpu::preprocess::compute_deconv_size_factors
...
// ---------------------------------------------------------------------------
static singlet_gpu::core::DeviceCSC upload_csc(
    int n_genes, int n_cells,
    ...
    cudaStream_t stream)
{
    ...
    singlet_gpu::core::DeviceCSC mat(n_genes, n_cells, nnz,
...
TEST(DeconvSizeFactors, RealData_GSM4037629_IfScranAvailable) {
    ...
    singlet_gpu::io::PzDeviceMatrix pz =
        singlet_gpu::io::load_pz(kGsm4037629GeneCounts, stream,
...
TEST(DeconvSizeFactors, TwoClusterScaling) {
    ...
    singlet_gpu::core::DeviceMemory<int> d_labels(kCells);

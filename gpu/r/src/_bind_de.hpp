...
//   wilcoxon_de_cpp  — wraps singlet_gpu::de::wilcoxon_de
//   ttest_de_cpp     — wraps singlet_gpu::de::ttest_de
//
// Both functions take:
...
//
// Convert a singlet_gpu::de::ClusterMarkers result (device-resident) into an
// R List.  O(top_n) device→host copies per cluster.
// ---------------------------------------------------------------------------
inline Rcpp::List
cluster_markers_to_r_list(const singlet_gpu::de::ClusterMarkers& cm, int top_n)
{
    if (top_n <= 0) {
        ...
        bool                deterministic)
{
    ...
    // Upload labels
    int n_clusters = 0;
    singlet_gpu::core::DeviceMemory<int> d_labels;
    ...
    // Run kernel
    singlet_gpu::de::WilcoxonResult res;
    try {
        res = singlet_gpu::de::wilcoxon_de(d, d_labels, n_clusters, cfg, stream);
        ...
        bool                deterministic)
{
    ...
    singlet_gpu::de::TtestResult res;
    try {
        res = singlet_gpu::de::ttest_de(d, d_labels, n_clusters, cfg, stream);
    } catch (const std::exception& e) {

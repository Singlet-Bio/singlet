...
//   detect_cna_cpp — wraps singlet_gpu::cna::detect_cna
//
        ...
        std::string          resource)
{
    ...
    singlet_gpu::cna::GeneAnnotation annot;
    ...
    singlet_gpu::core::DeviceMemory<float> d_ref;
    if (!h_ref.empty()) {
        d_ref = singlet_gpu::core::DeviceMemory<float>(static_cast<std::size_t>(n_genes));
    ...
    // ── Run kernel ─────────────────────────────────────────────────────────────
    singlet_gpu::cna::CnaResult res;
    try {
        res = singlet_gpu::cna::detect_cna(d_expr, annot, ref_ptr, cfg, stream);
    ...
    const int n_states = singlet_gpu::cna::HMM_NSTATES;

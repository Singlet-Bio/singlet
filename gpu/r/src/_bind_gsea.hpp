...
//   fgsea_cpp    — wraps singlet_gpu::gsea::fgsea_gpu
//   aucell_cpp   — wraps singlet_gpu::gsea::aucell_gpu
//
// Input format:
//   fgsea_cpp:
...
// ---------------------------------------------------------------------------
inline singlet_gpu::gsea::PathwayDB
        ...
        int                  seed)
{
    ...
    singlet_gpu::core::DeviceMemory<float> d_stats(n_genes);
    {
    ...
    singlet_gpu::gsea::FgseaResult res;
    try {
        res = singlet_gpu::gsea::fgsea_gpu(d_stats, n_genes, db, cfg, stream);
    } catch (const std::exception& e) {
        ...
        bool                 norm_auc)
{
    ...
    try {
        res = singlet_gpu::gsea::aucell_gpu(d, db, cfg, stream);
    } catch (const std::exception& e) {

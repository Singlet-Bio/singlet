...
//   pseudobulk_aggregate_cpp  — wraps singlet_gpu::de::pseudobulk_aggregate
//   donor_pseudobulk_de_cpp   — wraps singlet_gpu::de::donor_pseudobulk_de
//
// Input format:
...
//
// Design note: donor_assignments come from singlify's donor_assignments.tsv
        ...
        int                 min_cells)
{
    ...
    singlet_gpu::core::DeviceMemory<int> d_cluster(n_cells), d_donor(n_cells);
    {
    ...
    singlet_gpu::de::PseudobulkAggregateConfig cfg;
    cfg.min_cells  = min_cells;
    cfg.n_clusters = n_clusters;
    cfg.n_donors   = n_donors;

    singlet_gpu::de::PseudobulkAggregateResult res;
    try {
        res = singlet_gpu::de::pseudobulk_aggregate(d, d_cluster, d_donor,
                                                     n_cells, cfg, stream);
    } catch (const std::exception& e) {
        ...
        int                 seed)
{
    ...
    singlet_gpu::core::DeviceDense d_pb;
    ...
    singlet_gpu::core::DeviceMemory<int> d_cluster(n_pb), d_donor(n_pb);
    {
        auto chk = [](cudaError_t e, const char* t) {
            if (e != cudaSuccess)
                throw std::runtime_error(std::string(t) + ": " + cudaGetErrorString(e));
    ...
    singlet_gpu::de::DonorPseudobulkDeConfig cfg;
    cfg.n_clusters       = n_clusters;
    cfg.n_donors         = n_donors;
    cfg.apeglm_shrinkage = apeglm_shrinkage;
    cfg.seed             = static_cast<uint64_t>(seed);

    singlet_gpu::de::DonorPseudobulkDeResult res;
    try {
        res = singlet_gpu::de::donor_pseudobulk_de(d_pb, d_cluster, d_donor, cfg, stream);
    } catch (const std::exception& e) {

...
//   daesc_cpp — wraps singlet_gpu::ase::run_daesc
//
...
//
// Note: snp_ad and snp_dp come from singlify's snp_ad.1pz / snp_dp.1pz.
        ...
        std::string   resource)
{
    ...
    // ── Configure ─────────────────────────────────────────────────────────────
    singlet_gpu::ase::DaescConfig cfg;
    cfg.chunk_snps  = chunk_snps > 0 ? chunk_snps : singlet_gpu::ase::DEFAULT_CHUNK_SNPS;
    ...
    // ── Run kernel ─────────────────────────────────────────────────────────────
    singlet_gpu::ase::DaescResult res;
    try {
        res = singlet_gpu::ase::run_daesc(
            d_ad, d_dp,
